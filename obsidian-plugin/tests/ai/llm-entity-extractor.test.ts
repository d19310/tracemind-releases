...
import { buildExtractionPrompt, parseLLMResponse, type LLMExtractionResult } from '../../src/ai/llm-entity-extractor';
...
describe('LLM Entity Extractor - Prompt Builder', () => {
  it('includes system instructions and diary text in prompt', () => {
    ...
    assert.ok(prompt.includes('Extract all named entities'));
...
describe('LLM Entity Extractor - Response Parser', () => {
  it('parses valid JSON response with person entities', () => {
    ...
    assert.equal(result[1].name, 'Q2计划');
    assert.equal(result[1].type, 'object');
    assert.equal(result[1].subtype, 'project');
  });
