...
describe('Context Card - Create Theme', () => {
  it('creates a Theme card with subtype', () => {
    const card = ContextCard.create({
      name: 'AI技术',
      cardType: 'theme',
      attributes: { subtype: 'friction' },
    });

    assert.equal(card.cardType, 'theme');
    assert.equal(card.attributes.subtype, 'friction');
  });
});
...
describe('Context Card - Create with Maturity', () => {
  ...
  it('creates Object card with L1 maturity when project P0 attributes are filled', () => {
    const card = ContextCard.create({
      name: 'Q2项目',
      cardType: 'object',
      attributes: {
  ...
  it('creates Theme card with L1 maturity when friction P0 attributes are filled', () => {
    const card = ContextCard.create({
      name: '需求反复',
      cardType: 'theme',
      attributes: {
