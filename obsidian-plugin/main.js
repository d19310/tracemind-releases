"use strict";var rn=Object.create;var ce=Object.defineProperty;var an=Object.getOwnPropertyDescriptor;var sn=Object.getOwnPropertyNames;var on=Object.getPrototypeOf,ln=Object.prototype.hasOwnProperty;var Ie=(e,t)=>()=>(e&&(t=e(e=0)),t);var Be=(e,t)=>{for(var i in t)ce(e,i,{get:t[i],enumerable:!0})},mt=(e,t,i,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let r of sn(t))!ln.call(e,r)&&r!==i&&ce(e,r,{get:()=>t[r],enumerable:!(n=an(t,r))||n.enumerable});return e};var De=(e,t,i)=>(i=e!=null?rn(on(e)):{},mt(t||!e||!e.__esModule?ce(i,"default",{value:e,enumerable:!0}):i,e)),cn=e=>mt(ce({},"__esModule",{value:!0}),e);var Re={};Be(Re,{buildRequest:()=>Le,chat:()=>ue,extractStreamDelta:()=>Pe,parseResponse:()=>yt,streamChat:()=>_e,validateConfig:()=>un});function un(e){return e.provider?!e.apiKey||e.apiKey.trim()===""?{valid:!1,error:"apiKey is required"}:!e.model||e.model.trim()===""?{valid:!1,error:"model is required"}:{valid:!0}:{valid:!1,error:"provider is required"}}function Le(e,t){let i=t.find(r=>r.role==="system"),n=t.filter(r=>r.role!=="system");switch(e.provider){case"openai":{let r=(e.baseUrl||"https://api.openai.com").replace(/\/+$/,""),s=r.endsWith("/v1")?"/chat/completions":"/v1/chat/completions";return{url:`${r}${s}`,method:"POST",headers:{Authorization:`Bearer ${e.apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:e.model,messages:t})}}case"anthropic":return{url:e.baseUrl||"https://api.anthropic.com/v1/messages",method:"POST",headers:{"x-api-key":e.apiKey,"anthropic-version":"2023-06-01","Content-Type":"application/json"},body:JSON.stringify({model:e.model,system:i?.content,messages:n.map(r=>({role:r.role,content:r.content}))})};default:{let s=(e.baseUrl||(e.provider==="ollama"?"http://localhost:11434":"")).replace(/\/+$/,""),a=s.endsWith("/v1")?"/chat/completions":"/v1/chat/completions";return{url:`${s}${a}`,method:"POST",headers:{"Content-Type":"application/json",...e.apiKey?{Authorization:`Bearer ${e.apiKey}`}:{}},body:JSON.stringify({model:e.model,messages:t})}}}}async function ue(e,t){let i=Le(t,e),n=await fetch(i.url,{method:i.method||"POST",headers:i.headers,body:i.body});if(!n.ok){let a=await n.text();throw new Error(`HTTP ${n.status}: ${a}`)}let r=await n.json();return{content:yt(t.provider,r).content}}function Pe(e,t){if(!t.startsWith("data: "))return"";let i=t.slice(6).trim();if(!i||i==="[DONE]")return"";try{let n=JSON.parse(i);if(e==="anthropic")return n.type==="content_block_delta"&&n.delta?.text?n.delta.text:"";let r=n.choices;return r&&r.length>0&&r[0].delta?.content?r[0].delta.content:""}catch{return""}}async function _e(e,t,i){try{let n=Le(t,e),r=JSON.parse(n.body||"{}");r.stream=!0;let s={...n.headers,Accept:"text/event-stream"},a=await fetch(n.url,{method:n.method||"POST",headers:s,body:JSON.stringify(r)});if(!a.ok){let d=await a.text();throw new Error(`HTTP ${a.status}: ${d}`)}if(!a.body)throw new Error("Response body is null \u2014 streaming not supported");let o=a.body.getReader(),l=new TextDecoder,c="",u="";for(;;){let{done:d,value:p}=await o.read();if(d)break;u+=l.decode(p,{stream:!0});let f=u.split(`
`);u=f.pop()||"";for(let g of f){let h=g.trim();if(!h)continue;let m=Pe(t.provider,h);m&&(c+=m,i.onDelta(m))}}if(u.trim()){let d=Pe(t.provider,u.trim());d&&(c+=d,i.onDelta(d))}i.onDone(c)}catch(n){i.onError(n)}}function yt(e,t){switch(e){case"openai":{let i=t.choices;if(!i||i.length===0)throw new Error("Empty response from OpenAI");return{role:"assistant",content:i[0].message.content}}case"anthropic":{let i=t.content;if(!i||i.length===0)throw new Error("Empty response from Anthropic");return{role:"assistant",content:i[0].text}}default:{let i=t.choices;if(!i||i.length===0)throw new Error("Empty response from AI provider");return{role:"assistant",content:i[0].message.content}}}}var z=Ie(()=>{"use strict"});async function He(e){let{execSync:t}=await import("node:child_process");try{return t(`which ${e} 2>/dev/null || where ${e} 2>nul`,{encoding:"utf-8",timeout:5e3}).trim().split(`
`)[0]||null}catch{return null}}var St=Ie(()=>{"use strict"});var Dt={};Be(Dt,{claudeCodeProvider:()=>xn});var It,Bt,Mt,yn,xn,Pt=Ie(()=>{"use strict";St();It=require("node:child_process"),Bt=De(require("node:readline")),Mt="claude",yn=600*1e3,xn={name:"Claude Code",description:"Anthropic \u51FA\u54C1\u7684\u672C\u5730 AI \u7F16\u7A0B agent\uFF0C\u901A\u8FC7 claude CLI \u8C03\u7528",async detect(){let e=await He(Mt);if(!e)return!1;try{let{execSync:t}=await import("node:child_process"),i=t(`${e} --version 2>&1`,{encoding:"utf-8",timeout:1e4});return/[Cc]laude|[0-9]+\.[0-9]+\.[0-9]+/.test(i)}catch{return!1}},execute(e,t){let i=null,n=!1,r=null,s=null,a=null,o=Date.now(),l=t?.timeoutMs||yn;return(async()=>{try{let u=await He(Mt);if(!u){let x=new Error("\u627E\u4E0D\u5230 claude CLI\uFF0C\u8BF7\u786E\u8BA4\u5DF2\u5B89\u88C5 Claude Code");a?.(x),s?.({status:"failed",output:"",error:x.message,durationMs:Date.now()-o});return}let d=["-p","--output-format","stream-json","--input-format","stream-json","--verbose"];t?.model&&d.push("--model",t.model);let p={...process.env};if(t?.env&&Object.assign(p,t.env),i=(0,It.spawn)(u,d,{env:p,cwd:t?.cwd||process.cwd(),stdio:["pipe","pipe","pipe"]}),n){i.kill();return}i.on("error",x=>{a?.(x),s?.({status:"failed",output:"",error:x.message,durationMs:Date.now()-o})});let f=setTimeout(()=>{i&&!i.killed&&(i.kill(),s?.({status:"timeout",output:"",error:"\u6267\u884C\u8D85\u65F6",durationMs:Date.now()-o}))},l),g=Bt.createInterface({input:i.stdout,crlfDelay:1/0}),h="",m="";g.on("line",x=>{if(x.trim())try{let M=JSON.parse(x);switch(M.type){case"system":M.session_id&&(h=M.session_id),M.subtype==="init"&&r?.({type:"status",content:"running",sessionId:h});break;case"assistant":{let Me=M.message?.content||[];for(let S of Me)S.type==="text"&&S.text?(m+=S.text,r?.({type:"text",content:S.text})):S.type==="tool_use"&&r?.({type:"tool-use",toolName:S.name,toolInput:S.input});break}case"user":{let Me=M.message?.content||[];for(let S of Me)S.type==="tool_use"&&S.name&&r?.({type:"tool-result",toolName:S.name});break}case"result":clearTimeout(f);let gt=M.is_error||M.subtype==="error_during_execution",nn=M.output||m;s?.({status:gt?"failed":"completed",output:nn,error:gt?M.output||"\u672A\u77E5\u9519\u8BEF":void 0,durationMs:Date.now()-o});break}}catch{}}),g.on("close",()=>{clearTimeout(f),n||s?.({status:"completed",output:m,durationMs:Date.now()-o})});let v=JSON.stringify({type:"user",message:{role:"user",content:e}});i.stdin.write(v+`
`),i.stdin.end()}catch(u){a?.(u),s?.({status:"failed",output:"",error:u.message,durationMs:Date.now()-o})}})(),{set onMessage(u){r=u},get onMessage(){return r},set onDone(u){s=u},get onDone(){return s},set onError(u){a=u},get onError(){return a},abort(){n=!0,i&&!i.killed&&i.kill()}}}}});var ws={};Be(ws,{TraceMindPlugin:()=>Se,default:()=>bs});module.exports=cn(ws);var B=require("obsidian");var b=require("obsidian");z();z();var de=class extends b.PluginSettingTab{plugin;constructor(t,i){super(t,i),this.plugin=i}display(){let{containerEl:t}=this;t.empty(),t.createEl("h2",{text:"TraceMind \u8BBE\u7F6E"}),t.createEl("h3",{text:"AI Provider"});let i="",n="",r="",s="",a=!1,o="";new b.Setting(t).setName("\u540D\u79F0").setDesc("Provider \u663E\u793A\u540D\u79F0").addText(u=>{u.setPlaceholder("My GPT-4").onChange(d=>{i=d})}),new b.Setting(t).setName("\u6A21\u578B").setDesc("\u6A21\u578B\u540D\u79F0\uFF0C\u5982 gpt-4\u3001qwen-plus").addText(u=>{u.setPlaceholder("gpt-4").onChange(d=>{n=d})}),new b.Setting(t).setName("Base URL").setDesc("OpenAI \u517C\u5BB9 API \u5730\u5740").addText(u=>{u.setPlaceholder("https://api.openai.com/v1").onChange(d=>{r=d})}),new b.Setting(t).setName("API Key").setDesc("API \u5BC6\u94A5").addText(u=>{u.setPlaceholder("").onChange(d=>{s=d}),u.inputEl.type="password"}),new b.Setting(t).setName("\u601D\u8003\u6A21\u5F0F").setDesc('\u5F00\u542F\u540E\u8BF7\u6C42\u4F53\u5E26\u4E0A {"thinking":{"type":"enabled"}}').addToggle(u=>{u.setValue(a).onChange(d=>{a=d})}),new b.Setting(t).setName("Reasoning Effort").setDesc("\u90E8\u5206\u6A21\u578B\u652F\u6301 high \u6216 max").addDropdown(u=>{u.addOption("","\u9ED8\u8BA4").addOption("high","high").addOption("max","max").setValue(o).onChange(d=>{o=d})}),new b.Setting(t).addButton(u=>{u.setButtonText("\u6DFB\u52A0 Provider"),u.setCta(),u.onClick(async()=>{if(!i||!n||!r){new b.Notice("\u8BF7\u586B\u5199\u540D\u79F0\u3001\u6A21\u578B\u548C Base URL");return}let d=`provider-${Date.now()}`;this.plugin.settings.providers.push({id:d,name:i,model:n,baseUrl:r,apiKey:s,enableThinking:a,reasoningEffort:o}),await this.plugin.saveSettings(),this.display(),new b.Notice("Provider \u5DF2\u6DFB\u52A0")})});for(let u=0;u<this.plugin.settings.providers.length;u++){let d=this.plugin.settings.providers[u],p=this.plugin.settings.defaultProviderId===d.id,f=new b.Setting(t).setName(`${d.name}${p?" (\u9ED8\u8BA4)":""}`).setDesc(`${d.baseUrl} / ${d.model}${d.enableThinking?" / thinking:on":""}${d.reasoningEffort?` / reasoning:${d.reasoningEffort}`:""}`);f.addToggle(g=>{g.setTooltip("\u8BBE\u4E3A\u9ED8\u8BA4").setValue(p).onChange(async h=>{h&&(this.plugin.settings.defaultProviderId=d.id),await this.plugin.saveSettings(),this.display()})}),f.addToggle(g=>{g.setTooltip("\u601D\u8003\u6A21\u5F0F").setValue(d.enableThinking??!1).onChange(async h=>{d.enableThinking=h,await this.plugin.saveSettings()})}),f.addDropdown(g=>{g.addOption("","\u9ED8\u8BA4").addOption("high","high").addOption("max","max").setValue(d.reasoningEffort||"").onChange(async h=>{d.reasoningEffort=h,await this.plugin.saveSettings()})}),f.addButton(g=>{g.setButtonText("\u6D4B\u8BD5"),g.onClick(async()=>{new b.Notice("\u6B63\u5728\u6D4B\u8BD5...");try{let h=await ue([{role:"user",content:"\u4F60\u597D"}],{provider:d.baseUrl?.includes("anthropic.com")?"anthropic":"openai",apiKey:d.apiKey,model:d.model,baseUrl:d.baseUrl});new b.Notice("\u8FDE\u63A5\u6210\u529F: "+h.content.substring(0,50))}catch(h){new b.Notice("\u8FDE\u63A5\u5931\u8D25: "+h.message)}})}),f.addButton(g=>{g.setButtonText("\u5220\u9664"),g.onClick(async()=>{this.plugin.settings.providers.splice(u,1),this.plugin.settings.defaultProviderId===d.id&&(this.plugin.settings.defaultProviderId=this.plugin.settings.providers[0]?.id||"");let h=this.plugin.settings.agentProviderMapping;h.analysis===d.id&&(h.analysis=""),h.chat===d.id&&(h.chat=""),await this.plugin.saveSettings(),this.display()})})}this.plugin.settings.providers.length===0&&t.createEl("p",{text:"\u6682\u65E0 Provider\uFF0C\u8BF7\u6DFB\u52A0\u4E00\u4E2A",cls:"lifewiki-no-providers"}),t.createEl("h3",{text:"Agent \u914D\u7F6E"});let l=this.plugin.settings.agentProviderMapping,c={};for(let u of this.plugin.settings.providers)c[u.id]=u.name;new b.Setting(t).setName("AI \u5206\u6790").setDesc("\u65E5\u8BB0\u5206\u6790\u4F7F\u7528\u7684 AI Provider").addDropdown(u=>{u.addOption("","\u4F7F\u7528\u9ED8\u8BA4 Provider");for(let[d,p]of Object.entries(c))u.addOption(d,p);u.setValue(l.analysis).onChange(async d=>{this.plugin.settings.agentProviderMapping.analysis=d,await this.plugin.saveSettings()})}),new b.Setting(t).setName("AI \u804A\u5929").setDesc("\u804A\u5929\u4F7F\u7528\u7684 AI Provider").addDropdown(u=>{u.addOption("","\u4F7F\u7528\u9ED8\u8BA4 Provider");for(let[d,p]of Object.entries(c))u.addOption(d,p);u.setValue(l.chat).onChange(async d=>{this.plugin.settings.agentProviderMapping.chat=d,await this.plugin.saveSettings()})}),t.createEl("h3",{text:"\u672C\u5730 Agent\uFF08\u53EF\u9009\uFF09"}),t.createEl("p",{text:"\u4F7F\u7528\u672C\u5730\u5B89\u88C5\u7684 AI agent CLI\uFF08\u5982 Claude Code\uFF09\u4EE3\u66FF\u4E91\u7AEF API\u3002\u9700\u8981\u5148\u5728\u7EC8\u7AEF\u5B89\u88C5\u5BF9\u5E94\u7684 CLI \u5DE5\u5177\u3002",cls:"setting-item-description"}),new b.Setting(t).setName("\u672C\u5730 Agent").setDesc("\u9009\u62E9\u672C\u5730 agent \u540E\uFF0C\u804A\u5929\u6A21\u5F0F\u5C06\u4F7F\u7528\u672C\u5730 CLI \u800C\u975E\u4E91\u7AEF API\u3002\u7559\u7A7A\u5219\u4F7F\u7528\u4E91\u7AEF Provider\u3002").addDropdown(u=>{u.addOption("","\u4E0D\u4F7F\u7528\uFF08\u4E91\u7AEF API\uFF09"),u.addOption("claude-code","Claude Code"),u.setValue(this.plugin.settings.localAgentProvider||"").onChange(async d=>{this.plugin.settings.localAgentProvider=d,await this.plugin.saveSettings()})})}};var xt={providers:[],defaultProviderId:"",agentProviderMapping:{analysis:"",chat:""},localAgentProvider:""};var E=require("obsidian");var Oe=require("obsidian"),dn=".lifewiki/templates";async function pn(e,t){let i=`templates/${t}`,n=e.getAbstractFileByPath(i);if(n instanceof Oe.TFile)return await e.read(n);let r=`${dn}/${t}`;return n=e.getAbstractFileByPath(r),n instanceof Oe.TFile?await e.read(n):null}function Q(e,t){return t.split(".").reduce((i,n)=>i?.[n],e)}function hn(e,t){let i=e,n=/\{\{#if\s+([^}]+)\}\}([\s\S]*?)\{\{\/if\}\}/g;i=i.replace(n,(s,a,o)=>{if(!Q(t,a.trim())){let u=o.split(/\{\{else\}\}/);return u.length>1?u[1].trim():""}return o.split(/\{\{else\}\}/)[0].trim()});let r=/\{\{#each\s+([^}]+)\}\}([\s\S]*?)\{\{\/each\}\}/g;return i=i.replace(r,(s,a,o)=>{let l=Q(t,a.trim());return!Array.isArray(l)||l.length===0?"":l.map(c=>{let u=o;return u=u.replace(/\{\{this\.([^}]+)\}\}/g,(d,p)=>Q(c,p.trim())??""),u=u.replace(/\{\{([^#\/][^}]*?)\}\}/g,(d,p)=>{let f=p.trim();return f==="this"?String(c):Q(c,f)??""}),(typeof c=="string"||typeof c=="number")&&(u=u.replace(/\{\{this\}\}/g,String(c))),u}).join("")}),i}function fn(e,t){let i=e,n=/\{\{([^#\/][^}]*?)\}\}/g;return i=i.replace(n,(r,s)=>{let a=s.trim(),o=Q(t,a);return o==null?"":typeof o=="object"?JSON.stringify(o):String(o)}),i}async function vt(e,t,i){let n=await pn(e,t);n===null&&(console.warn(`[TemplateLoader] Template not found: ${t}, falling back to default`),n=gn(t,i));let r=hn(n,i);return r=fn(r,i),r}function gn(e,t){switch(e.replace("-template.md","")){case"journal":return`# ${t.date||"Untitled"}
...
\u4F1A\u8BDD\u8BB0\u5F55\u5C06\u88AB\u6C38\u4E45\u5220\u9664\u3002`)}confirm(s)&&await this.deleteBlock(t,i,n,r)}async deleteBlock(t,i,n,r){try{if(n&&i?await this.deleteChildBlockFromFile(t,i):await this.deleteParentBlockFromFile(t,r),r){let a=this.plugin.getSessionManager(),o=i||t;await a.clearSession(o)}await this.loadBlocks(),this.renderBlocks(),this.selectedBlockId=null,this.selectedBlockContent=null;let s=this.plugin.getAIAnalysisView();s&&s.setActiveBlock(null,null)}catch(s){console.error("[LifeWiki] Error deleting block:",s),alert("\u5220\u9664\u5931\u8D25: "+(s instanceof Error?s.message:"\u672A\u77E5\u9519\u8BEF"))}}async deleteChildBlockFromFile(t,i){let n=`Daily/${this.currentDate}.md`,r=this.app.vault.getAbstractFileByPath(n);if((!r||!(r instanceof E.TFile))&&(r=this.app.vault.getAbstractFileByPath(`${this.currentDate}.md`)),(!r||!(r instanceof E.TFile))&&(r=this.app.vault.getAbstractFileByPath(`${J}/${this.currentDate}.md`)),!(r instanceof E.TFile))return;let a=(await this.app.vault.read(r)).split(`
`),o=new RegExp(`^- \\d{2}:\\d{2}\\s.+<!-- ${t} -->`),l=a.filter(u=>!u.match(o));await this.app.vault.modify(r,l.join(`
`));let c=this.blocks.find(u=>u.id===i);c&&(c.children=c.children.filter(u=>u.id!==t))}async deleteParentBlockFromFile(t,i){let n=`Daily/${this.currentDate}.md`,r=this.app.vault.getAbstractFileByPath(n);if((!r||!(r instanceof E.TFile))&&(r=this.app.vault.getAbstractFileByPath(`${this.currentDate}.md`)),(!r||!(r instanceof E.TFile))&&(r=this.app.vault.getAbstractFileByPath(`${J}/${this.currentDate}.md`)),!(r instanceof E.TFile))return;let a=(await this.app.vault.read(r)).split(`
...
`)){let p=d.indexOf(":");if(p>0){let f=d.slice(0,p).trim(),g=d.slice(p+1).trim();f&&g&&f!=="id"&&f!=="name"&&l.push(f+": "+g)}}let c=a.replace(/^---\n[\s\S]*?\n---\n?/,"").trim(),u="=== "+s.name+` \u6863\u6848\u6458\u8981 ===
\u7C7B\u578B\uFF1A`+s.cardType+`
...
`)});let f=i.getChatSession().messages,g={role:"system",content:this.buildChatSystemPrompt()},h="";await n.streamChat([g,...f],{onDelta:m=>{h+=m;let v=this.getLastAssistantContent()||"";this.updateLastAssistantMessage(v+m),this.scrollToBottom()},onDone:async()=>{h&&i.addChatMessage({role:"assistant",content:h}),await this.finalizeLastAssistantMessage()},onError:m=>{console.error("Follow-up AI stream error:",m)}},"chat")}}else d.text?(this.updateLastAssistantMessage(d.text),i.addChatMessage({role:"assistant",content:d.text})):i.addChatMessage({role:"assistant",content:u}),await this.finalizeLastAssistantMessage();this.isLoading=!1,this.updateSendBtnState()},onError:u=>{console.error("AI chat error:",u),this.hideThinkingIndicator(),this.addChatMessage("assistant","\u62B1\u6B49\uFF0CAI \u54CD\u5E94\u5931\u8D25: "+u.message),this.isLoading=!1,this.updateSendBtnState()}},"chat")}catch(l){console.error("AI chat error:",l),this.hideThinkingIndicator(),this.addChatMessage("assistant","\u62B1\u6B49\uFF0CAI \u54CD\u5E94\u5931\u8D25: "+l.message),this.isLoading=!1,this.updateSendBtnState()}}updateLastAssistantMessage(t){if(!this.chatMessagesEl)return;let i=this.chatMessagesEl.querySelectorAll(".lifewiki-chat-msg.assistant"),n=i[i.length-1];if(n){n.empty();let r=t.replace(/\[TRACEMIND_ACTION\][\s\S]*?\[\/TRACEMIND_ACTION\]/g,"").replace(/\[TRACEMIND_ACTION\][\s\S]*$/,"").replace(/\[\/TRACEMIND_ACTION\]/g,"").replace(/\[TRACEMIND_ACTION\]/g,"");n.createEl("pre",{cls:"lifewiki-chat-streaming",text:r||"..."})}}getLastAssistantContent(){if(!this.chatMessagesEl)return"";let t=this.chatMessagesEl.querySelectorAll(".lifewiki-chat-msg.assistant");return t[t.length-1]?.textContent||""}async finalizeLastAssistantMessage(){if(!this.chatMessagesEl)return;let t=this.chatMessagesEl.querySelectorAll(".lifewiki-chat-msg.assistant"),i=t[t.length-1];if(i){let n=i.textContent||"";n&&await this.renderMessageMarkdown(i,n)}}async handleEntityArchiving(t){let i=this.plugin.getEntityManager();if(i)for(let n of t)try{let r={status:"active",source:"diary"};n.type==="person"?(r.person_kind=/公司|组织|机构|团队/.test(n.smallType||n.context)?"\u7EC4\u7EC7":"\u4E2A\u4EBA",/同事|朋友|客户|供应商|合作伙伴|合作方/.test(n.smallType)&&(r.relationship_to_user=n.smallType)):n.type==="object"?(r.subtype=n.smallType||"other",n.context&&(r.description=n.context)):n.type==="theme"&&(r.subtype=n.smallType||"friction");let s=n.context||`\u4ECE\u65E5\u8BB0\u4E2D\u5F52\u6863\u7684${n.type}`;await i.createEntity({type:n.type,title:n.name,titleRaw:n.name,aliases:[],tags:[],summary:s,confidence:.8,verificationStatus:"verified",createdAt:new Date().toISOString(),createdBy:"ai",lastUpdated:new Date().toISOString(),relatedEntities:[],interactions:[{timestamp:new Date().toISOString(),type:"ai_analysis",content:`\u5F52\u6863\u4E3A${n.smallType}\uFF1A${n.context||"\u65E0"}`,sourceBlockId:this.activeBlockId||void 0}],metadata:r})}catch(r){console.error(`[AIAnalysisPanel] Failed to create entity ${n.name}:`,r)}}async handleEntityUpdate(t){let i=this.plugin.getEntityManager();if(i)for(let n of t)try{let r=i.getEntity(n.entityId);if(!r)continue;let s={lastUpdated:new Date().toISOString()},a=[...r.interactions??[]];for(let o of n.updates)if(o.field.startsWith("metadata.")){let l=o.field.replace("metadata.","");s.metadata={...r.metadata,[l]:o.value}}else o.field==="interactions"?(a.push({timestamp:new Date().toISOString(),type:"ai_analysis",content:o.value,sourceBlockId:this.activeBlockId||void 0}),s.interactions=a):o.field==="summary"&&(s.summary=o.value);await i.updateEntity(n.entityId,s)}catch(r){console.error(`[AIAnalysisPanel] Failed to update entity ${n.name}:`,r)}}async handleRelations(t){let i=this.plugin.getEntityManager();if(i)for(let n of t)try{let r=i.findEntity(n.from),s=i.findEntity(n.to);if(!r||!s)continue;let a=r.relatedEntities||[],o={entityId:s.id,relation:n.relation,context:n.context||`\u901A\u8FC7\u65E5\u8BB0\u5206\u6790\u5EFA\u7ACB\u5173\u7CFB\uFF1A${n.from}\u662F${n.to}\u7684${n.relation}`};a.some(c=>c.entityId===s.id&&c.relation===o.relation)||await i.updateEntity(r.id,{relatedEntities:[...a,o],lastUpdated:new Date().toISOString()})}catch(r){console.error("[AIAnalysisPanel] Failed to create relation:",r)}}async updateBlockCategory(t,i){try{let n=this.app.workspace.getLeavesOfType(D);if(n.length===0)return;let r=n[0].view;if(!r)return;let s=r.getBlockById(t);if(!s)return;(s.category==="\u5F85\u5206\u6790"||s.category!==i)&&(s.category=i,await r.saveBlockToFile(s),console.log(`[AIAnalysisPanel] Updated block ${t} category to ${i}`))}catch(n){console.error("[AIAnalysisPanel] Failed to update block category:",n)}}async showEntityConfirmationDialog(t){if(this.chatMessagesEl)for(let i of t){let n=this.chatMessagesEl.createEl("div",{cls:"lifewiki-entity-confirm"});n.createEl("div",{cls:"lifewiki-entity-confirm-title",text:`\u8BC6\u522B\u5230\u65B0${this.getEntityTypeLabel(i.inferredType)}: **${i.name}**`}),n.createEl("div",{cls:"lifewiki-entity-confirm-reason",text:i.reason||"\u4ECE\u65E5\u8BB0\u4E2D\u53D1\u73B0"});let r=n.createEl("div",{cls:"lifewiki-entity-confirm-buttons"}),s=r.createEl("button",{cls:"lifewiki-entity-confirm-btn archive",text:"\u5F52\u6863",attr:{type:"button"}}),a=r.createEl("button",{cls:"lifewiki-entity-confirm-btn skip",text:"\u8DF3\u8FC7",attr:{type:"button"}});s.addEventListener("click",async()=>{await this.archiveEntity(i),n.remove()}),a.addEventListener("click",()=>{n.remove()})}}getEntityTypeLabel(t){return{person:"\u4EBA\u8109",object:"\u5BA2\u4F53",theme:"\u4E3B\u9898"}[t]||"\u5B9E\u4F53"}maturityLabel(t){return t}async archiveEntity(t){let i=this.plugin.getEntityManager();if(!i)return;let r={person:"person",object:"object",theme:"theme"}[t.inferredType]||"person";try{await i.createEntity({type:r,title:t.name,titleRaw:t.name,aliases:[],tags:[],summary:t.reason||"\u4ECE\u65E5\u8BB0\u4E2D\u5F52\u6863",confidence:.8,verificationStatus:"verified",createdAt:new Date().toISOString(),createdBy:"ai",lastUpdated:new Date().toISOString(),relatedEntities:[],interactions:[{timestamp:new Date().toISOString(),type:"ai_analysis",content:`\u5F52\u6863\uFF1A${t.reason||"\u4ECE\u65E5\u8BB0\u4E2D\u53D1\u73B0"}`,sourceBlockId:this.activeBlockId||void 0}],metadata:{status:"active",source:"diary"}}),this.addChatMessage("assistant",`\u5DF2\u5F52\u6863 **${t.name}**`)}catch(s){console.error("[AIAnalysisPanel] Failed to archive entity:",s),this.addChatMessage("assistant","\u5F52\u6863\u5931\u8D25")}}async onClose(){}};var Lt=require("obsidian"),ee="tracemind-calendar",xe=class extends Lt.ItemView{plugin;currentMonth;onDateClickCallback=null;constructor(t,i){super(t),this.plugin=i,this.currentMonth={year:new Date().getFullYear(),month:new Date().getMonth()}}getViewType(){return ee}getDisplayText(){return"\u65E5\u5386"}getIcon(){return"calendar"}async onOpen(){this.renderCalendar()}async onClose(){}setOnDateClick(t){this.onDateClickCallback=t}async handleDateClick(t){this.onDateClickCallback&&this.onDateClickCallback(t)}getMonthDays(t,i){let n=new Date(t,i,1),r=new Date(t,i+1,0),s=[],a=n.getDay();for(let l=a-1;l>=0;l--){let c=new Date(t,i,-l);s.push(c)}for(let l=1;l<=r.getDate();l++)s.push(new Date(t,i,l));let o=42-s.length;for(let l=1;l<=o;l++)s.push(new Date(t,i+1,l));return s}isToday(t){let i=new Date;return t.getFullYear()===i.getFullYear()&&t.getMonth()===i.getMonth()&&t.getDate()===i.getDate()}isCurrentMonth(t){return t.getFullYear()===this.currentMonth.year&&t.getMonth()===this.currentMonth.month}getNextMonth(t,i){return i===11?{year:t+1,month:0}:{year:t,month:i+1}}getPrevMonth(t,i){return i===0?{year:t-1,month:11}:{year:t,month:i-1}}async diaryExistsForDate(t){try{let i=`Daily/${t}.md`,n=this.app.vault.getAbstractFileByPath(i);return n!=null}catch{return!1}}async goToNextMonth(){this.currentMonth=this.getNextMonth(this.currentMonth.year,this.currentMonth.month),this.renderCalendar()}async goToPrevMonth(){this.currentMonth=this.getPrevMonth(this.currentMonth.year,this.currentMonth.month),this.renderCalendar()}async goToToday(){let t=new Date;this.currentMonth={year:t.getFullYear(),month:t.getMonth()},this.renderCalendar()}async renderCalendar(){let t=this.containerEl;t.empty(),this.addStyles();let i=t.createEl("div",{cls:"tracemind-calendar"});this.renderHeader(i),this.renderWeekdayHeader(i),await this.renderDays(i)}renderHeader(t){let i=t.createEl("div",{cls:"lifewiki-calendar-header"});i.createEl("button",{cls:"lifewiki-calendar-nav-btn",text:"\u2039"}).addEventListener("click",()=>this.goToPrevMonth()),i.createEl("span",{cls:"lifewiki-calendar-title",text:this.getMonthTitle()}).addEventListener("click",()=>this.goToToday()),i.createEl("button",{cls:"lifewiki-calendar-nav-btn",text:"\u203A"}).addEventListener("click",()=>this.goToNextMonth())}renderWeekdayHeader(t){let i=["\u65E5","\u4E00","\u4E8C","\u4E09","\u56DB","\u4E94","\u516D"],n=t.createEl("div",{cls:"lifewiki-calendar-weekdays"});for(let r of i)n.createEl("div",{cls:"lifewiki-calendar-weekday",text:r})}async renderDays(t){let i=this.getMonthDays(this.currentMonth.year,this.currentMonth.month),n=t.createEl("div",{cls:"lifewiki-calendar-grid"});for(let r of i){let s=await this.createDayElement(r,n);n.appendChild(s)}}async createDayElement(t,i){let n=i.createEl("div",{cls:"lifewiki-calendar-day"}),r=this.isCurrentMonth(t),s=this.isToday(t);r||n.addClass("lifewiki-calendar-day-other-month"),s&&n.addClass("lifewiki-calendar-day-today");let a=this.formatDate(t);await this.diaryExistsForDate(a)&&n.addClass("lifewiki-calendar-day-has-diary");let l=n.createEl("span",{cls:"lifewiki-calendar-day-num",text:String(t.getDate())});return n.addEventListener("click",()=>{this.handleDateClick(t)}),n}getMonthTitle(){let t=["\u4E00\u6708","\u4E8C\u6708","\u4E09\u6708","\u56DB\u6708","\u4E94\u6708","\u516D\u6708","\u4E03\u6708","\u516B\u6708","\u4E5D\u6708","\u5341\u6708","\u5341\u4E00\u6708","\u5341\u4E8C\u6708"];return`${this.currentMonth.year}\u5E74 ${t[this.currentMonth.month]}`}formatDate(t){let i=t.getFullYear(),n=String(t.getMonth()+1).padStart(2,"0"),r=String(t.getDate()).padStart(2,"0");return`${i}-${n}-${r}`}addStyles(){let t="lifewiki-calendar-styles";if(document.getElementById(t))return;let i=document.createElement("style");i.id=t,i.textContent=`
		...
		`,document.head.appendChild(i)}};async function ve(e,t){if(!e.vault.getAbstractFileByPath(t))try{await e.vault.createFolder(t)}catch(n){if(!n.message?.includes("already exists"))throw n}}function Qt(e){return typeof e>"u"||e===null}function vn(e){return typeof e=="object"&&e!==null}function En(e){return Array.isArray(e)?e:Qt(e)?[]:[e]}function bn(e,t){var i,n,r,s;if(t)for(s=Object.keys(t),i=0,n=s.length;i<n;i+=1)r=s[i],e[r]=t[r];return e}function wn(e,t){var i="",n;for(n=0;n<t;n+=1)i+=e;return i}function kn(e){return e===0&&Number.NEGATIVE_INFINITY===1/e}var Cn=Qt,An=vn,Tn=En,Fn=wn,Sn=kn,Mn=bn,k={isNothing:Cn,isObject:An,toArray:Tn,repeat:Fn,isNegativeZero:Sn,extend:Mn};function Jt(e,t){var i="",n=e.reason||"(unknown reason)";return e.mark?(e.mark.name&&(i+='in "'+e.mark.name+'" '),i+="("+(e.mark.line+1)+":"+(e.mark.column+1)+")",!t&&e.mark.snippet&&(i+=`

`+e.mark.snippet),n+" "+i):n}function ie(e,t){Error.call(this),this.name="YAMLException",this.reason=e,this.mark=t,this.message=Jt(this,!1),Error.captureStackTrace?Error.captureStackTrace(this,this.constructor):this.stack=new Error().stack||""}ie.prototype=Object.create(Error.prototype);ie.prototype.constructor=ie;ie.prototype.toString=function(t){return this.name+": "+Jt(this,t)};var T=ie;function Ue(e,t,i,n,r){var s="",a="",o=Math.floor(r/2)-1;return n-t>o&&(s=" ... ",t=n-o+s.length),i-n>o&&(a=" ...",i=n+o-a.length),{str:s+e.slice(t,i).replace(/\t/g,"\u2192")+a,pos:n-t+s.length}}function ze(e,t){return k.repeat(" ",t-e.length)+e}function In(e,t){if(t=Object.create(t||null),!e.buffer)return null;t.maxLength||(t.maxLength=79),typeof t.indent!="number"&&(t.indent=1),typeof t.linesBefore!="number"&&(t.linesBefore=3),typeof t.linesAfter!="number"&&(t.linesAfter=2);for(var i=/\r?\n|\r|\0/g,n=[0],r=[],s,a=-1;s=i.exec(e.buffer);)r.push(s.index),n.push(s.index+s[0].length),e.position<=s.index&&a<0&&(a=n.length-2);a<0&&(a=n.length-1);var o="",l,c,u=Math.min(e.line+t.linesAfter,r.length).toString().length,d=t.maxLength-(t.indent+u+3);for(l=1;l<=t.linesBefore&&!(a-l<0);l++)c=Ue(e.buffer,n[a-l],r[a-l],e.position-(n[a]-n[a-l]),d),o=k.repeat(" ",t.indent)+ze((e.line-l+1).toString(),u)+" | "+c.str+`
`+o;for(c=Ue(e.buffer,n[a],r[a],e.position,d),o+=k.repeat(" ",t.indent)+ze((e.line+1).toString(),u)+" | "+c.str+`
`,o+=k.repeat("-",t.indent+u+3+c.pos)+`^
`,l=1;l<=t.linesAfter&&!(a+l>=r.length);l++)c=Ue(e.buffer,n[a+l],r[a+l],e.position-(n[a]-n[a+l]),d),o+=k.repeat(" ",t.indent)+ze((e.line+l+1).toString(),u)+" | "+c.str+`
`;return o.replace(/\n$/,"")}var Bn=In,Dn=["kind","multi","resolve","construct","instanceOf","predicate","represent","representName","defaultStyle","styleAliases"],Pn=["scalar","sequence","mapping"];function Ln(e){var t={};return e!==null&&Object.keys(e).forEach(function(i){e[i].forEach(function(n){t[String(n)]=i})}),t}function _n(e,t){if(t=t||{},Object.keys(t).forEach(function(i){if(Dn.indexOf(i)===-1)throw new T('Unknown option "'+i+'" is met in definition of "'+e+'" YAML type.')}),this.options=t,this.tag=e,this.kind=t.kind||null,this.resolve=t.resolve||function(){return!0},this.construct=t.construct||function(i){return i},this.instanceOf=t.instanceOf||null,this.predicate=t.predicate||null,this.represent=t.represent||null,this.representName=t.representName||null,this.defaultStyle=t.defaultStyle||null,this.multi=t.multi||!1,this.styleAliases=Ln(t.styleAliases||null),Pn.indexOf(this.kind)===-1)throw new T('Unknown kind "'+this.kind+'" is specified for "'+e+'" YAML type.')}var C=_n;function _t(e,t){var i=[];return e[t].forEach(function(n){var r=i.length;i.forEach(function(s,a){s.tag===n.tag&&s.kind===n.kind&&s.multi===n.multi&&(r=a)}),i[r]=n}),i}function Rn(){var e={scalar:{},sequence:{},mapping:{},fallback:{},multi:{scalar:[],sequence:[],mapping:[],fallback:[]}},t,i;function n(r){r.multi?(e.multi[r.kind].push(r),e.multi.fallback.push(r)):e[r.kind][r.tag]=e.fallback[r.tag]=r}for(t=0,i=arguments.length;t<i;t+=1)arguments[t].forEach(n);return e}function Ye(e){return this.extend(e)}Ye.prototype.extend=function(t){var i=[],n=[];if(t instanceof C)n.push(t);else if(Array.isArray(t))n=n.concat(t);else if(t&&(Array.isArray(t.implicit)||Array.isArray(t.explicit)))t.implicit&&(i=i.concat(t.implicit)),t.explicit&&(n=n.concat(t.explicit));else throw new T("Schema.extend argument should be a Type, [ Type ], or a schema definition ({ implicit: [...], explicit: [...] })");i.forEach(function(s){if(!(s instanceof C))throw new T("Specified list of YAML types (or a single Type object) contains a non-Type object.");if(s.loadKind&&s.loadKind!=="scalar")throw new T("There is a non-scalar type in the implicit list of a schema. Implicit resolving of such types is not supported.");if(s.multi)throw new T("There is a multi type in the implicit list of a schema. Multi tags can only be listed as explicit.")}),n.forEach(function(s){if(!(s instanceof C))throw new T("Specified list of YAML types (or a single Type object) contains a non-Type object.")});var r=Object.create(Ye.prototype);return r.implicit=(this.implicit||[]).concat(i),r.explicit=(this.explicit||[]).concat(n),r.compiledImplicit=_t(r,"implicit"),r.compiledExplicit=_t(r,"explicit"),r.compiledTypeMap=Rn(r.compiledImplicit,r.compiledExplicit),r};var Xt=Ye,Zt=new C("tag:yaml.org,2002:str",{kind:"scalar",construct:function(e){return e!==null?e:""}}),ei=new C("tag:yaml.org,2002:seq",{kind:"sequence",construct:function(e){return e!==null?e:[]}}),ti=new C("tag:yaml.org,2002:map",{kind:"mapping",construct:function(e){return e!==null?e:{}}}),ii=new Xt({explicit:[Zt,ei,ti]});function On(e){if(e===null)return!0;var t=e.length;return t===1&&e==="~"||t===4&&(e==="null"||e==="Null"||e==="NULL")}function Nn(){return null}function $n(e){return e===null}var ni=new C("tag:yaml.org,2002:null",{kind:"scalar",resolve:On,construct:Nn,predicate:$n,represent:{canonical:function(){return"~"},lowercase:function(){return"null"},uppercase:function(){return"NULL"},camelcase:function(){return"Null"},empty:function(){return""}},defaultStyle:"lowercase"});function jn(e){if(e===null)return!1;var t=e.length;return t===4&&(e==="true"||e==="True"||e==="TRUE")||t===5&&(e==="false"||e==="False"||e==="FALSE")}function Hn(e){return e==="true"||e==="True"||e==="TRUE"}function Un(e){return Object.prototype.toString.call(e)==="[object Boolean]"}var ri=new C("tag:yaml.org,2002:bool",{kind:"scalar",resolve:jn,construct:Hn,predicate:Un,represent:{lowercase:function(e){return e?"true":"false"},uppercase:function(e){return e?"TRUE":"FALSE"},camelcase:function(e){return e?"True":"False"}},defaultStyle:"lowercase"});function zn(e){return 48<=e&&e<=57||65<=e&&e<=70||97<=e&&e<=102}function Vn(e){return 48<=e&&e<=55}function Yn(e){return 48<=e&&e<=57}function Kn(e){if(e===null)return!1;var t=e.length,i=0,n=!1,r;if(!t)return!1;if(r=e[i],(r==="-"||r==="+")&&(r=e[++i]),r==="0"){if(i+1===t)return!0;if(r=e[++i],r==="b"){for(i++;i<t;i++)if(r=e[i],r!=="_"){if(r!=="0"&&r!=="1")return!1;n=!0}return n&&r!=="_"}if(r==="x"){for(i++;i<t;i++)if(r=e[i],r!=="_"){if(!zn(e.charCodeAt(i)))return!1;n=!0}return n&&r!=="_"}if(r==="o"){for(i++;i<t;i++)if(r=e[i],r!=="_"){if(!Vn(e.charCodeAt(i)))return!1;n=!0}return n&&r!=="_"}}if(r==="_")return!1;for(;i<t;i++)if(r=e[i],r!=="_"){if(!Yn(e.charCodeAt(i)))return!1;n=!0}return!(!n||r==="_")}function Wn(e){var t=e,i=1,n;if(t.indexOf("_")!==-1&&(t=t.replace(/_/g,"")),n=t[0],(n==="-"||n==="+")&&(n==="-"&&(i=-1),t=t.slice(1),n=t[0]),t==="0")return 0;if(n==="0"){if(t[1]==="b")return i*parseInt(t.slice(2),2);if(t[1]==="x")return i*parseInt(t.slice(2),16);if(t[1]==="o")return i*parseInt(t.slice(2),8)}return i*parseInt(t,10)}function Gn(e){return Object.prototype.toString.call(e)==="[object Number]"&&e%1===0&&!k.isNegativeZero(e)}var ai=new C("tag:yaml.org,2002:int",{kind:"scalar",resolve:Kn,construct:Wn,predicate:Gn,represent:{binary:function(e){return e>=0?"0b"+e.toString(2):"-0b"+e.toString(2).slice(1)},octal:function(e){return e>=0?"0o"+e.toString(8):"-0o"+e.toString(8).slice(1)},decimal:function(e){return e.toString(10)},hexadecimal:function(e){return e>=0?"0x"+e.toString(16).toUpperCase():"-0x"+e.toString(16).toUpperCase().slice(1)}},defaultStyle:"decimal",styleAliases:{binary:[2,"bin"],octal:[8,"oct"],decimal:[10,"dec"],hexadecimal:[16,"hex"]}}),qn=new RegExp("^(?:[-+]?(?:[0-9][0-9_]*)(?:\\.[0-9_]*)?(?:[eE][-+]?[0-9]+)?|\\.[0-9_]+(?:[eE][-+]?[0-9]+)?|[-+]?\\.(?:inf|Inf|INF)|\\.(?:nan|NaN|NAN))$");function Qn(e){return!(e===null||!qn.test(e)||e[e.length-1]==="_")}function Jn(e){var t,i;return t=e.replace(/_/g,"").toLowerCase(),i=t[0]==="-"?-1:1,"+-".indexOf(t[0])>=0&&(t=t.slice(1)),t===".inf"?i===1?Number.POSITIVE_INFINITY:Number.NEGATIVE_INFINITY:t===".nan"?NaN:i*parseFloat(t,10)}var Xn=/^[-+]?[0-9]+e/;function Zn(e,t){var i;if(isNaN(e))switch(t){case"lowercase":return".nan";case"uppercase":return".NAN";case"camelcase":return".NaN"}else if(Number.POSITIVE_INFINITY===e)switch(t){case"lowercase":return".inf";case"uppercase":return".INF";case"camelcase":return".Inf"}else if(Number.NEGATIVE_INFINITY===e)switch(t){case"lowercase":return"-.inf";case"uppercase":return"-.INF";case"camelcase":return"-.Inf"}else if(k.isNegativeZero(e))return"-0.0";return i=e.toString(10),Xn.test(i)?i.replace("e",".e"):i}function er(e){return Object.prototype.toString.call(e)==="[object Number]"&&(e%1!==0||k.isNegativeZero(e))}var si=new C("tag:yaml.org,2002:float",{kind:"scalar",resolve:Qn,construct:Jn,predicate:er,represent:Zn,defaultStyle:"lowercase"}),oi=ii.extend({implicit:[ni,ri,ai,si]}),li=oi,ci=new RegExp("^([0-9][0-9][0-9][0-9])-([0-9][0-9])-([0-9][0-9])$"),ui=new RegExp("^([0-9][0-9][0-9][0-9])-([0-9][0-9]?)-([0-9][0-9]?)(?:[Tt]|[ \\t]+)([0-9][0-9]?):([0-9][0-9]):([0-9][0-9])(?:\\.([0-9]*))?(?:[ \\t]*(Z|([-+])([0-9][0-9]?)(?::([0-9][0-9]))?))?$");function tr(e){return e===null?!1:ci.exec(e)!==null||ui.exec(e)!==null}function ir(e){var t,i,n,r,s,a,o,l=0,c=null,u,d,p;if(t=ci.exec(e),t===null&&(t=ui.exec(e)),t===null)throw new Error("Date resolve error");if(i=+t[1],n=+t[2]-1,r=+t[3],!t[4])return new Date(Date.UTC(i,n,r));if(s=+t[4],a=+t[5],o=+t[6],t[7]){for(l=t[7].slice(0,3);l.length<3;)l+="0";l=+l}return t[9]&&(u=+t[10],d=+(t[11]||0),c=(u*60+d)*6e4,t[9]==="-"&&(c=-c)),p=new Date(Date.UTC(i,n,r,s,a,o,l)),c&&p.setTime(p.getTime()-c),p}function nr(e){return e.toISOString()}var di=new C("tag:yaml.org,2002:timestamp",{kind:"scalar",resolve:tr,construct:ir,instanceOf:Date,represent:nr});function rr(e){return e==="<<"||e===null}var pi=new C("tag:yaml.org,2002:merge",{kind:"scalar",resolve:rr}),Qe=`ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=
\r`;function ar(e){if(e===null)return!1;var t,i,n=0,r=e.length,s=Qe;for(i=0;i<r;i++)if(t=s.indexOf(e.charAt(i)),!(t>64)){if(t<0)return!1;n+=6}return n%8===0}function sr(e){var t,i,n=e.replace(/[\r\n=]/g,""),r=n.length,s=Qe,a=0,o=[];for(t=0;t<r;t++)t%4===0&&t&&(o.push(a>>16&255),o.push(a>>8&255),o.push(a&255)),a=a<<6|s.indexOf(n.charAt(t));return i=r%4*6,i===0?(o.push(a>>16&255),o.push(a>>8&255),o.push(a&255)):i===18?(o.push(a>>10&255),o.push(a>>2&255)):i===12&&o.push(a>>4&255),new Uint8Array(o)}function or(e){var t="",i=0,n,r,s=e.length,a=Qe;for(n=0;n<s;n++)n%3===0&&n&&(t+=a[i>>18&63],t+=a[i>>12&63],t+=a[i>>6&63],t+=a[i&63]),i=(i<<8)+e[n];return r=s%3,r===0?(t+=a[i>>18&63],t+=a[i>>12&63],t+=a[i>>6&63],t+=a[i&63]):r===2?(t+=a[i>>10&63],t+=a[i>>4&63],t+=a[i<<2&63],t+=a[64]):r===1&&(t+=a[i>>2&63],t+=a[i<<4&63],t+=a[64],t+=a[64]),t}function lr(e){return Object.prototype.toString.call(e)==="[object Uint8Array]"}var hi=new C("tag:yaml.org,2002:binary",{kind:"scalar",resolve:ar,construct:sr,predicate:lr,represent:or}),cr=Object.prototype.hasOwnProperty,ur=Object.prototype.toString;function dr(e){if(e===null)return!0;var t=[],i,n,r,s,a,o=e;for(i=0,n=o.length;i<n;i+=1){if(r=o[i],a=!1,ur.call(r)!=="[object Object]")return!1;for(s in r)if(cr.call(r,s))if(!a)a=!0;else return!1;if(!a)return!1;if(t.indexOf(s)===-1)t.push(s);else return!1}return!0}function pr(e){return e!==null?e:[]}var fi=new C("tag:yaml.org,2002:omap",{kind:"sequence",resolve:dr,construct:pr}),hr=Object.prototype.toString;function fr(e){if(e===null)return!0;var t,i,n,r,s,a=e;for(s=new Array(a.length),t=0,i=a.length;t<i;t+=1){if(n=a[t],hr.call(n)!=="[object Object]"||(r=Object.keys(n),r.length!==1))return!1;s[t]=[r[0],n[r[0]]]}return!0}function gr(e){if(e===null)return[];var t,i,n,r,s,a=e;for(s=new Array(a.length),t=0,i=a.length;t<i;t+=1)n=a[t],r=Object.keys(n),s[t]=[r[0],n[r[0]]];return s}var gi=new C("tag:yaml.org,2002:pairs",{kind:"sequence",resolve:fr,construct:gr}),mr=Object.prototype.hasOwnProperty;function yr(e){if(e===null)return!0;var t,i=e;for(t in i)if(mr.call(i,t)&&i[t]!==null)return!1;return!0}function xr(e){return e!==null?e:{}}var mi=new C("tag:yaml.org,2002:set",{kind:"mapping",resolve:yr,construct:xr}),Je=li.extend({implicit:[di,pi],explicit:[hi,fi,gi,mi]}),O=Object.prototype.hasOwnProperty,Ee=1,yi=2,xi=3,be=4,Ve=1,vr=2,Rt=3,Er=/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x84\x86-\x9F\uFFFE\uFFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]/,br=/[\x85\u2028\u2029]/,wr=/[,\[\]\{\}]/,vi=/^(?:!|!!|![a-z\-]+!)$/i,Ei=/^(?:!|[^,\[\]\{\}])(?:%[0-9a-f]{2}|[0-9a-z\-#;\/\?:@&=\+\$,_\.!~\*'\(\)\[\]])*$/i;function Ot(e){return Object.prototype.toString.call(e)}function I(e){return e===10||e===13}function $(e){return e===9||e===32}function F(e){return e===9||e===32||e===10||e===13}function Y(e){return e===44||e===91||e===93||e===123||e===125}function kr(e){var t;return 48<=e&&e<=57?e-48:(t=e|32,97<=t&&t<=102?t-97+10:-1)}function Cr(e){return e===120?2:e===117?4:e===85?8:0}function Ar(e){return 48<=e&&e<=57?e-48:-1}function Nt(e){return e===48?"\0":e===97?"\x07":e===98?"\b":e===116||e===9?"	":e===110?`
`:e===118?"\v":e===102?"\f":e===114?"\r":e===101?"\x1B":e===32?" ":e===34?'"':e===47?"/":e===92?"\\":e===78?"\x85":e===95?"\xA0":e===76?"\u2028":e===80?"\u2029":""}function Tr(e){return e<=65535?String.fromCharCode(e):String.fromCharCode((e-65536>>10)+55296,(e-65536&1023)+56320)}function bi(e,t,i){t==="__proto__"?Object.defineProperty(e,t,{configurable:!0,enumerable:!0,writable:!0,value:i}):e[t]=i}var wi=new Array(256),ki=new Array(256);for(N=0;N<256;N++)wi[N]=Nt(N)?1:0,ki[N]=Nt(N);var N;function Fr(e,t){this.input=e,this.filename=t.filename||null,this.schema=t.schema||Je,this.onWarning=t.onWarning||null,this.legacy=t.legacy||!1,this.json=t.json||!1,this.listener=t.listener||null,this.implicitTypes=this.schema.compiledImplicit,this.typeMap=this.schema.compiledTypeMap,this.length=e.length,this.position=0,this.line=0,this.lineStart=0,this.lineIndent=0,this.firstTabInLine=-1,this.documents=[]}function Ci(e,t){var i={name:e.filename,buffer:e.input.slice(0,-1),position:e.position,line:e.line,column:e.position-e.lineStart};return i.snippet=Bn(i),new T(t,i)}function y(e,t){throw Ci(e,t)}function we(e,t){e.onWarning&&e.onWarning.call(null,Ci(e,t))}var $t={YAML:function(t,i,n){var r,s,a;t.version!==null&&y(t,"duplication of %YAML directive"),n.length!==1&&y(t,"YAML directive accepts exactly one argument"),r=/^([0-9]+)\.([0-9]+)$/.exec(n[0]),r===null&&y(t,"ill-formed argument of the YAML directive"),s=parseInt(r[1],10),a=parseInt(r[2],10),s!==1&&y(t,"unacceptable YAML version of the document"),t.version=n[0],t.checkLineBreaks=a<2,a!==1&&a!==2&&we(t,"unsupported YAML version of the document")},TAG:function(t,i,n){var r,s;n.length!==2&&y(t,"TAG directive accepts exactly two arguments"),r=n[0],s=n[1],vi.test(r)||y(t,"ill-formed tag handle (first argument) of the TAG directive"),O.call(t.tagMap,r)&&y(t,'there is a previously declared suffix for "'+r+'" tag handle'),Ei.test(s)||y(t,"ill-formed tag prefix (second argument) of the TAG directive");try{s=decodeURIComponent(s)}catch{y(t,"tag prefix is malformed: "+s)}t.tagMap[r]=s}};function R(e,t,i,n){var r,s,a,o;if(t<i){if(o=e.input.slice(t,i),n)for(r=0,s=o.length;r<s;r+=1)a=o.charCodeAt(r),a===9||32<=a&&a<=1114111||y(e,"expected valid JSON character");else Er.test(o)&&y(e,"the stream contains non-printable characters");e.result+=o}}function jt(e,t,i,n){var r,s,a,o;for(k.isObject(i)||y(e,"cannot merge mappings; the provided source object is unacceptable"),r=Object.keys(i),a=0,o=r.length;a<o;a+=1)s=r[a],O.call(t,s)||(bi(t,s,i[s]),n[s]=!0)}function K(e,t,i,n,r,s,a,o,l){var c,u;if(Array.isArray(r))for(r=Array.prototype.slice.call(r),c=0,u=r.length;c<u;c+=1)Array.isArray(r[c])&&y(e,"nested arrays are not supported inside keys"),typeof r=="object"&&Ot(r[c])==="[object Object]"&&(r[c]="[object Object]");if(typeof r=="object"&&Ot(r)==="[object Object]"&&(r="[object Object]"),r=String(r),t===null&&(t={}),n==="tag:yaml.org,2002:merge")if(Array.isArray(s))for(c=0,u=s.length;c<u;c+=1)jt(e,t,s[c],i);else jt(e,t,s,i);else!e.json&&!O.call(i,r)&&O.call(t,r)&&(e.line=a||e.line,e.lineStart=o||e.lineStart,e.position=l||e.position,y(e,"duplicated mapping key")),bi(t,r,s),delete i[r];return t}function Xe(e){var t;t=e.input.charCodeAt(e.position),t===10?e.position++:t===13?(e.position++,e.input.charCodeAt(e.position)===10&&e.position++):y(e,"a line break is expected"),e.line+=1,e.lineStart=e.position,e.firstTabInLine=-1}function w(e,t,i){for(var n=0,r=e.input.charCodeAt(e.position);r!==0;){for(;$(r);)r===9&&e.firstTabInLine===-1&&(e.firstTabInLine=e.position),r=e.input.charCodeAt(++e.position);if(t&&r===35)do r=e.input.charCodeAt(++e.position);while(r!==10&&r!==13&&r!==0);if(I(r))for(Xe(e),r=e.input.charCodeAt(e.position),n++,e.lineIndent=0;r===32;)e.lineIndent++,r=e.input.charCodeAt(++e.position);else break}return i!==-1&&n!==0&&e.lineIndent<i&&we(e,"deficient indentation"),n}function Ae(e){var t=e.position,i;return i=e.input.charCodeAt(t),!!((i===45||i===46)&&i===e.input.charCodeAt(t+1)&&i===e.input.charCodeAt(t+2)&&(t+=3,i=e.input.charCodeAt(t),i===0||F(i)))}function Ze(e,t){t===1?e.result+=" ":t>1&&(e.result+=k.repeat(`
`,t-1))}function Sr(e,t,i){var n,r,s,a,o,l,c,u,d=e.kind,p=e.result,f;if(f=e.input.charCodeAt(e.position),F(f)||Y(f)||f===35||f===38||f===42||f===33||f===124||f===62||f===39||f===34||f===37||f===64||f===96||(f===63||f===45)&&(r=e.input.charCodeAt(e.position+1),F(r)||i&&Y(r)))return!1;for(e.kind="scalar",e.result="",s=a=e.position,o=!1;f!==0;){if(f===58){if(r=e.input.charCodeAt(e.position+1),F(r)||i&&Y(r))break}else if(f===35){if(n=e.input.charCodeAt(e.position-1),F(n))break}else{if(e.position===e.lineStart&&Ae(e)||i&&Y(f))break;if(I(f))if(l=e.line,c=e.lineStart,u=e.lineIndent,w(e,!1,-1),e.lineIndent>=t){o=!0,f=e.input.charCodeAt(e.position);continue}else{e.position=a,e.line=l,e.lineStart=c,e.lineIndent=u;break}}o&&(R(e,s,a,!1),Ze(e,e.line-l),s=a=e.position,o=!1),$(f)||(a=e.position+1),f=e.input.charCodeAt(++e.position)}return R(e,s,a,!1),e.result?!0:(e.kind=d,e.result=p,!1)}function Mr(e,t){var i,n,r;if(i=e.input.charCodeAt(e.position),i!==39)return!1;for(e.kind="scalar",e.result="",e.position++,n=r=e.position;(i=e.input.charCodeAt(e.position))!==0;)if(i===39)if(R(e,n,e.position,!0),i=e.input.charCodeAt(++e.position),i===39)n=e.position,e.position++,r=e.position;else return!0;else I(i)?(R(e,n,r,!0),Ze(e,w(e,!1,t)),n=r=e.position):e.position===e.lineStart&&Ae(e)?y(e,"unexpected end of the document within a single quoted scalar"):(e.position++,r=e.position);y(e,"unexpected end of the stream within a single quoted scalar")}function Ir(e,t){var i,n,r,s,a,o;if(o=e.input.charCodeAt(e.position),o!==34)return!1;for(e.kind="scalar",e.result="",e.position++,i=n=e.position;(o=e.input.charCodeAt(e.position))!==0;){if(o===34)return R(e,i,e.position,!0),e.position++,!0;if(o===92){if(R(e,i,e.position,!0),o=e.input.charCodeAt(++e.position),I(o))w(e,!1,t);else if(o<256&&wi[o])e.result+=ki[o],e.position++;else if((a=Cr(o))>0){for(r=a,s=0;r>0;r--)o=e.input.charCodeAt(++e.position),(a=kr(o))>=0?s=(s<<4)+a:y(e,"expected hexadecimal character");e.result+=Tr(s),e.position++}else y(e,"unknown escape sequence");i=n=e.position}else I(o)?(R(e,i,n,!0),Ze(e,w(e,!1,t)),i=n=e.position):e.position===e.lineStart&&Ae(e)?y(e,"unexpected end of the document within a double quoted scalar"):(e.position++,n=e.position)}y(e,"unexpected end of the stream within a double quoted scalar")}function Br(e,t){var i=!0,n,r,s,a=e.tag,o,l=e.anchor,c,u,d,p,f,g=Object.create(null),h,m,v,x;if(x=e.input.charCodeAt(e.position),x===91)u=93,f=!1,o=[];else if(x===123)u=125,f=!0,o={};else return!1;for(e.anchor!==null&&(e.anchorMap[e.anchor]=o),x=e.input.charCodeAt(++e.position);x!==0;){if(w(e,!0,t),x=e.input.charCodeAt(e.position),x===u)return e.position++,e.tag=a,e.anchor=l,e.kind=f?"mapping":"sequence",e.result=o,!0;i?x===44&&y(e,"expected the node content, but found ','"):y(e,"missed comma between flow collection entries"),m=h=v=null,d=p=!1,x===63&&(c=e.input.charCodeAt(e.position+1),F(c)&&(d=p=!0,e.position++,w(e,!0,t))),n=e.line,r=e.lineStart,s=e.position,W(e,t,Ee,!1,!0),m=e.tag,h=e.result,w(e,!0,t),x=e.input.charCodeAt(e.position),(p||e.line===n)&&x===58&&(d=!0,x=e.input.charCodeAt(++e.position),w(e,!0,t),W(e,t,Ee,!1,!0),v=e.result),f?K(e,o,g,m,h,v,n,r,s):d?o.push(K(e,null,g,m,h,v,n,r,s)):o.push(h),w(e,!0,t),x=e.input.charCodeAt(e.position),x===44?(i=!0,x=e.input.charCodeAt(++e.position)):i=!1}y(e,"unexpected end of the stream within a flow collection")}function Dr(e,t){var i,n,r=Ve,s=!1,a=!1,o=t,l=0,c=!1,u,d;if(d=e.input.charCodeAt(e.position),d===124)n=!1;else if(d===62)n=!0;else return!1;for(e.kind="scalar",e.result="";d!==0;)if(d=e.input.charCodeAt(++e.position),d===43||d===45)Ve===r?r=d===43?Rt:vr:y(e,"repeat of a chomping mode identifier");else if((u=Ar(d))>=0)u===0?y(e,"bad explicit indentation width of a block scalar; it cannot be less than one"):a?y(e,"repeat of an indentation width identifier"):(o=t+u-1,a=!0);else break;if($(d)){do d=e.input.charCodeAt(++e.position);while($(d));if(d===35)do d=e.input.charCodeAt(++e.position);while(!I(d)&&d!==0)}for(;d!==0;){for(Xe(e),e.lineIndent=0,d=e.input.charCodeAt(e.position);(!a||e.lineIndent<o)&&d===32;)e.lineIndent++,d=e.input.charCodeAt(++e.position);if(!a&&e.lineIndent>o&&(o=e.lineIndent),I(d)){l++;continue}if(e.lineIndent<o){r===Rt?e.result+=k.repeat(`
`,s?1+l:l):r===Ve&&s&&(e.result+=`
`);break}for(n?$(d)?(c=!0,e.result+=k.repeat(`
`,s?1+l:l)):c?(c=!1,e.result+=k.repeat(`
`,l+1)):l===0?s&&(e.result+=" "):e.result+=k.repeat(`
`,l):e.result+=k.repeat(`
`,s?1+l:l),s=!0,a=!0,l=0,i=e.position;!I(d)&&d!==0;)d=e.input.charCodeAt(++e.position);R(e,i,e.position,!1)}return!0}function Ht(e,t){var i,n=e.tag,r=e.anchor,s=[],a,o=!1,l;if(e.firstTabInLine!==-1)return!1;for(e.anchor!==null&&(e.anchorMap[e.anchor]=s),l=e.input.charCodeAt(e.position);l!==0&&(e.firstTabInLine!==-1&&(e.position=e.firstTabInLine,y(e,"tab characters must not be used in indentation")),!(l!==45||(a=e.input.charCodeAt(e.position+1),!F(a))));){if(o=!0,e.position++,w(e,!0,-1)&&e.lineIndent<=t){s.push(null),l=e.input.charCodeAt(e.position);continue}if(i=e.line,W(e,t,xi,!1,!0),s.push(e.result),w(e,!0,-1),l=e.input.charCodeAt(e.position),(e.line===i||e.lineIndent>t)&&l!==0)y(e,"bad indentation of a sequence entry");else if(e.lineIndent<t)break}return o?(e.tag=n,e.anchor=r,e.kind="sequence",e.result=s,!0):!1}function Pr(e,t,i){var n,r,s,a,o,l,c=e.tag,u=e.anchor,d={},p=Object.create(null),f=null,g=null,h=null,m=!1,v=!1,x;if(e.firstTabInLine!==-1)return!1;for(e.anchor!==null&&(e.anchorMap[e.anchor]=d),x=e.input.charCodeAt(e.position);x!==0;){if(!m&&e.firstTabInLine!==-1&&(e.position=e.firstTabInLine,y(e,"tab characters must not be used in indentation")),n=e.input.charCodeAt(e.position+1),s=e.line,(x===63||x===58)&&F(n))x===63?(m&&(K(e,d,p,f,g,null,a,o,l),f=g=h=null),v=!0,m=!0,r=!0):m?(m=!1,r=!0):y(e,"incomplete explicit mapping pair; a key node is missed; or followed by a non-tabulated empty line"),e.position+=1,x=n;else{if(a=e.line,o=e.lineStart,l=e.position,!W(e,i,yi,!1,!0))break;if(e.line===s){for(x=e.input.charCodeAt(e.position);$(x);)x=e.input.charCodeAt(++e.position);if(x===58)x=e.input.charCodeAt(++e.position),F(x)||y(e,"a whitespace character is expected after the key-value separator within a block mapping"),m&&(K(e,d,p,f,g,null,a,o,l),f=g=h=null),v=!0,m=!1,r=!1,f=e.tag,g=e.result;else if(v)y(e,"can not read an implicit mapping pair; a colon is missed");else return e.tag=c,e.anchor=u,!0}else if(v)y(e,"can not read a block mapping entry; a multiline key may not be an implicit key");else return e.tag=c,e.anchor=u,!0}if((e.line===s||e.lineIndent>t)&&(m&&(a=e.line,o=e.lineStart,l=e.position),W(e,t,be,!0,r)&&(m?g=e.result:h=e.result),m||(K(e,d,p,f,g,h,a,o,l),f=g=h=null),w(e,!0,-1),x=e.input.charCodeAt(e.position)),(e.line===s||e.lineIndent>t)&&x!==0)y(e,"bad indentation of a mapping entry");else if(e.lineIndent<t)break}return m&&K(e,d,p,f,g,null,a,o,l),v&&(e.tag=c,e.anchor=u,e.kind="mapping",e.result=d),v}function Lr(e){var t,i=!1,n=!1,r,s,a;if(a=e.input.charCodeAt(e.position),a!==33)return!1;if(e.tag!==null&&y(e,"duplication of a tag property"),a=e.input.charCodeAt(++e.position),a===60?(i=!0,a=e.input.charCodeAt(++e.position)):a===33?(n=!0,r="!!",a=e.input.charCodeAt(++e.position)):r="!",t=e.position,i){do a=e.input.charCodeAt(++e.position);while(a!==0&&a!==62);e.position<e.length?(s=e.input.slice(t,e.position),a=e.input.charCodeAt(++e.position)):y(e,"unexpected end of the stream within a verbatim tag")}else{for(;a!==0&&!F(a);)a===33&&(n?y(e,"tag suffix cannot contain exclamation marks"):(r=e.input.slice(t-1,e.position+1),vi.test(r)||y(e,"named tag handle cannot contain such characters"),n=!0,t=e.position+1)),a=e.input.charCodeAt(++e.position);s=e.input.slice(t,e.position),wr.test(s)&&y(e,"tag suffix cannot contain flow indicator characters")}s&&!Ei.test(s)&&y(e,"tag name cannot contain such characters: "+s);try{s=decodeURIComponent(s)}catch{y(e,"tag name is malformed: "+s)}return i?e.tag=s:O.call(e.tagMap,r)?e.tag=e.tagMap[r]+s:r==="!"?e.tag="!"+s:r==="!!"?e.tag="tag:yaml.org,2002:"+s:y(e,'undeclared tag handle "'+r+'"'),!0}function _r(e){var t,i;if(i=e.input.charCodeAt(e.position),i!==38)return!1;for(e.anchor!==null&&y(e,"duplication of an anchor property"),i=e.input.charCodeAt(++e.position),t=e.position;i!==0&&!F(i)&&!Y(i);)i=e.input.charCodeAt(++e.position);return e.position===t&&y(e,"name of an anchor node must contain at least one character"),e.anchor=e.input.slice(t,e.position),!0}function Rr(e){var t,i,n;if(n=e.input.charCodeAt(e.position),n!==42)return!1;for(n=e.input.charCodeAt(++e.position),t=e.position;n!==0&&!F(n)&&!Y(n);)n=e.input.charCodeAt(++e.position);return e.position===t&&y(e,"name of an alias node must contain at least one character"),i=e.input.slice(t,e.position),O.call(e.anchorMap,i)||y(e,'unidentified alias "'+i+'"'),e.result=e.anchorMap[i],w(e,!0,-1),!0}function W(e,t,i,n,r){var s,a,o,l=1,c=!1,u=!1,d,p,f,g,h,m;if(e.listener!==null&&e.listener("open",e),e.tag=null,e.anchor=null,e.kind=null,e.result=null,s=a=o=be===i||xi===i,n&&w(e,!0,-1)&&(c=!0,e.lineIndent>t?l=1:e.lineIndent===t?l=0:e.lineIndent<t&&(l=-1)),l===1)for(;Lr(e)||_r(e);)w(e,!0,-1)?(c=!0,o=s,e.lineIndent>t?l=1:e.lineIndent===t?l=0:e.lineIndent<t&&(l=-1)):o=!1;if(o&&(o=c||r),(l===1||be===i)&&(Ee===i||yi===i?h=t:h=t+1,m=e.position-e.lineStart,l===1?o&&(Ht(e,m)||Pr(e,m,h))||Br(e,h)?u=!0:(a&&Dr(e,h)||Mr(e,h)||Ir(e,h)?u=!0:Rr(e)?(u=!0,(e.tag!==null||e.anchor!==null)&&y(e,"alias node should not have any properties")):Sr(e,h,Ee===i)&&(u=!0,e.tag===null&&(e.tag="?")),e.anchor!==null&&(e.anchorMap[e.anchor]=e.result)):l===0&&(u=o&&Ht(e,m))),e.tag===null)e.anchor!==null&&(e.anchorMap[e.anchor]=e.result);else if(e.tag==="?"){for(e.result!==null&&e.kind!=="scalar"&&y(e,'unacceptable node kind for !<?> tag; it should be "scalar", not "'+e.kind+'"'),d=0,p=e.implicitTypes.length;d<p;d+=1)if(g=e.implicitTypes[d],g.resolve(e.result)){e.result=g.construct(e.result),e.tag=g.tag,e.anchor!==null&&(e.anchorMap[e.anchor]=e.result);break}}else if(e.tag!=="!"){if(O.call(e.typeMap[e.kind||"fallback"],e.tag))g=e.typeMap[e.kind||"fallback"][e.tag];else for(g=null,f=e.typeMap.multi[e.kind||"fallback"],d=0,p=f.length;d<p;d+=1)if(e.tag.slice(0,f[d].tag.length)===f[d].tag){g=f[d];break}g||y(e,"unknown tag !<"+e.tag+">"),e.result!==null&&g.kind!==e.kind&&y(e,"unacceptable node kind for !<"+e.tag+'> tag; it should be "'+g.kind+'", not "'+e.kind+'"'),g.resolve(e.result,e.tag)?(e.result=g.construct(e.result,e.tag),e.anchor!==null&&(e.anchorMap[e.anchor]=e.result)):y(e,"cannot resolve a node with !<"+e.tag+"> explicit tag")}return e.listener!==null&&e.listener("close",e),e.tag!==null||e.anchor!==null||u}function Or(e){var t=e.position,i,n,r,s=!1,a;for(e.version=null,e.checkLineBreaks=e.legacy,e.tagMap=Object.create(null),e.anchorMap=Object.create(null);(a=e.input.charCodeAt(e.position))!==0&&(w(e,!0,-1),a=e.input.charCodeAt(e.position),!(e.lineIndent>0||a!==37));){for(s=!0,a=e.input.charCodeAt(++e.position),i=e.position;a!==0&&!F(a);)a=e.input.charCodeAt(++e.position);for(n=e.input.slice(i,e.position),r=[],n.length<1&&y(e,"directive name must not be less than one character in length");a!==0;){for(;$(a);)a=e.input.charCodeAt(++e.position);if(a===35){do a=e.input.charCodeAt(++e.position);while(a!==0&&!I(a));break}if(I(a))break;for(i=e.position;a!==0&&!F(a);)a=e.input.charCodeAt(++e.position);r.push(e.input.slice(i,e.position))}a!==0&&Xe(e),O.call($t,n)?$t[n](e,n,r):we(e,'unknown document directive "'+n+'"')}if(w(e,!0,-1),e.lineIndent===0&&e.input.charCodeAt(e.position)===45&&e.input.charCodeAt(e.position+1)===45&&e.input.charCodeAt(e.position+2)===45?(e.position+=3,w(e,!0,-1)):s&&y(e,"directives end mark is expected"),W(e,e.lineIndent-1,be,!1,!0),w(e,!0,-1),e.checkLineBreaks&&br.test(e.input.slice(t,e.position))&&we(e,"non-ASCII line breaks are interpreted as content"),e.documents.push(e.result),e.position===e.lineStart&&Ae(e)){e.input.charCodeAt(e.position)===46&&(e.position+=3,w(e,!0,-1));return}if(e.position<e.length-1)y(e,"end of the stream or a document separator is expected");else return}function Ai(e,t){e=String(e),t=t||{},e.length!==0&&(e.charCodeAt(e.length-1)!==10&&e.charCodeAt(e.length-1)!==13&&(e+=`
`),e.charCodeAt(0)===65279&&(e=e.slice(1)));var i=new Fr(e,t),n=e.indexOf("\0");for(n!==-1&&(i.position=n,y(i,"null byte is not allowed in input")),i.input+="\0";i.input.charCodeAt(i.position)===32;)i.lineIndent+=1,i.position+=1;for(;i.position<i.length-1;)Or(i);return i.documents}function Nr(e,t,i){t!==null&&typeof t=="object"&&typeof i>"u"&&(i=t,t=null);var n=Ai(e,i);if(typeof t!="function")return n;for(var r=0,s=n.length;r<s;r+=1)t(n[r])}function $r(e,t){var i=Ai(e,t);if(i.length!==0){if(i.length===1)return i[0];throw new T("expected a single document in the stream, but found more")}}var jr=Nr,Hr=$r,Ti={loadAll:jr,load:Hr},Fi=Object.prototype.toString,Si=Object.prototype.hasOwnProperty,et=65279,Ur=9,ne=10,zr=13,Vr=32,Yr=33,Kr=34,Ke=35,Wr=37,Gr=38,qr=39,Qr=42,Mi=44,Jr=45,ke=58,Xr=61,Zr=62,ea=63,ta=64,Ii=91,Bi=93,ia=96,Di=123,na=124,Pi=125,A={};A[0]="\\0";A[7]="\\a";A[8]="\\b";A[9]="\\t";A[10]="\\n";A[11]="\\v";A[12]="\\f";A[13]="\\r";A[27]="\\e";A[34]='\\"';A[92]="\\\\";A[133]="\\N";A[160]="\\_";A[8232]="\\L";A[8233]="\\P";var ra=["y","Y","yes","Yes","YES","on","On","ON","n","N","no","No","NO","off","Off","OFF"],aa=/^[-+]?[0-9_]+(?::[0-9_]+)+(?:\.[0-9_]*)?$/;function sa(e,t){var i,n,r,s,a,o,l;if(t===null)return{};for(i={},n=Object.keys(t),r=0,s=n.length;r<s;r+=1)a=n[r],o=String(t[a]),a.slice(0,2)==="!!"&&(a="tag:yaml.org,2002:"+a.slice(2)),l=e.compiledTypeMap.fallback[a],l&&Si.call(l.styleAliases,o)&&(o=l.styleAliases[o]),i[a]=o;return i}function oa(e){var t,i,n;if(t=e.toString(16).toUpperCase(),e<=255)i="x",n=2;else if(e<=65535)i="u",n=4;else if(e<=4294967295)i="U",n=8;else throw new T("code point within a string may not be greater than 0xFFFFFFFF");return"\\"+i+k.repeat("0",n-t.length)+t}var la=1,re=2;function ca(e){this.schema=e.schema||Je,this.indent=Math.max(1,e.indent||2),this.noArrayIndent=e.noArrayIndent||!1,this.skipInvalid=e.skipInvalid||!1,this.flowLevel=k.isNothing(e.flowLevel)?-1:e.flowLevel,this.styleMap=sa(this.schema,e.styles||null),this.sortKeys=e.sortKeys||!1,this.lineWidth=e.lineWidth||80,this.noRefs=e.noRefs||!1,this.noCompatMode=e.noCompatMode||!1,this.condenseFlow=e.condenseFlow||!1,this.quotingType=e.quotingType==='"'?re:la,this.forceQuotes=e.forceQuotes||!1,this.replacer=typeof e.replacer=="function"?e.replacer:null,this.implicitTypes=this.schema.compiledImplicit,this.explicitTypes=this.schema.compiledExplicit,this.tag=null,this.result="",this.duplicates=[],this.usedDuplicates=null}function Ut(e,t){for(var i=k.repeat(" ",t),n=0,r=-1,s="",a,o=e.length;n<o;)r=e.indexOf(`
`,n),r===-1?(a=e.slice(n),n=o):(a=e.slice(n,r+1),n=r+1),a.length&&a!==`
`&&(s+=i),s+=a;return s}function We(e,t){return`
`+k.repeat(" ",e.indent*t)}function ua(e,t){var i,n,r;for(i=0,n=e.implicitTypes.length;i<n;i+=1)if(r=e.implicitTypes[i],r.resolve(t))return!0;return!1}function Ce(e){return e===Vr||e===Ur}function ae(e){return 32<=e&&e<=126||161<=e&&e<=55295&&e!==8232&&e!==8233||57344<=e&&e<=65533&&e!==et||65536<=e&&e<=1114111}function zt(e){return ae(e)&&e!==et&&e!==zr&&e!==ne}function Vt(e,t,i){var n=zt(e),r=n&&!Ce(e);return(i?n:n&&e!==Mi&&e!==Ii&&e!==Bi&&e!==Di&&e!==Pi)&&e!==Ke&&!(t===ke&&!r)||zt(t)&&!Ce(t)&&e===Ke||t===ke&&r}function da(e){return ae(e)&&e!==et&&!Ce(e)&&e!==Jr&&e!==ea&&e!==ke&&e!==Mi&&e!==Ii&&e!==Bi&&e!==Di&&e!==Pi&&e!==Ke&&e!==Gr&&e!==Qr&&e!==Yr&&e!==na&&e!==Xr&&e!==Zr&&e!==qr&&e!==Kr&&e!==Wr&&e!==ta&&e!==ia}function pa(e){return!Ce(e)&&e!==ke}function te(e,t){var i=e.charCodeAt(t),n;return i>=55296&&i<=56319&&t+1<e.length&&(n=e.charCodeAt(t+1),n>=56320&&n<=57343)?(i-55296)*1024+n-56320+65536:i}function Li(e){var t=/^\n* /;return t.test(e)}var _i=1,Ge=2,Ri=3,Oi=4,V=5;function ha(e,t,i,n,r,s,a,o){var l,c=0,u=null,d=!1,p=!1,f=n!==-1,g=-1,h=da(te(e,0))&&pa(te(e,e.length-1));if(t||a)for(l=0;l<e.length;c>=65536?l+=2:l++){if(c=te(e,l),!ae(c))return V;h=h&&Vt(c,u,o),u=c}else{for(l=0;l<e.length;c>=65536?l+=2:l++){if(c=te(e,l),c===ne)d=!0,f&&(p=p||l-g-1>n&&e[g+1]!==" ",g=l);else if(!ae(c))return V;h=h&&Vt(c,u,o),u=c}p=p||f&&l-g-1>n&&e[g+1]!==" "}return!d&&!p?h&&!a&&!r(e)?_i:s===re?V:Ge:i>9&&Li(e)?V:a?s===re?V:Ge:p?Oi:Ri}function fa(e,t,i,n,r){e.dump=(function(){if(t.length===0)return e.quotingType===re?'""':"''";if(!e.noCompatMode&&(ra.indexOf(t)!==-1||aa.test(t)))return e.quotingType===re?'"'+t+'"':"'"+t+"'";var s=e.indent*Math.max(1,i),a=e.lineWidth===-1?-1:Math.max(Math.min(e.lineWidth,40),e.lineWidth-s),o=n||e.flowLevel>-1&&i>=e.flowLevel;function l(c){return ua(e,c)}switch(ha(t,o,e.indent,a,l,e.quotingType,e.forceQuotes&&!n,r)){case _i:return t;case Ge:return"'"+t.replace(/'/g,"''")+"'";case Ri:return"|"+Yt(t,e.indent)+Kt(Ut(t,s));case Oi:return">"+Yt(t,e.indent)+Kt(Ut(ga(t,a),s));case V:return'"'+ma(t)+'"';default:throw new T("impossible error: invalid scalar style")}})()}function Yt(e,t){var i=Li(e)?String(t):"",n=e[e.length-1]===`
`,r=n&&(e[e.length-2]===`
`||e===`
`),s=r?"+":n?"":"-";return i+s+`
`}function Kt(e){return e[e.length-1]===`
`?e.slice(0,-1):e}function ga(e,t){for(var i=/(\n+)([^\n]*)/g,n=(function(){var c=e.indexOf(`
`);return c=c!==-1?c:e.length,i.lastIndex=c,Wt(e.slice(0,c),t)})(),r=e[0]===`
`||e[0]===" ",s,a;a=i.exec(e);){var o=a[1],l=a[2];s=l[0]===" ",n+=o+(!r&&!s&&l!==""?`
`:"")+Wt(l,t),r=s}return n}function Wt(e,t){if(e===""||e[0]===" ")return e;for(var i=/ [^ ]/g,n,r=0,s,a=0,o=0,l="";n=i.exec(e);)o=n.index,o-r>t&&(s=a>r?a:o,l+=`
`+e.slice(r,s),r=s+1),a=o;return l+=`
`,e.length-r>t&&a>r?l+=e.slice(r,a)+`
`+e.slice(a+1):l+=e.slice(r),l.slice(1)}function ma(e){for(var t="",i=0,n,r=0;r<e.length;i>=65536?r+=2:r++)i=te(e,r),n=A[i],!n&&ae(i)?(t+=e[r],i>=65536&&(t+=e[r+1])):t+=n||oa(i);return t}function ya(e,t,i){var n="",r=e.tag,s,a,o;for(s=0,a=i.length;s<a;s+=1)o=i[s],e.replacer&&(o=e.replacer.call(i,String(s),o)),(L(e,t,o,!1,!1)||typeof o>"u"&&L(e,t,null,!1,!1))&&(n!==""&&(n+=","+(e.condenseFlow?"":" ")),n+=e.dump);e.tag=r,e.dump="["+n+"]"}function Gt(e,t,i,n){var r="",s=e.tag,a,o,l;for(a=0,o=i.length;a<o;a+=1)l=i[a],e.replacer&&(l=e.replacer.call(i,String(a),l)),(L(e,t+1,l,!0,!0,!1,!0)||typeof l>"u"&&L(e,t+1,null,!0,!0,!1,!0))&&((!n||r!=="")&&(r+=We(e,t)),e.dump&&ne===e.dump.charCodeAt(0)?r+="-":r+="- ",r+=e.dump);e.tag=s,e.dump=r||"[]"}function xa(e,t,i){var n="",r=e.tag,s=Object.keys(i),a,o,l,c,u;for(a=0,o=s.length;a<o;a+=1)u="",n!==""&&(u+=", "),e.condenseFlow&&(u+='"'),l=s[a],c=i[l],e.replacer&&(c=e.replacer.call(i,l,c)),L(e,t,l,!1,!1)&&(e.dump.length>1024&&(u+="? "),u+=e.dump+(e.condenseFlow?'"':"")+":"+(e.condenseFlow?"":" "),L(e,t,c,!1,!1)&&(u+=e.dump,n+=u));e.tag=r,e.dump="{"+n+"}"}function va(e,t,i,n){var r="",s=e.tag,a=Object.keys(i),o,l,c,u,d,p;if(e.sortKeys===!0)a.sort();else if(typeof e.sortKeys=="function")a.sort(e.sortKeys);else if(e.sortKeys)throw new T("sortKeys must be a boolean or a function");for(o=0,l=a.length;o<l;o+=1)p="",(!n||r!=="")&&(p+=We(e,t)),c=a[o],u=i[c],e.replacer&&(u=e.replacer.call(i,c,u)),L(e,t+1,c,!0,!0,!0)&&(d=e.tag!==null&&e.tag!=="?"||e.dump&&e.dump.length>1024,d&&(e.dump&&ne===e.dump.charCodeAt(0)?p+="?":p+="? "),p+=e.dump,d&&(p+=We(e,t)),L(e,t+1,u,!0,d)&&(e.dump&&ne===e.dump.charCodeAt(0)?p+=":":p+=": ",p+=e.dump,r+=p));e.tag=s,e.dump=r||"{}"}function qt(e,t,i){var n,r,s,a,o,l;for(r=i?e.explicitTypes:e.implicitTypes,s=0,a=r.length;s<a;s+=1)if(o=r[s],(o.instanceOf||o.predicate)&&(!o.instanceOf||typeof t=="object"&&t instanceof o.instanceOf)&&(!o.predicate||o.predicate(t))){if(i?o.multi&&o.representName?e.tag=o.representName(t):e.tag=o.tag:e.tag="?",o.represent){if(l=e.styleMap[o.tag]||o.defaultStyle,Fi.call(o.represent)==="[object Function]")n=o.represent(t,l);else if(Si.call(o.represent,l))n=o.represent[l](t,l);else throw new T("!<"+o.tag+'> tag resolver accepts not "'+l+'" style');e.dump=n}return!0}return!1}function L(e,t,i,n,r,s,a){e.tag=null,e.dump=i,qt(e,i,!1)||qt(e,i,!0);var o=Fi.call(e.dump),l=n,c;n&&(n=e.flowLevel<0||e.flowLevel>t);var u=o==="[object Object]"||o==="[object Array]",d,p;if(u&&(d=e.duplicates.indexOf(i),p=d!==-1),(e.tag!==null&&e.tag!=="?"||p||e.indent!==2&&t>0)&&(r=!1),p&&e.usedDuplicates[d])e.dump="*ref_"+d;else{if(u&&p&&!e.usedDuplicates[d]&&(e.usedDuplicates[d]=!0),o==="[object Object]")n&&Object.keys(e.dump).length!==0?(va(e,t,e.dump,r),p&&(e.dump="&ref_"+d+e.dump)):(xa(e,t,e.dump),p&&(e.dump="&ref_"+d+" "+e.dump));else if(o==="[object Array]")n&&e.dump.length!==0?(e.noArrayIndent&&!a&&t>0?Gt(e,t-1,e.dump,r):Gt(e,t,e.dump,r),p&&(e.dump="&ref_"+d+e.dump)):(ya(e,t,e.dump),p&&(e.dump="&ref_"+d+" "+e.dump));else if(o==="[object String]")e.tag!=="?"&&fa(e,e.dump,t,s,l);else{if(o==="[object Undefined]")return!1;if(e.skipInvalid)return!1;throw new T("unacceptable kind of an object to dump "+o)}e.tag!==null&&e.tag!=="?"&&(c=encodeURI(e.tag[0]==="!"?e.tag.slice(1):e.tag).replace(/!/g,"%21"),e.tag[0]==="!"?c="!"+c:c.slice(0,18)==="tag:yaml.org,2002:"?c="!!"+c.slice(18):c="!<"+c+">",e.dump=c+" "+e.dump)}return!0}function Ea(e,t){var i=[],n=[],r,s;for(qe(e,i,n),r=0,s=n.length;r<s;r+=1)t.duplicates.push(i[n[r]]);t.usedDuplicates=new Array(s)}function qe(e,t,i){var n,r,s;if(e!==null&&typeof e=="object")if(r=t.indexOf(e),r!==-1)i.indexOf(r)===-1&&i.push(r);else if(t.push(e),Array.isArray(e))for(r=0,s=e.length;r<s;r+=1)qe(e[r],t,i);else for(n=Object.keys(e),r=0,s=n.length;r<s;r+=1)qe(e[n[r]],t,i)}function ba(e,t){t=t||{};var i=new ca(t);i.noRefs||Ea(e,i);var n=e;return i.replacer&&(n=i.replacer.call({"":n},"",n)),L(i,0,n,!0,!0)?i.dump+`
`:""}var wa=ba,ka={dump:wa};function tt(e,t){return function(){throw new Error("Function yaml."+e+" is removed in js-yaml 4. Use yaml."+t+" instead, which is now safe by default.")}}var Ca=C,Aa=Xt,Ta=ii,Fa=oi,Sa=li,Ma=Je,Ia=Ti.load,Ba=Ti.loadAll,Da=ka.dump,Pa=T,La={binary:hi,float:si,map:ti,null:ni,pairs:gi,set:mi,timestamp:di,bool:ri,int:ai,merge:pi,omap:fi,seq:ei,str:Zt},_a=tt("safeLoad","load"),Ra=tt("safeLoadAll","loadAll"),Oa=tt("safeDump","dump"),j={Type:Ca,Schema:Aa,FAILSAFE_SCHEMA:Ta,JSON_SCHEMA:Fa,CORE_SCHEMA:Sa,DEFAULT_SCHEMA:Ma,load:Ia,loadAll:Ba,dump:Da,YAMLException:Pa,types:La,safeLoad:_a,safeLoadAll:Ra,safeDump:Oa};var H={name:"",occupation:"",company:"",city:"",skills:[],roles:[],relationships:[],goals:[],focusAreas:[],lastUpdated:new Date().toISOString().split("T")[0]};function Ni(e){let t={...H},i=e.match(/^---\n([\s\S]*?)\n---/);if(i)try{let n=j.load(i[1]);n&&(typeof n.name=="string"&&(t.name=n.name),typeof n.occupation=="string"&&(t.occupation=n.occupation),typeof n.company=="string"&&(t.company=n.company),typeof n.city=="string"&&(t.city=n.city),Array.isArray(n.skills)&&(t.skills=n.skills.map(String)),Array.isArray(n.roles)&&(t.roles=n.roles.map(String)),Array.isArray(n.relationships)&&(t.relationships=n.relationships.map(String)),Array.isArray(n.goals)&&(t.goals=n.goals.map(String)),Array.isArray(n.focusAreas)&&(t.focusAreas=n.focusAreas.map(String)))}catch{}return!t.name&&!t.company?Na(e):t}function Na(e){let t={...H},i=e.split(`
`),n="";for(let r of i){let s=r.trim();if(s.startsWith("## ")){n=s.slice(3);continue}if(n==="\u57FA\u672C\u4FE1\u606F"&&s.startsWith("- ")){let a=s.slice(2),[o,...l]=a.split("\uFF1A"),c=l.join("\uFF1A");o==="\u59D3\u540D"&&(t.name=c),(o==="\u804C\u4E1A"||o==="\u804C\u4F4D")&&(t.occupation=c),(o==="\u516C\u53F8/\u7EC4\u7EC7"||o==="\u516C\u53F8")&&(t.company=c),o==="\u57CE\u5E02"&&(t.city=c)}if(s.startsWith("- ")&&!s.includes("\uFF1A")){let a=s.slice(2);if(a==="_\u6682\u65E0_")continue;n==="\u6280\u80FD\u4E0E\u4E13\u4E1A"&&t.skills.push(a),n==="\u89D2\u8272\u4E0E\u5173\u7CFB"&&t.roles.push(a),n==="\u76EE\u6807\u4E0E\u8BA1\u5212"&&t.goals.push(a),n==="\u5173\u6CE8\u9886\u57DF"&&t.focusAreas.push(a)}if(s.startsWith("- \u5173\u7CFB\uFF1A")&&n==="\u89D2\u8272\u4E0E\u5173\u7CFB"){let a=s.slice(5);a!=="_\u6682\u65E0_"&&t.relationships.push(a)}}return t}var $a="TraceMind/PROFILE.md";async function $i(e){let t=e.vault.getFileByPath($a);if(!t)return{...H};try{let i=await e.vault.read(t);return Ni(i)}catch{return{...H}}}function se(e){let t=0;for(let i=0;i<e.length;i++){let n=e.charCodeAt(i);t=(t<<5)-t+n,t=t&t}return Math.abs(t).toString(16).padStart(8,"0").slice(0,8)}var ja=["company","role","relationship_to_user"],Ha=["responsibility","communicationStyle"],Ua=["personality","preferences","skills"],za=["subtype","status"],Va=["deadline","description"],Ya=["priority","goals"],Ka=["subtype"],Wa=["occurrenceCount","context"],Ga=["context"],qa={company:"P0",project:"P0",task:"P0",product:"P1",technology:"P1",document:"P2",location:"P2",other:"P2"},ao=Object.keys(qa);var it={P0:1.5,P1:1,P2:.5},Te={person:{p0:ja,p1:Ha,p2:Ua},object:{p0:za,p1:Va,p2:Ya},theme:{p0:Ka,p1:Wa,p2:Ga}};function nt(e,t){let i=Te[e],n=i.p0.length===0||i.p0.every(a=>t[a]!=null),r=i.p1.some(a=>t[a]!=null),s=i.p2.some(a=>t[a]!=null);return n&&r&&s?"L3":n&&r?"L2":n?"L1":"L0"}function ji(e,t,i,n=1){let r=Te[e];if(r.p0.length+r.p1.length+r.p2.length===0)return 0;let s=0,a=r.p0.filter(c=>t[c]!=null).length,o=a/Math.max(r.p0.length,1),l=a===0&&r.p0.length>0?r.p0.length/3*.5:o;if(s+=l*it.P0,r.p1.length>0){let c=r.p1.filter(u=>t[u]!=null).length;s+=c/r.p1.length*it.P1}if(r.p2.length>0){let c=r.p2.filter(u=>t[u]!=null).length;s+=c/r.p2.length*it.P2}return s*n*(1+Math.log1p(i))}var rt={create(e){let t=new Date().toISOString();return{id:se(e.name),userId:e.userId||"",cardType:e.cardType,name:e.name,aliases:e.aliases||[],attributes:e.attributes||{},relatedPeople:[],relatedObjects:[],relatedThemes:[],evidenceEntryIds:[],confidence:.5,maturity:nt(e.cardType,e.attributes||{}),status:"needs_confirmation",lifecycle:"candidate",importance:0,createdAt:t,lastUpdated:t}}};var Qa=j.dump,Ja=j.load,Fe="---";function G(e){let t={id:e.id,name:e.name,type:e.cardType,maturity:e.maturity,confidence:e.confidence,status:e.status,aliases:e.aliases,createdAt:e.createdAt,lastUpdated:e.lastUpdated};for(let[a,o]of Object.entries(e.attributes))o!=null&&(t[a]=o);let i=[`# ${e.name}`,""],n=Za(e);n.length>0&&(i.push("## \u57FA\u672C\u4FE1\u606F"),i.push(...n),i.push(""));let r=e.attributes.interactions||[];if(r.length>0){i.push("## \u4E92\u52A8\u8BB0\u5F55");for(let a of r.slice(-5)){let o=a.timestamp?new Date(a.timestamp).toISOString().split("T")[0]:"";i.push("- "+o+" "+a.content)}i.push("")}else e.attributes.interactions;let s=Qa(t).trim();return`${Fe}
${s}
${Fe}

${i.join(`
`)}`}function oe(e){let t=Xa(e),i=t.frontmatter,n=t.body,r=i.type,s=Array.isArray(i.aliases)?i.aliases:[],a=new Set(["id","name","type","maturity","confidence","status","aliases","createdAt","lastUpdated","lifecycle","importance","userId","relatedPeople","relatedObjects","relatedThemes","evidenceEntryIds"]),o={};for(let[l,c]of Object.entries(i))!a.has(l)&&c!=null&&(o[l]=c);return{id:i.id||se(i.name),userId:"",cardType:r,name:i.name,aliases:s,attributes:o,relatedPeople:[],relatedObjects:[],relatedThemes:[],evidenceEntryIds:[],confidence:i.confidence??.5,maturity:i.maturity??"L0",status:i.status??"needs_confirmation",lifecycle:"candidate",importance:0,createdAt:i.createdAt||new Date().toISOString(),lastUpdated:i.lastUpdated||new Date().toISOString()}}function Xa(e){let t=e.trim();if(!t.startsWith(Fe))return{frontmatter:{},body:t};let i=t.indexOf(Fe,3);if(i===-1)return{frontmatter:{},body:t};let n=t.slice(3,i).trim(),r=t.slice(i+3).trim();return{frontmatter:Ja(n)||{},body:r}}function Za(e){let t=[];return e.cardType==="person"&&(e.attributes.company&&t.push("- \u516C\u53F8\uFF1A"+e.attributes.company),e.attributes.role&&t.push("- \u804C\u4F4D\uFF1A"+e.attributes.role),e.attributes.relationship_to_user&&t.push("- \u5173\u7CFB\uFF1A"+e.attributes.relationship_to_user),e.attributes.responsibility&&t.push("- \u804C\u8D23\uFF1A"+e.attributes.responsibility)),e.cardType==="object"&&(e.attributes.subtype&&t.push("- \u7C7B\u578B\uFF1A"+(X(e.cardType,e.attributes.subtype)||e.attributes.subtype)),e.attributes.status&&t.push("- \u72B6\u6001\uFF1A"+e.attributes.status),e.attributes.deadline&&t.push("- \u622A\u6B62\u65E5\u671F\uFF1A"+e.attributes.deadline)),e.cardType==="theme"&&(e.attributes.subtype&&t.push("- \u7C7B\u578B\uFF1A"+(X(e.cardType,e.attributes.subtype)||e.attributes.subtype)),e.attributes.occurrenceCount&&t.push("- \u51FA\u73B0\u6B21\u6570\uFF1A"+e.attributes.occurrenceCount)),t}function es(e,t){let i=`\u4F60\u662F\u4E00\u4E2A\u7CBE\u51C6\u7684\u5B9E\u4F53\u63D0\u53D6\u548C\u65E5\u8BB0\u5206\u7C7B\u4E13\u5BB6\u3002\u8BF7\u5BF9\u4EE5\u4E0B\u65E5\u8BB0\u6587\u672C\u8FDB\u884C\u5206\u6790\u3002
...
${Ct()}`;if(t){let n=t.includes("\u5DF2\u77E5\u5B9E\u4F53");i+=`
...
\u65E5\u8BB0\u6587\u672C\uFF1A
${e}`,i}function ts(e){let t=["person","object","theme"];try{let i=e.trim(),n=i.indexOf('{"entities"');if(n>=0){let l=0,c=n;for(let u=n;u<i.length;u++)if(i[u]==="{")l++;else if(i[u]==="}"&&(l--,l===0)){c=u+1;break}i=i.slice(n,c),console.log('[TraceMind] parseLLM: extracted JSON via {"entities"} pattern:',i.substring(0,200))}else{let l=i.lastIndexOf(">");if(l>=0&&l>i.length*.3){let u=i.slice(l+1).trim();u.startsWith("{")&&(i=u)}console.log("[TraceMind] parseLLM: after tag removal (first 200):",i.substring(0,200));let c=i.match(/```(?:json)?\s*\n?([\s\S]*?)```/);if(c&&(i=c[1].trim()),!i.startsWith("{")){let u=i.indexOf("{"),d=i.lastIndexOf("}");u!==-1&&d!==-1&&(i=i.slice(u,d+1))}console.log("[TraceMind] parseLLM: final JSON (first 200):",i.substring(0,200))}let r=JSON.parse(i);if(console.log("[TraceMind] parseLLM: parsed JSON:",JSON.stringify(r).substring(0,200)),!r.entities||!Array.isArray(r.entities))return{entities:[]};let s=["\u5DE5\u4F5C","\u751F\u6D3B","\u5B66\u4E60","\u8FD0\u52A8","\u5176\u4ED6"],a=typeof r.domain=="string"&&s.includes(r.domain)?r.domain:void 0,o=[];for(let l of r.entities)!l.name||typeof l.name!="string"||l.name.trim()===""||t.includes(l.type)&&o.push({name:l.name.trim(),type:l.type,subtype:l.subtype,confidence:typeof l.confidence=="number"?l.confidence:.5});return{domain:a,entities:o}}catch{return{entities:[]}}}async function Hi(e,t){console.log("[TraceMind] LLM extract called, baseUrl:",t.baseUrl,"model:",t.model);let i=es(e,t.profileContext),n=t.baseUrl.replace(/\/+$/,""),r=n.endsWith("/v1")||n.endsWith("/chat/completions")?n.endsWith("/chat/completions")?n:`${n}/chat/completions`:`${n}/v1/chat/completions`;console.log("[TraceMind] LLM URL:",r),console.log("[TraceMind] LLM prompt:",i.substring(0,200));let s=await fetch(r,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t.apiKey}`},body:JSON.stringify({model:t.model,messages:[{role:"user",content:i}],temperature:0,max_tokens:2e3})});if(console.log("[TraceMind] LLM response status:",s.status),!s.ok){let u=await s.text();throw new Error(`LLM extraction failed: HTTP ${s.status} - ${u}`)}let l=(await s.json()).choices?.[0]?.message?.content??"";console.log("[TraceMind] LLM raw response:",l);let c=ts(l);return console.log("[TraceMind] LLM parsed: domain=",c.domain,"entities=",c.entities.length,c.entities),c}var is={L0:30,L1:20,L2:10,L3:10},ns={P0:10,P1:5,P2:2};function rs(e){let t=is[e.maturityLevel]??10;return e.attributePriority&&(t+=ns[e.attributePriority]??2),e.type==="new_entity"&&(t+=10),t}function at(e,t,i,n){let r=[],s=Te[e];for(let a of s.p0)i[a]==null&&r.push({type:"missing_attribute",entityName:"",entityType:e,maturityLevel:t,attributePriority:"P0",missingAttribute:a,score:0,description:`Missing P0 attribute: ${a}`});for(let a of s.p1)i[a]==null&&r.push({type:"missing_attribute",entityName:"",entityType:e,maturityLevel:t,attributePriority:"P1",missingAttribute:a,score:0,description:`Missing P1 attribute: ${a}`});for(let a of s.p2)i[a]==null&&r.push({type:"missing_attribute",entityName:"",entityType:e,maturityLevel:t,attributePriority:"P2",missingAttribute:a,score:0,description:`Missing P2 attribute: ${a}`});t!=="L0"&&n.length===0&&r.push({type:"missing_relation",entityName:"",entityType:e,maturityLevel:t,attributePriority:"P1",score:0,description:"No relations established"});for(let a of r)a.score=rs(a);return r.sort((a,o)=>o.score-a.score)}function as(e){let t={children:new Map,fail:null,output:[]};for(let n of e){let r=t;for(let s of n.text){let a=r.children.get(s);a||(a={children:new Map,fail:null,output:[]},r.children.set(s,a)),r=a}r.output.push({entityName:n.entityName,entityId:n.entityId,matchType:n.matchType,patternLen:n.text.length})}let i=[];for(let n of t.children.values())n.fail=t,i.push(n);for(;i.length>0;){let n=i.shift();for(let[r,s]of n.children){i.push(s);let a=n.fail;for(;a!==null&&!a.children.has(r);)a=a.fail;s.fail=a&&a.children.get(r)||t,s.fail&&s.output.push(...s.fail.output)}}return t}function ss(e,t){let i=[],n=new Set,r=e;for(let s=0;s<t.length;s++){let a=t[s];for(;r!==e&&!r.children.has(a);)r=r.fail;let o=r.children.get(a);o?r=o:r=e;for(let l of r.output){let c=l.entityId+":"+l.matchType;n.has(c)||(n.add(c),i.push({matchedText:t.slice(s-l.patternLen+1,s+1),entityName:l.entityName,entityId:l.entityId,matchType:l.matchType,position:s-l.patternLen+1}))}}return i}function Ui(e,t){if(t.length===0)return[];let i=[];for(let r of t){r.name.length>=2&&i.push({text:r.name,entityName:r.name,entityId:r.id,matchType:"exact"});for(let s of r.aliases||[])s.length>=2&&i.push({text:s,entityName:r.name,entityId:r.id,matchType:"alias"});if(r.name.length>=2){let s=r.name.slice(0,Math.min(3,r.name.length));i.filter(o=>o.entityId===r.id&&o.text===s).length===0&&s.length>=2&&i.push({text:s,entityName:r.name,entityId:r.id,matchType:"prefix"})}}i.sort((r,s)=>s.text.length-r.text.length);let n=as(i);return ss(n,e)}var os=5,zi={company:"\u8FD9\u4E2A**\u516C\u53F8/\u7EC4\u7EC7**\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u4E1A\u52A1\u9886\u57DF\u3001\u5408\u4F5C\u5173\u7CFB\u3001\u76F8\u5173\u80CC\u666F\u7B49\u3002",project:"\u8FD9\u4E2A**\u9879\u76EE**\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5F53\u524D\u72B6\u6001\u3001\u65F6\u95F4\u8282\u70B9\u3001\u76F8\u5173\u80CC\u666F\u7B49\u3002",task:"\u8FD9\u4E2A**\u4EFB\u52A1**\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5F53\u524D\u72B6\u6001\u3001\u65F6\u95F4\u8282\u70B9\u3001\u76F8\u5173\u80CC\u666F\u7B49\u3002",product:"\u8FD9\u4E2A**\u4EA7\u54C1**\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5F53\u524D\u72B6\u6001\u3001\u5173\u952E\u7279\u6027\u3001\u76F8\u5173\u80CC\u666F\u7B49\u3002",technology:"\u8FD9\u4E2A**\u6280\u672F**\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5F53\u524D\u72B6\u6001\u3001\u4E3B\u8981\u7528\u9014\u3001\u76F8\u5173\u80CC\u666F\u7B49\u3002",document:"\u8FD9\u4E2A**\u6587\u6863**\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5F53\u524D\u72B6\u6001\u3001\u4E3B\u8981\u7528\u9014\u3001\u76F8\u5173\u80CC\u666F\u7B49\u3002",location:"\u8FD9\u4E2A**\u5730\u70B9**\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5728\u54EA\u91CC\u3001\u6709\u4EC0\u4E48\u7279\u522B\u4E4B\u5904\u7B49\u3002"},Vi={friction:"\u8FD9\u4E2A**\u6469\u64E6**\u80FD\u804A\u804A\u5417\uFF1F\u6BD4\u5982\u662F\u4EC0\u4E48\u5BFC\u81F4\u7684\u3001\u6301\u7EED\u591A\u4E45\u4E86\u3001\u5F71\u54CD\u6709\u591A\u5927\uFF1F",goal:"\u8FD9\u4E2A**\u76EE\u6807**\u80FD\u804A\u804A\u5417\uFF1F\u6BD4\u5982\u76EE\u524D\u8FDB\u5C55\u3001\u4E0B\u4E00\u6B65\u8BA1\u5212\u3001\u6709\u4EC0\u4E48\u963B\u7887\uFF1F",judgment:"\u8FD9\u4E2A**\u5224\u65AD**\u80FD\u804A\u804A\u5417\uFF1F\u6BD4\u5982\u57FA\u4E8E\u4EC0\u4E48\u5F62\u6210\u7684\u3001\u6709\u591A\u5927\u628A\u63E1\uFF1F",idea:"\u8FD9\u4E2A**\u60F3\u6CD5**\u80FD\u804A\u804A\u5417\uFF1F\u6BD4\u5982\u600E\u4E48\u4EA7\u751F\u7684\u3001\u6709\u6CA1\u6709\u66F4\u5177\u4F53\u7684\u601D\u8003\uFF1F"};function Yi(e,t){return e.type==="new_entity"?e.entityType==="object"&&t&&zi[t]?`${e.entityName} ${zi[t]}`:e.entityType==="object"?`${e.entityName} \u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5F53\u524D\u72B6\u6001\u3001\u65F6\u95F4\u8282\u70B9\u3001\u76F8\u5173\u80CC\u666F\u7B49\u3002`:e.entityType==="theme"&&t&&Vi[t]?`${e.entityName} ${Vi[t]}`:e.entityType==="theme"?`${e.entityName} \u80FD\u804A\u804A\u5417\uFF1F\u6BD4\u5982\u8FD9\u4E2A\u60C5\u51B5\u5F71\u54CD\u6709\u591A\u5927\u3001\u6301\u7EED\u591A\u4E45\u4E86\uFF1F`:e.entityType==="person"?`${e.entityName} \u662F\u8C01\uFF1F\u80FD\u4ECB\u7ECD\u4E00\u4E0B\u5417\uFF1F\u6BD4\u5982\u5728\u54EA\u5BB6\u516C\u53F8\u3001\u4EC0\u4E48\u804C\u4F4D\u3001\u548C\u4F60\u7684\u5173\u7CFB\u7B49\u3002`:`${e.entityName} \u662F\u4EC0\u4E48\uFF1F`:e.type==="missing_attribute"&&e.missingAttribute?{company:e.entityName+" \u5728\u54EA\u4E2A\u516C\u53F8\u6216\u7EC4\u7EC7\u5DE5\u4F5C\uFF1F",role:e.entityName+" \u7684\u804C\u4F4D\u6216\u89D2\u8272\u662F\u4EC0\u4E48\uFF1F",relationship_to_user:"\u4F60\u548C "+e.entityName+" \u662F\u4EC0\u4E48\u5173\u7CFB\uFF1F",responsibility:e.entityName+" \u8D1F\u8D23\u4EC0\u4E48\u5DE5\u4F5C\uFF1F",subtype:e.entityName+" \u662F\u4EC0\u4E48\u7C7B\u578B\uFF1F\u6BD4\u5982\u9879\u76EE\u3001\u4EFB\u52A1\u3001\u4EA7\u54C1\u7B49\uFF1F",status:e.entityName+" \u5F53\u524D\u7684\u72B6\u6001\u662F\u4EC0\u4E48\uFF1F",deadline:e.entityName+" \u6709\u622A\u6B62\u65E5\u671F\u6216\u65F6\u95F4\u8282\u70B9\u5417\uFF1F"}[e.missingAttribute]||`\u5173\u4E8E ${e.entityName} \u7684 ${e.missingAttribute} \u4FE1\u606F\u662F\u4EC0\u4E48\uFF1F`:e.type==="missing_relation"?`${e.entityName} \u548C\u4EC0\u4E48\u5176\u4ED6\u5B9E\u4F53\u6709\u5173\u8054\uFF1F`:e.type==="recurring_pattern"?`${e.entityName} \u5DF2\u7ECF\u591A\u6B21\u51FA\u73B0\uFF0C\u5B83\u4EE3\u8868\u4EC0\u4E48\uFF1F`:`\u8BF7\u63D0\u4F9B\u66F4\u591A\u5173\u4E8E ${e.entityName} \u7684\u4FE1\u606F\u3002`}function ls(e,t){for(let[i,n]of t)if(n.name===e.name)return{cardId:i,maturity:n.maturity};return null}var le=class{static analyzeBlock(t,i){return console.warn("[TraceMind] analyzeBlock (sync) is deprecated, use analyzeBlockAsync for LLM extraction"),{entities:[],newEntities:[],existingEntities:[],hasClarifications:!1,gapCount:0}}static async analyzeBlockAsync(t,i,n,r){let s=r&&r.length>0?Ui(t,r):[],a=[...new Set(s.map(h=>h.entityName))],o=new Set(s.map(h=>h.entityId)),l=r&&r.length>0?r.filter(h=>{if(o.has(h.id))return!0;for(let m=0;m<=h.name.length-2;m++)if(t.includes(h.name.slice(m,m+2)))return!0;for(let m of h.aliases||[])for(let v=0;v<=m.length-2;v++)if(t.includes(m.slice(v,v+2)))return!0;return!1}).slice(0,10):[],c=l.length>0?l.map(h=>{let m=h.aliases&&h.aliases.length>0?"\uFF08\u522B\u540D\uFF1A"+h.aliases.join("\u3001")+"\uFF09":"";return h.name+m+" ["+h.cardType+"]"}).join("\u3001"):"";console.log("[TraceMind] AC scan found",s.length,"matches,",l.length,"candidates for LLM:",c);let u=[],d;if(n&&n.apiKey&&n.baseUrl&&n.model){console.log("[TraceMind] LLM config:",{baseUrl:n.baseUrl,model:n.model,hasApiKey:!!n.apiKey});try{let h={...n,profileContext:n.profileContext?n.profileContext+(c?`

\u5DF2\u77E5\u5B9E\u4F53\uFF08\u5DF2\u5EFA\u6863\uFF0C\u4E0D\u8981\u91CD\u590D\u63D0\u53D6\uFF0C\u6CE8\u610F\u76F8\u4F3C\u540D\u79F0\uFF09\uFF1A`+c:""):c?`

\u5DF2\u77E5\u5B9E\u4F53\uFF08\u5DF2\u5EFA\u6863\uFF0C\u4E0D\u8981\u91CD\u590D\u63D0\u53D6\uFF0C\u6CE8\u610F\u76F8\u4F3C\u540D\u79F0\uFF09\uFF1A`+c:""},m=await Hi(t,h);console.log("[TraceMind] LLM extracted:",m.entities.length,m.entities,"domain:",m.domain),u=m.entities.map(v=>({...v})),d=m.domain}catch(h){console.warn("[TraceMind] LLM extraction failed:",h.message)}}else console.log("[TraceMind] No LLM config provided, skipping extraction");let p=new Set(u.map(h=>h.name)),f=new Set;for(let h of s)if(!f.has(h.entityName)&&(f.add(h.entityName),!p.has(h.entityName))){let m=r?.find(v=>v.name===h.entityName);m&&u.push({name:h.entityName,type:m.cardType,confidence:.9})}let g=cs(u,i);return g.domainCategory=d,g}static summarizeResult(t){if(t.entities.length===0)return"\u672A\u68C0\u6D4B\u5230\u9700\u8981\u5173\u6CE8\u7684\u5B9E\u4F53\u3002";let i=[];if(t.newEntities.length>0){let n=t.newEntities.map(r=>r.name).join("\u3001");i.push(`\u53D1\u73B0 ${t.newEntities.length} \u4E2A\u65B0\u5B9E\u4F53\uFF1A${n}`)}if(t.existingEntities.length>0){let n=t.existingEntities.map(r=>r.name).join("\u3001");i.push(`\u63D0\u53CA ${t.existingEntities.length} \u4E2A\u5DF2\u6709\u5B9E\u4F53\uFF1A${n}`)}return t.hasClarifications&&i.push("\u9700\u8981\u8FDB\u4E00\u6B65\u6F84\u6E05\u4FE1\u606F\u3002"),i.join(`
`)}};function cs(e,t){let i=[];for(let a of e){let o=ls(a,t),l=a.subtype?{subtype:a.subtype}:{},c=o?.maturity??nt(a.type,l),u=ji(a.type,l,0),d=[];if(o){let g=at(a.type,c,l,[]);d.push(...g)}else{d.push({type:"new_entity",entityName:a.name,entityType:a.type,maturityLevel:"L0",attributePriority:"P0",score:40,description:`New entity: ${a.name}`});let g=at(a.type,"L0",l,[]);d.push(...g)}let p=d.slice(0,2).map(g=>Yi(g,a.subtype)),f={...a,isNew:!o,existingCardId:o?.cardId,maturity:c,priorityScore:u,clarificationQuestions:p,knowledgeGaps:d};i.push(f)}i.sort((a,o)=>a.isNew!==o.isNew?a.isNew?-1:1:o.priorityScore-a.priorityScore);let n=i.slice(0,os),r=n.flatMap(a=>a.knowledgeGaps??[]),s=r.sort((a,o)=>o.score-a.score)[0];return{entities:n,newEntities:n.filter(a=>a.isNew),existingEntities:n.filter(a=>!a.isNew),hasClarifications:n.some(a=>a.isNew),gapCount:r.length,firstQuestion:s?Yi(s):void 0}}var us=j.load;function U(e,t){let i=ds(e),n=i?.name;n||(n=t.split("/").pop()?.replace(".md","")||"");let r=i?.type||"person",s=i?.maturity||"L0",a=i?.confidence??.5,o=Array.isArray(i?.aliases)?i.aliases:[];return{id:i?.id||se(n),name:n,cardType:r,type:ps(r),subtype:i?.subtype,maturity:s,confidence:a,filePath:t,aliases:o,relationCount:0,lastUpdated:i?.lastUpdated||new Date().toISOString()}}function Ki(e){let t=[];for(let i of e)if(i.content.trim())try{let n=U(i.content,i.path);t.push(n)}catch{}return{entries:t,lastRebuild:new Date().toISOString()}}function ds(e){let t=e.trim();if(!t.startsWith("---"))return null;let i=t.indexOf("---",3);if(i===-1)return null;let n=t.slice(3,i).trim();return us(n)||null}function ps(e){switch(e){case"person":return"person";case"object":return"thing";case"theme":return"idea";default:return"thing"}}function Wi(e,t){let i=t.toLowerCase();return e.entries.filter(n=>n.name.toLowerCase().includes(i)||n.aliases.some(r=>r.toLowerCase().includes(i)))}function q(e,t){let i=e.entries.findIndex(r=>r.id===t.id),n=[...e.entries];return i>=0?n[i]=t:n.push(t),{entries:n,lastRebuild:e.lastRebuild}}function Gi(e){let t=JSON.parse(e);return{blockId:t.blockId,content:t.content,messages:t.messages||[],analysisResult:t.analysisResult,createdAt:t.createdAt,updatedAt:t.updatedAt,currentPhase:t.currentPhase||"analysis"}}var hs="TraceMind/sessions";function st(e){return`${hs}/${e}.json`}function qi(e){return Gi(e)}var _=require("obsidian");var ot=["Daily","Person","Object","Theme","TraceMind"],Qi="TraceMind/PROFILE.md";async function lt(e){try{return await e.stat(Qi)===null}catch{return!0}}function Ji(e,t){new ct(e,t).open()}var ct=class extends _.Modal{onComplete;constructor(t,i){super(t),this.onComplete=i}onOpen(){let{contentEl:t}=this;t.createEl("h2",{text:"\u6B22\u8FCE\u4F7F\u7528 TraceMind"}),t.createEl("p",{text:"TraceMind \u5C06\u5E2E\u52A9\u4F60\u4ECE\u65E5\u8BB0\u4E2D\u81EA\u52A8\u8BC6\u522B\u548C\u6574\u7406\u77E5\u8BC6\u5B9E\u4F53\u3002\u8BF7\u5148\u5B8C\u6210\u521D\u59CB\u8BBE\u7F6E\u3002"});let i=t.createEl("div",{cls:"tracemind-steps"});t.createEl("h3",{text:"\u6B65\u9AA4 1\uFF1A\u521B\u5EFA\u76EE\u5F55\u7ED3\u6784"}),new _.Setting(t).setName("\u521B\u5EFA\u77E5\u8BC6\u76EE\u5F55").setDesc("\u521B\u5EFA Daily\u3001Person\u3001Object\u3001Theme \u76EE\u5F55").addButton(n=>{n.setButtonText("\u521B\u5EFA\u76EE\u5F55"),n.onClick(async()=>{for(let r of ot)await ve(this.app,r);n.setButtonText("\u5DF2\u5B8C\u6210"),n.setDisabled(!0),new _.Notice("\u76EE\u5F55\u521B\u5EFA\u5B8C\u6210")})}),t.createEl("h3",{text:"\u6B65\u9AA4 2\uFF1A\u521B\u5EFA\u7528\u6237\u6863\u6848"}),new _.Setting(t).setName("\u521D\u59CB\u5316 PROFILE.md").setDesc("\u5728 TraceMind \u76EE\u5F55\u4E0B\u521B\u5EFA\u7528\u6237\u6863\u6848\u6587\u4EF6").addButton(n=>{n.setButtonText("\u521B\u5EFA\u6863\u6848"),n.onClick(async()=>{await this.app.vault.create("TraceMind/PROFILE.md",`---
name: ""
occupation: ""
company: ""
city: ""
...
## \u5173\u6CE8\u9886\u57DF
- _\u6682\u65E0_
`),n.setButtonText("\u5DF2\u5B8C\u6210"),n.setDisabled(!0),new _.Notice("\u6863\u6848\u521B\u5EFA\u5B8C\u6210")})}),t.createEl("h3",{text:"\u6B65\u9AA4 3\uFF1A\u5F00\u59CB\u4F7F\u7528"}),new _.Setting(t).setName("\u5B8C\u6210\u8BBE\u7F6E").setDesc("\u8BBE\u7F6E\u5B8C\u6210\u540E\uFF0C\u4F60\u53EF\u4EE5\u5F00\u59CB\u4F7F\u7528 TraceMind \u4E86").addButton(n=>{n.setButtonText("\u5B8C\u6210"),n.setCta(),n.onClick(async()=>{this.close(),await this.onComplete()})})}onClose(){let{contentEl:t}=this;t.empty()}};var fs=/^---\n([\s\S]*?)\n---\n?/;function Xi(e){let t=e.match(fs);if(!t)return null;let i=t[1],n=e.slice(t[0].length),r={};for(let c of i.split(`
`)){let u=c.indexOf(":");if(u===-1)continue;let d=c.slice(0,u).trim(),p=c.slice(u+1).trim();d&&p&&(r[d]=p)}let s=r.date,a=r.contentHash,o=r.generatedAt,l=parseInt(r.blockCount||"0",10);return!s||!a||!o||!n.trim()?null:{date:s,content:n.trim(),contentHash:a,generatedAt:o,blockCount:l}}function Zi(e){return`${["---",`date: ${e.date}`,`generatedAt: ${e.generatedAt}`,`contentHash: ${e.contentHash}`,`blockCount: ${e.blockCount}`,"---"].join(`
`)}

${e.content}
`}function ut(e){return`TraceMind/insights/${e}.md`}z();var en=/^###\s+(\d{2}:\d{2})\s+(.+)$/m,gs=/<!--\s*TM:([a-z0-9]+)\s*-->/;function ms(){return Math.random().toString(16).slice(2,10).padStart(8,"0")}function dt(e){let t=[],i=e.split(`
`),n=0;for(;n<i.length;){let s=i[n].match(en);if(s){let a=s[1],l=s[2].trim().split(/\s+/).filter(d=>d.startsWith("#")).map(d=>d.slice(1));n++;let c=[],u=[];for(;n<i.length;){let d=i[n];if(d.match(en))break;if(!d.trim()){n++;continue}let p=d.match(gs);if(p){let f=p[1];t.push({timestamp:a,content:c.join(`
...
`).trim(),tags:l,blockId:ms(),children:u})}}else n++}return t}var ys=["Daily","Person","Object","Theme","TraceMind/sessions","TraceMind/index","TraceMind/insights"],Se=class extends B.Plugin{settings;userProfile={...H};analysisService=le;entityIndex={entries:[],lastRebuild:""};entityManager;sessionManager;aiProvider;aiAnalysisView;blockEditorView;calendarView;async onload(){console.log("TraceMind: loading...");try{await this.loadSettings(),await this.ensureVaultStructure(),await this.rebuildEntityIndex(),this.userProfile=await $i(this.app),kt(),this.entityManager=new pt(this.app,this),this.sessionManager=new ht(this.app),this.aiProvider=new ft(this),await this.sessionManager.initialize(),this.registerView(D,t=>(this.blockEditorView=new fe(t,this),this.blockEditorView)),this.registerView(Z,t=>(this.aiAnalysisView=new ye(t,this),this.aiAnalysisView)),this.registerView(ee,t=>(this.calendarView=new xe(t,this),this.calendarView.setOnDateClick(i=>this.navigateToDate(i)),this.calendarView)),this.addSettingTab(new de(this.app,this)),this.addRibbonIcon("brain","\u6253\u5F00 TraceMind",()=>{this.openTracemindView()}),this.addRibbonIcon("calendar","\u6253\u5F00\u65E5\u5386",()=>{this.openCalendarView()}),this.addCommand({id:"open-tracemind",name:"\u6253\u5F00 TraceMind \u89C6\u56FE",callback:()=>this.openTracemindView()}),this.addCommand({id:"open-calendar",name:"\u6253\u5F00\u65E5\u5386",callback:()=>this.openCalendarView()}),this.addCommand({id:"analyze-block",name:"\u5206\u6790\u5F53\u524D\u65E5\u8BB0\u5757",callback:()=>this.analyzeCurrentBlock()}),this.addCommand({id:"rebuild-index",name:"\u91CD\u5EFA\u5B9E\u4F53\u7D22\u5F15",callback:()=>this.rebuildEntityIndexCommand()}),this.registerEvent(this.app.workspace.on("editor-change",()=>{this.onEditorChange()})),new B.Notice("TraceMind \u5DF2\u52A0\u8F7D"),console.log("TraceMind: loaded successfully"),await lt(this.app.vault.adapter)&&Ji(this.app,async()=>{await this.ensureVaultStructure()})}catch(t){console.error("TraceMind: Failed to load",t),new B.Notice("TraceMind \u52A0\u8F7D\u5931\u8D25: "+t.message)}}onunload(){console.log("TraceMind: unloading...")}async loadSettings(){let t=await this.loadData();this.settings={...xt,...t}}async saveSettings(){await this.saveData(this.settings)}async ensureVaultStructure(){for(let t of ys)await ve(this.app,t);console.log("TraceMind: vault structure ensured")}autoAnalysisTimer=null;onEditorChange(){this.autoAnalysisTimer&&clearTimeout(this.autoAnalysisTimer),this.autoAnalysisTimer=setTimeout(()=>{this.autoAnalysisTimer=null},2e3)}async rebuildEntityIndexCommand(){await this.rebuildEntityIndex(),new B.Notice(`\u5B9E\u4F53\u7D22\u5F15\u5DF2\u91CD\u5EFA: ${this.entityIndex.entries.length} \u4E2A\u5B9E\u4F53`)}async rebuildEntityIndex(){let t=["Person","Object","Theme"],i=[];for(let n of t)try{let r=await this.app.vault.adapter.list(n+"/");for(let s of r.files)if(s.endsWith(".md")){let a=await this.app.vault.adapter.read(s);i.push({path:s,content:a})}}catch{}this.entityIndex=Ki(i),console.log(`TraceMind: entity index rebuilt with ${this.entityIndex.entries.length} entries`)}async navigateToDate(t){this.app.workspace.getLeavesOfType(D).length===0&&await this.openTracemindView();let n=this.app.workspace.getLeavesOfType(D);for(let r of n){let s=r.view;typeof s.setCurrentDate=="function"&&await s.setCurrentDate(t)}}async openCalendarView(){let{workspace:t}=this.app,i=t.getLeavesOfType(ee);if(i.length>0)t.revealLeaf(i[0]);else{let n=t.getRightLeaf(!1);n&&(await n.setViewState({type:ee,active:!0}),t.revealLeaf(n))}}async openTracemindView(){let{workspace:t}=this.app,i=t.getLeavesOfType(D);if(i.length>0)t.revealLeaf(i[0]);else{let r=t.getLeaf(!1);r&&(await r.setViewState({type:D,active:!0}),t.revealLeaf(r))}if(t.getLeavesOfType(Z).length===0){let r=t.getRightLeaf(!1);r&&(await r.setViewState({type:Z,active:!0}),t.revealLeaf(r))}}async openBlockEditor(){return this.openTracemindView()}async analyzeCurrentBlock(){let t=this.app.workspace.getActiveFile();if(!t){new B.Notice("\u8BF7\u5148\u6253\u5F00\u4E00\u4E2A\u65E5\u8BB0\u6587\u4EF6");return}let i=await this.app.vault.read(t),n=t.basename;console.log("[TraceMind] analyzeCurrentBlock file:",t.path,"content length:",i.length),console.log("[TraceMind] analyzeCurrentBlock content preview:",i.substring(0,300));try{let r=await this.aiProvider.analyzeBlock(i,n);new B.Notice(`\u5206\u6790\u5B8C\u6210: \u68C0\u6D4B\u5230 ${r.entities.length} \u4E2A\u5B9E\u4F53`),console.log("[TraceMind] analyzeCurrentBlock: tmResult:",r),this.updateAIAnalysis(r)}catch(r){new B.Notice("\u5206\u6790\u5931\u8D25: "+r.message),console.error("TraceMind: analysis error",r)}}getAIAnalysisView(){return this.aiAnalysisView}getBlockEditorView(){return this.blockEditorView}getCalendarView(){return this.calendarView}getEntityManager(){return this.entityManager}getSessionManager(){return this.sessionManager}getAIProvider(){return this.aiProvider}getUserProfile(){return this.userProfile}getUserProfileContext(){let t=this.userProfile,i=[];return t.name&&i.push("\u59D3\u540D\uFF1A"+t.name),t.occupation&&i.push("\u804C\u4E1A\uFF1A"+t.occupation),t.company&&i.push("\u516C\u53F8/\u7EC4\u7EC7\uFF1A"+t.company),t.city&&i.push("\u57CE\u5E02\uFF1A"+t.city),t.skills.length>0&&i.push("\u6280\u80FD\uFF1A"+t.skills.join("\u3001")),t.relationships.length>0&&i.push("\u5173\u7CFB\uFF1A"+t.relationships.join("\u3001")),t.goals.length>0&&i.push("\u76EE\u6807\uFF1A"+t.goals.join("\u3001")),t.focusAreas.length>0&&i.push("\u5173\u6CE8\u9886\u57DF\uFF1A"+t.focusAreas.join("\u3001")),i.length===0?"":`\u7528\u6237\u6863\u6848\uFF1A
`+i.map(function(n){return"- "+n}).join(`
`)}buildAnalysisResult(t,i,n){return tn(t,i,n)}updateAIAnalysis(t){this.aiAnalysisView&&this.aiAnalysisView.updateAnalysis(t)}getBlockEditorDate(){if(this.blockEditorView&&this.blockEditorView.currentDate){let t=this.blockEditorView.currentDate;if(t instanceof Date&&!isNaN(t.getTime())){let i=t.getFullYear(),n=String(t.getMonth()+1).padStart(2,"0"),r=String(t.getDate()).padStart(2,"0");return`${i}-${n}-${r}`}if(typeof t=="string"&&t.match(/^\d{4}-\d{2}-\d{2}$/))return t}return null}async getCachedInsight(t){try{let i=ut(t),n=this.app.vault.getFileByPath(i);if(!n)return null;let r=await this.app.vault.read(n);return Xi(r)}catch{return null}}async readDailyDiary(t){try{let i=`Daily/${t}.md`,n=this.app.vault.getFileByPath(i);if(!n){let r=this.app.vault.getFileByPath(`${t}.md`);return r?await this.app.vault.read(r):null}return await this.app.vault.read(n)}catch{return null}}async readYesterdayDiary(t){let i=new Date(t);for(let n=1;n<=7;n++){let r=new Date(i);r.setDate(r.getDate()-n);let s=r.getFullYear(),a=String(r.getMonth()+1).padStart(2,"0"),o=String(r.getDate()).padStart(2,"0"),l=`${s}-${a}-${o}`,c=await this.readDailyDiary(l);if(c)return c}return""}async hasMinimumBlocks(t){let i=await this.readDailyDiary(t);return i?dt(i).length>=5:!1}async generateDailyInsight(t,i){let n=await this.readDailyDiary(t);if(!n)throw new Error("\u627E\u4E0D\u5230\u4ECA\u5929\u7684\u65E5\u8BB0\u6587\u4EF6");let r=await this.readYesterdayDiary(t),s=this.getUserProfileContext(),a=Ft(this.entityIndex.entries),o=Tt({todayBlocks:n,yesterdayBlocks:r,profileContext:s,entityIndexSummary:a}),l=this.getAIProvider().getProviderForContext("analysis");if(!l||!l.apiKey)throw new Error("\u8BF7\u5148\u5728\u8BBE\u7F6E\u4E2D\u914D\u7F6E AI Provider");let c={provider:"openai",apiKey:l.apiKey,model:l.model,baseUrl:l.baseUrl},u="",d=null;if(await _e(o,c,{onDelta:x=>{u+=x,i.onDelta(x)},onDone:x=>{},onError:x=>{d=x,i.onError(x)}}),d)throw d;if(!u)throw new Error("LLM \u8FD4\u56DE\u4E86\u7A7A\u5185\u5BB9");let p=await me(n,r),f=dt(n),g={date:t,content:u,contentHash:p,generatedAt:new Date().toISOString(),blockCount:f.length},h=ut(t),m=Zi(g),v=this.app.vault.getFileByPath(h);return v?await this.app.vault.modify(v,m):await this.app.vault.create(h,m),i.onDone(u),g}};function tn(e,t,i,n){let r=[],s=[],a=[],o={people:r,objects:s,dimensions:a},l={person:"people",object:"objects",theme:"dimensions"},c=[];for(let u of e){let d=l[u.type],p=i.indexOf(u.name),f=u.name;if(p>=0){let g=Math.max(0,p-20),h=Math.min(i.length,p+u.name.length+30),m=i.slice(g,h);g>0&&(m="..."+m),h<i.length&&(m+="..."),f=m}o[d].push({type:u.type,name:u.name,confidence:u.confidence??.5,context:f,isArchived:!!u.existingCardId,newEntity:u.isNew,maturity:u.maturity,priorityScore:u.priorityScore,clarificationQuestions:u.clarificationQuestions}),u.isNew&&c.push(u.name)}return{blockId:t,timestamp:new Date().toISOString(),category:c.length>0?"\u5F85\u786E\u8BA4":n||"\u5DE5\u4F5C",areas:n?[n]:[],entities:{people:r,objects:s,dimensions:a},needsConfirmation:c,aiResponse:xs(e)}}function xs(e){let t=[];for(let i of e)i.clarificationQuestions.length>0&&t.push(`\u5173\u4E8E ${i.name}\uFF1A${i.clarificationQuestions[0]}`);return t.length===0?`\u68C0\u6D4B\u5230\u4EE5\u4E0B\u5B9E\u4F53\uFF1A${e.map(n=>n.name).join("\u3001")}\u3002`:t.join(`
`)}var pt=class{constructor(t,i){this.app=t;this.plugin=i}app;plugin;findEntity(t){let i=Wi(this.plugin.entityIndex,t);return i.find(r=>r.name.toLowerCase()===t.toLowerCase())||i[0]||null}getEntity(t){return this.plugin.entityIndex.entries.find(i=>i.id===t)||null}async createEntity(t){let i=vs(t.type),n=t.aliases||[],r=rt.create({name:t.title,cardType:i,attributes:t.metadata||{},aliases:n});t.interactions&&Array.isArray(t.interactions)&&(r.attributes.interactions=t.interactions);let s=G(r),o=`${Es(i)}${t.title}.md`;this.app.vault.getFileByPath(o)||await this.app.vault.create(o,s);let c=U(s,o);return this.plugin.entityIndex=q(this.plugin.entityIndex,c),{...t,id:c.id}}async updateEntity(t,i){let n=this.getEntity(t);if(!n)return;let r=this.app.vault.getFileByPath(n.filePath);if(!r)return;let s=await this.app.vault.read(r),a=oe(s);for(let[c,u]of Object.entries(i))c==="lastUpdated"?a.lastUpdated=u:c==="interactions"?a.attributes.interactions=u:c==="aliases"&&Array.isArray(u)?a.aliases=u:a.attributes[c]=u;a.lastUpdated=i.lastUpdated||new Date().toISOString();let o=G(a);await this.app.vault.modify(r,o);let l=U(o,n.filePath);this.plugin.entityIndex=q(this.plugin.entityIndex,l)}wikifyContent(t){let i=t;i=i.replace(/\[\[(Person|Object|Theme)\/(?:\[\[(?:Person|Object|Theme)\/[^\]]+\]\])\|([^\]]+)\]\]/g,"[[$1/$2|$2]]");let r=[...this.plugin.entityIndex.entries].sort((a,o)=>o.name.length-a.name.length),s=[];for(let a of r){let o=a.cardType==="person"?"Person":a.cardType==="object"?"Object":"Theme";a.name.length>=2&&s.push({term:a.name,name:a.name,folder:o});for(let l of a.aliases||[])l.length>=2&&s.push({term:l,name:a.name,folder:o})}s.sort((a,o)=>o.term.length-a.term.length);for(let a of s){let o=a.term.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");if(!new RegExp("(?<!\\[\\[)"+o+"(?!\\]\\|)").test(i))continue;let l="[["+a.folder+"/"+a.name+"|"+a.term+"]]",c=new RegExp("(?<!\\[\\[)"+o+"(?!\\]\\|)","g");i=i.replace(c,l)}return i}async refreshWikilinks(t){let i=this.getEntity(t);if(!i)return;let n=this.app.vault.getFileByPath(i.filePath);if(!n)return;let r=await this.app.vault.read(n),s=oe(r),a=s.attributes.interactions||[],o=!1;for(let l of a)if(l.content&&typeof l.content=="string"){let c=this.wikifyContent(l.content);c!==l.content&&(l.content=c,o=!0)}if(o){s.attributes.interactions=a;let l=G(s);await this.app.vault.modify(n,l);let c=U(l,i.filePath);this.plugin.entityIndex=q(this.plugin.entityIndex,c)}}async addInteraction(t,i){let n=this.getEntity(t);if(!n)return;let r=this.app.vault.getFileByPath(n.filePath);if(!r)return;let s=await this.app.vault.read(r),a=oe(s),o=a.attributes.interactions||[];o.push(i),a.attributes.interactions=o,a.lastUpdated=new Date().toISOString();let l=G(a);await this.app.vault.modify(r,l);let c=U(l,n.filePath);this.plugin.entityIndex=q(this.plugin.entityIndex,c)}async linkRelatedEntities(t){if(!(t.length<2))for(let i of t){let n=this.findEntity(i.name);if(!n)continue;let r=this.app.vault.getFileByPath(n.filePath);if(!r)continue;let s=await this.app.vault.read(r),a=oe(s);for(let c of t)c.name!==i.name&&(c.type==="person"?a.relatedPeople.includes(c.name)||a.relatedPeople.push(c.name):c.type==="object"?a.relatedObjects.includes(c.name)||a.relatedObjects.push(c.name):c.type==="theme"&&(a.relatedThemes.includes(c.name)||a.relatedThemes.push(c.name)));let o=G(a);await this.app.vault.modify(r,o);let l=U(o,n.filePath);this.plugin.entityIndex=q(this.plugin.entityIndex,l)}}async enrichEntity(t,i){return i}buildEntityIndex(){let t=new Map;for(let i of this.plugin.entityIndex.entries)t.set(i.id,new Set([i.name,...i.aliases]));return t}},ht=class{constructor(t){this.app=t}app;cache=new Map;chatSession={blockId:"chat:global",messages:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};async initialize(){try{let t=await this.app.vault.adapter.list("TraceMind/sessions/");for(let i of t.files)if(i.endsWith(".json")){let n=await this.app.vault.adapter.read(i),r=qi(n);this.cache.set(r.blockId,this.toViewSession(r))}}catch{}}getSession(t,i){return this.cache.get(t)||null}getOrCreateSession(t,i){let n=this.cache.get(t);if(n)return n;let r={blockId:t,content:"",messages:[],analysisResult:null,reviewCards:{},createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),currentPhase:"detection"};return this.cache.set(t,r),r}setContent(t,i,n){let r=this.getOrCreateSession(t,n);r.content=i,r.updatedAt=new Date().toISOString(),this.writeSession(t,r)}setSession(t,i,n){let r=this.getOrCreateSession(t,n),s={...r,...i,blockId:t,updatedAt:new Date().toISOString(),analysisResult:i.analysisResult??r.analysisResult};return this.cache.set(t,s),this.writeSession(t,s),s}setAnalysisResult(t,i,n){let r=this.getOrCreateSession(t,n);r.analysisResult=i,r.updatedAt=new Date().toISOString(),r.currentPhase="complete",this.cache.set(t,r),this.writeSession(t,r)}addMessage(t,i,n){let r=this.getOrCreateSession(t,n);r.messages.push(i),r.updatedAt=new Date().toISOString(),this.cache.set(t,r),this.writeSession(t,r)}addChatMessage(t){this.chatSession.messages.push(t),this.chatSession.updatedAt=new Date().toISOString()}getChatSession(){return this.chatSession}clearChatSession(){this.chatSession={blockId:"chat:global",messages:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}}async clearSession(t){this.cache.delete(t);try{let i=this.app.vault.getFileByPath(st(t));i&&await this.app.vault.delete(i)}catch{}}updateReviewCard(t,i,n,r){let s=this.getOrCreateSession(t,r);s.reviewCards||(s.reviewCards={}),s.reviewCards[i]={status:n.status||"pending",supplement:n.supplement,updatedAt:new Date().toISOString()},s.updatedAt=new Date().toISOString(),this.cache.set(t,s),this.writeSession(t,s)}writeSession(t,i){try{let n=st(t),r={blockId:i.blockId,content:i.content,messages:i.messages,createdAt:i.createdAt,updatedAt:i.updatedAt,currentPhase:i.currentPhase};i.analysisResult&&(r.analysisResult=i.analysisResult);let s=JSON.stringify(r,null,2),a=this.app.vault.getFileByPath(n);a?this.app.vault.modify(a,s):this.app.vault.create(n,s)}catch(n){console.error("TraceMind: failed to write session",n)}}toViewSession(t){return{blockId:t.blockId,content:t.content,messages:t.messages,analysisResult:t.analysisResult??null,reviewCards:t.reviewCards??{},createdAt:t.createdAt,updatedAt:t.updatedAt,currentPhase:t.currentPhase||"detection"}}},ft=class{constructor(t){this.plugin=t}plugin;isReady(){let{settings:t}=this.plugin;if(!t.defaultProviderId)return!1;let i=t.providers.find(n=>n.id===t.defaultProviderId);return!!i&&!!i.apiKey&&!!i.baseUrl}async chat(t,i){let n=this.getProviderForContext(i??"chat");if(!n)throw new Error("No AI provider configured");let{chat:r}=await Promise.resolve().then(()=>(z(),Re));return{content:(await r(t.map(a=>({role:a.role,content:a.content})),{provider:"openai",apiKey:n.apiKey,model:n.model,baseUrl:n.baseUrl})).content,usage:{promptTokens:0,completionTokens:0,totalTokens:0}}}async streamChat(t,i,n){let r=this.getProviderForContext(n??"chat");if(!r){i.onError(new Error("No AI provider configured"));return}let{streamChat:s}=await Promise.resolve().then(()=>(z(),Re));await s(t.map(a=>({role:a.role,content:a.content})),{provider:"openai",apiKey:r.apiKey,model:r.model,baseUrl:r.baseUrl},i)}async analyzeBlock(t,i=""){let n=new Map;for(let l of this.plugin.entityIndex.entries)n.set(l.id,{name:l.name,cardType:l.type,maturity:l.maturity||"L0"});console.log("[TraceMind] analyzeBlock: loaded",n.size,"existing cards from index");let r=this.getProviderForContext("analysis");if(!r)return console.warn("[TraceMind] analyzeBlock: no AI provider configured, cannot extract entities"),new B.Notice("\u8BF7\u5148\u5728\u8BBE\u7F6E\u4E2D\u914D\u7F6E AI Provider"),{entities:[],newEntities:[],existingEntities:[],hasClarifications:!1,gapCount:0};console.log("[TraceMind] analyzeBlock: using LLM extraction, provider:",r.name);let s=this.plugin.getUserProfileContext(),a=await le.analyzeBlockAsync(t,n,{apiKey:r.apiKey||"",model:r.model||"gpt-4",baseUrl:r.baseUrl||"",profileContext:s||void 0},this.plugin.entityIndex.entries);console.log("[TraceMind] analyzeBlock result entities:",a.entities.length,a);let o=tn(a.entities,i,t,a.domainCategory);return{...o,analysisResult:o}}getProviderForContext(t){let{settings:i}=this.plugin,n=i.agentProviderMapping,r=t==="analysis"?n.analysis:n.chat;if(r){let s=i.providers.find(a=>a.id===r);if(s)return s}return this.getDefaultProvider()}getDefaultProvider(){let{settings:t}=this.plugin;return t.defaultProviderId&&t.providers.find(i=>i.id===t.defaultProviderId)||null}};function vs(e){return e==="person"?"person":e==="object"?"object":e==="theme"?"theme":e==="project"||e==="thing"?"object":e==="idea"||e==="knowledge"?"theme":"object"}function Es(e){return{person:"Person/",object:"Object/",theme:"Theme/"}[e]||""}var bs=Se;0&&(module.exports={TraceMindPlugin});
