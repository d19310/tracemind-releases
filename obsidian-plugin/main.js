"use strict";var Ce=Object.defineProperty;var zi=Object.getOwnPropertyDescriptor;var Vi=Object.getOwnPropertyNames;var Yi=Object.prototype.hasOwnProperty;var pt=(e,t)=>()=>(e&&(t=e(e=0)),t);var Te=(e,t)=>{for(var i in t)Ce(e,i,{get:t[i],enumerable:!0})},Ki=(e,t,i,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let r of Vi(t))!Yi.call(e,r)&&r!==i&&Ce(e,r,{get:()=>t[r],enumerable:!(n=zi(t,r))||n.enumerable});return e};var Wi=e=>Ki(Ce({},"__esModule",{value:!0}),e);var ft={};Te(ft,{buildRequest:()=>Se,chat:()=>oe,extractStreamDelta:()=>Ae,parseResponse:()=>ht,streamChat:()=>Ie,validateConfig:()=>Gi});function Gi(e){return e.provider?!e.apiKey||e.apiKey.trim()===""?{valid:!1,error:"apiKey is required"}:!e.model||e.model.trim()===""?{valid:!1,error:"model is required"}:{valid:!0}:{valid:!1,error:"provider is required"}}function Se(e,t){let i=t.find(r=>r.role==="system"),n=t.filter(r=>r.role!=="system");switch(e.provider){case"openai":{let r=(e.baseUrl||"https://api.openai.com").replace(/\/+$/,""),s=r.endsWith("/v1")?"/chat/completions":"/v1/chat/completions";return{url:`${r}${s}`,method:"POST",headers:{Authorization:`Bearer ${e.apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:e.model,messages:t})}}case"anthropic":return{url:e.baseUrl||"https://api.anthropic.com/v1/messages",method:"POST",headers:{"x-api-key":e.apiKey,"anthropic-version":"2023-06-01","Content-Type":"application/json"},body:JSON.stringify({model:e.model,system:i?.content,messages:n.map(r=>({role:r.role,content:r.content}))})};default:{let s=(e.baseUrl||(e.provider==="ollama"?"http://localhost:11434":"")).replace(/\/+$/,""),a=s.endsWith("/v1")?"/chat/completions":"/v1/chat/completions";return{url:`${s}${a}`,method:"POST",headers:{"Content-Type":"application/json",...e.apiKey?{Authorization:`Bearer ${e.apiKey}`}:{}},body:JSON.stringify({model:e.model,messages:t})}}}}async function oe(e,t){let i=Se(t,e),n=await fetch(i.url,{method:i.method||"POST",headers:i.headers,body:i.body});if(!n.ok){let a=await n.text();throw new Error(`HTTP ${n.status}: ${a}`)}let r=await n.json();return{content:ht(t.provider,r).content}}function Ae(e,t){if(!t.startsWith("data: "))return"";let i=t.slice(6).trim();if(!i||i==="[DONE]")return"";try{let n=JSON.parse(i);if(e==="anthropic")return n.type==="content_block_delta"&&n.delta?.text?n.delta.text:"";let r=n.choices;return r&&r.length>0&&r[0].delta?.content?r[0].delta.content:""}catch{return""}}async function Ie(e,t,i){try{let n=Se(t,e),r=JSON.parse(n.body||"{}");r.stream=!0;let s={...n.headers,Accept:"text/event-stream"},a=await fetch(n.url,{method:n.method||"POST",headers:s,body:JSON.stringify(r)});if(!a.ok){let d=await a.text();throw new Error(`HTTP ${a.status}: ${d}`)}if(!a.body)throw new Error("Response body is null \u2014 streaming not supported");let o=a.body.getReader(),l=new TextDecoder,c="",u="";for(;;){let{done:d,value:p}=await o.read();if(d)break;u+=l.decode(p,{stream:!0});let f=u.split(`
`);u=f.pop()||"";for(let g of f){let h=g.trim();if(!h)continue;let y=Ae(t.provider,h);y&&(c+=y,i.onDelta(y))}}if(u.trim()){let d=Ae(t.provider,u.trim());d&&(c+=d,i.onDelta(d))}i.onDone(c)}catch(n){i.onError(n)}}function ht(e,t){switch(e){case"openai":{let i=t.choices;if(!i||i.length===0)throw new Error("Empty response from OpenAI");return{role:"assistant",content:i[0].message.content}}case"anthropic":{let i=t.content;if(!i||i.length===0)throw new Error("Empty response from Anthropic");return{role:"assistant",content:i[0].text}}default:{let i=t.choices;if(!i||i.length===0)throw new Error("Empty response from AI provider");return{role:"assistant",content:i[0].message.content}}}}var W=pt(()=>{"use strict"});var Ne={};Te(Ne,{buildDailyInsightPrompt:()=>_e,buildEntityIndexSummary:()=>Oe,computeContentHash:()=>Re});function _e(e){let t=[];return t.push("## \u7528\u6237\u80CC\u666F"),t.push(e.profileContext||"\u6682\u65E0\u7528\u6237\u80CC\u666F\u4FE1\u606F"),t.push(""),t.push("## \u5B9E\u4F53\u6863\u6848\u6458\u8981"),t.push(e.entityIndexSummary||"\u6682\u65E0\u5B9E\u4F53\u6863\u6848"),t.push(""),t.push("## \u4ECA\u5929\u7684\u65E5\u8BB0"),t.push(e.todayBlocks||"(\u4ECA\u5929\u8FD8\u6CA1\u6709\u5199\u65E5\u8BB0)"),t.push(""),t.push("## \u524D\u4E00\u5929\u7684\u65E5\u8BB0"),t.push(e.yesterdayBlocks||"(\u524D\u4E00\u5929\u6CA1\u6709\u65E5\u8BB0)"),[{role:"system",content:en},{role:"user",content:t.join(`
`)}]}async function Re(e,t){let i=e+"|||"+t,r=new TextEncoder().encode(i),s=await crypto.subtle.digest("SHA-256",r);return Array.from(new Uint8Array(s)).map(o=>o.toString(16).padStart(2,"0")).join("")}function Oe(e){if(e.length===0)return"\u6682\u65E0\u5B9E\u4F53\u6863\u6848";let t=new Map;for(let s of e){let a=s.cardType||s.type||"unknown";t.has(a)||t.set(a,[]),t.get(a).push(s)}let i={person:"\u4EBA\u7269",object:"\u5BA2\u4F53",theme:"\u4E3B\u9898"},n=[],r=["person","object","theme"];for(let s of r){let a=t.get(s);if(!a||a.length===0)continue;let o=[...a].sort((u,d)=>new Date(d.lastUpdated).getTime()-new Date(u.lastUpdated).getTime()).slice(0,20),l=i[s]||s,c=o.map(u=>`${u.name}(${u.maturity||"L0"})`).join(", ");n.push(`${l}(${o.length}): ${c}`)}return n.join("; ")||"\u6682\u65E0\u5B9E\u4F53\u6863\u6848"}var en,pe=pt(()=>{"use strict";en=`\u4F60\u662F\u4E00\u4F4D\u6D1E\u5BDF\u529B\u654F\u9510\u7684\u65E5\u8BB0\u5206\u6790\u4E13\u5BB6\u3002\u7528\u6237\u6BCF\u5929\u8BB0\u5F55\u751F\u6D3B\u548C\u5DE5\u4F5C\u65E5\u8BB0\uFF0C\u4F60\u9700\u8981\u6839\u636E\u5F53\u5929\u7684\u65E5\u8BB0\u5185\u5BB9\uFF0C\u751F\u6210\u4E00\u4EFD\u7ED3\u6784\u5316\u7684"\u4ECA\u65E5\u6D1E\u5BDF"\u62A5\u544A\u3002
...
`))}async submitBlock(t){let i=t.value.trim();if(!i||this.isLoading)return;this.isLoading=!0;let n=new Date,r=`${n.getHours().toString().padStart(2,"0")}:${n.getMinutes().toString().padStart(2,"0")}`,s={id:yt(),timestamp:r,source:"TraceMind",category:"\u5F85\u5206\u6790",content:i,children:[],parentId:null};this.blocks.push(s),t.value="",this.inputValue="",this.renderBlocks(),await this.appendBlockToFile(s),await this.startAIAnalysis(s),this.isLoading=!1}async appendBlockToFile(t){let i=`Daily/${this.currentDate}.md`,n=this.app.vault.getAbstractFileByPath(i);if(n instanceof v.TFile||this.app.vault.getAbstractFileByPath("Daily")instanceof v.TFolder||await this.app.vault.createFolder("Daily"),(!n||!(n instanceof v.TFile))&&(n=this.app.vault.getAbstractFileByPath(`${this.currentDate}.md`)),!(n instanceof v.TFile)){let o=await mt(this.app.vault,"journal-template.md",{date:this.currentDate})+`
...
`)});let d=i.getChatSession().messages,p=await n.chat([a,...d],"chat"),f=this.stripThinking(p.content);f&&(await this.streamChatMessage(f),i.addChatMessage({role:"assistant",content:f}))}}else c.text&&(await this.streamChatMessage(c.text),i.addChatMessage({role:"assistant",content:c.text}))}}catch(n){console.error("AI chat error:",n),this.hideThinkingIndicator(),this.addChatMessage("assistant","\u62B1\u6B49\uFF0CAI \u54CD\u5E94\u5931\u8D25: "+n.message)}this.isLoading=!1,this.updateSendBtnState()}async handleEntityArchiving(t){let i=this.plugin.getEntityManager();if(i)for(let n of t)try{let r={status:"active",source:"diary"};n.type==="person"?(r.person_kind=/公司|组织|机构|团队/.test(n.smallType||n.context)?"\u7EC4\u7EC7":"\u4E2A\u4EBA",/同事|朋友|客户|供应商|合作伙伴|合作方/.test(n.smallType)&&(r.relationship_to_user=n.smallType)):n.type==="object"?(r.subtype=n.smallType||"other",n.context&&(r.description=n.context)):n.type==="theme"&&(r.subtype=n.smallType||"domain");let s=n.context||`\u4ECE\u65E5\u8BB0\u4E2D\u5F52\u6863\u7684${n.type}`;await i.createEntity({type:n.type,title:n.name,titleRaw:n.name,aliases:[],tags:[],summary:s,confidence:.8,verificationStatus:"verified",createdAt:new Date().toISOString(),createdBy:"ai",lastUpdated:new Date().toISOString(),relatedEntities:[],interactions:[{timestamp:new Date().toISOString(),type:"ai_analysis",content:`\u5F52\u6863\u4E3A${n.smallType}\uFF1A${n.context||"\u65E0"}`,sourceBlockId:this.activeBlockId||void 0}],metadata:r})}catch(r){console.error(`[AIAnalysisPanel] Failed to create entity ${n.name}:`,r)}}async handleEntityUpdate(t){let i=this.plugin.getEntityManager();if(i)for(let n of t)try{let r=i.getEntity(n.entityId);if(!r)continue;let s={lastUpdated:new Date().toISOString()},a=[...r.interactions??[]];for(let o of n.updates)if(o.field.startsWith("metadata.")){let l=o.field.replace("metadata.","");s.metadata={...r.metadata,[l]:o.value}}else o.field==="interactions"?(a.push({timestamp:new Date().toISOString(),type:"ai_analysis",content:o.value,sourceBlockId:this.activeBlockId||void 0}),s.interactions=a):o.field==="summary"&&(s.summary=o.value);await i.updateEntity(n.entityId,s)}catch(r){console.error(`[AIAnalysisPanel] Failed to update entity ${n.name}:`,r)}}async handleRelations(t){let i=this.plugin.getEntityManager();if(i)for(let n of t)try{let r=i.findEntity(n.from),s=i.findEntity(n.to);if(!r||!s)continue;let a=r.relatedEntities||[],o={entityId:s.id,relation:n.relation,context:n.context||`\u901A\u8FC7\u65E5\u8BB0\u5206\u6790\u5EFA\u7ACB\u5173\u7CFB\uFF1A${n.from}\u662F${n.to}\u7684${n.relation}`};a.some(c=>c.entityId===s.id&&c.relation===o.relation)||await i.updateEntity(r.id,{relatedEntities:[...a,o],lastUpdated:new Date().toISOString()})}catch(r){console.error("[AIAnalysisPanel] Failed to create relation:",r)}}async updateBlockCategory(t,i){try{let n=this.app.workspace.getLeavesOfType(B);if(n.length===0)return;let r=n[0].view;if(!r)return;let s=r.getBlockById(t);if(!s)return;(s.category==="\u5F85\u5206\u6790"||s.category!==i)&&(s.category=i,await r.saveBlockToFile(s),console.log(`[AIAnalysisPanel] Updated block ${t} category to ${i}`))}catch(n){console.error("[AIAnalysisPanel] Failed to update block category:",n)}}async showEntityConfirmationDialog(t){if(this.chatMessagesEl)for(let i of t){let n=this.chatMessagesEl.createEl("div",{cls:"lifewiki-entity-confirm"});n.createEl("div",{cls:"lifewiki-entity-confirm-title",text:`\u8BC6\u522B\u5230\u65B0${this.getEntityTypeLabel(i.inferredType)}: **${i.name}**`}),n.createEl("div",{cls:"lifewiki-entity-confirm-reason",text:i.reason||"\u4ECE\u65E5\u8BB0\u4E2D\u53D1\u73B0"});let r=n.createEl("div",{cls:"lifewiki-entity-confirm-buttons"}),s=r.createEl("button",{cls:"lifewiki-entity-confirm-btn archive",text:"\u5F52\u6863",attr:{type:"button"}}),a=r.createEl("button",{cls:"lifewiki-entity-confirm-btn skip",text:"\u8DF3\u8FC7",attr:{type:"button"}});s.addEventListener("click",async()=>{await this.archiveEntity(i),n.remove()}),a.addEventListener("click",()=>{n.remove()})}}getEntityTypeLabel(t){return{person:"\u4EBA\u8109",object:"\u5BA2\u4F53",theme:"\u4E3B\u9898"}[t]||"\u5B9E\u4F53"}maturityLabel(t){return t}async archiveEntity(t){let i=this.plugin.getEntityManager();if(!i)return;let r={person:"person",object:"object",theme:"theme"}[t.inferredType]||"person";try{await i.createEntity({type:r,title:t.name,titleRaw:t.name,aliases:[],tags:[],summary:t.reason||"\u4ECE\u65E5\u8BB0\u4E2D\u5F52\u6863",confidence:.8,verificationStatus:"verified",createdAt:new Date().toISOString(),createdBy:"ai",lastUpdated:new Date().toISOString(),relatedEntities:[],interactions:[{timestamp:new Date().toISOString(),type:"ai_analysis",content:`\u5F52\u6863\uFF1A${t.reason||"\u4ECE\u65E5\u8BB0\u4E2D\u53D1\u73B0"}`,sourceBlockId:this.activeBlockId||void 0}],metadata:{status:"active",source:"diary"}}),this.addChatMessage("assistant",`\u5DF2\u5F52\u6863 **${t.name}**`)}catch(s){console.error("[AIAnalysisPanel] Failed to archive entity:",s),this.addChatMessage("assistant","\u5F52\u6863\u5931\u8D25")}}async onClose(){}};var Et=require("obsidian"),J="tracemind-calendar",fe=class extends Et.ItemView{plugin;currentMonth;onDateClickCallback=null;constructor(t,i){super(t),this.plugin=i,this.currentMonth={year:new Date().getFullYear(),month:new Date().getMonth()}}getViewType(){return J}getDisplayText(){return"\u65E5\u5386"}getIcon(){return"calendar"}async onOpen(){this.renderCalendar()}async onClose(){}setOnDateClick(t){this.onDateClickCallback=t}async handleDateClick(t){this.onDateClickCallback&&this.onDateClickCallback(t)}getMonthDays(t,i){let n=new Date(t,i,1),r=new Date(t,i+1,0),s=[],a=n.getDay();for(let l=a-1;l>=0;l--){let c=new Date(t,i,-l);s.push(c)}for(let l=1;l<=r.getDate();l++)s.push(new Date(t,i,l));let o=42-s.length;for(let l=1;l<=o;l++)s.push(new Date(t,i+1,l));return s}isToday(t){let i=new Date;return t.getFullYear()===i.getFullYear()&&t.getMonth()===i.getMonth()&&t.getDate()===i.getDate()}isCurrentMonth(t){return t.getFullYear()===this.currentMonth.year&&t.getMonth()===this.currentMonth.month}getNextMonth(t,i){return i===11?{year:t+1,month:0}:{year:t,month:i+1}}getPrevMonth(t,i){return i===0?{year:t-1,month:11}:{year:t,month:i-1}}async diaryExistsForDate(t){try{let i=`Daily/${t}.md`,n=this.app.vault.getAbstractFileByPath(i);return n!=null}catch{return!1}}async goToNextMonth(){this.currentMonth=this.getNextMonth(this.currentMonth.year,this.currentMonth.month),this.renderCalendar()}async goToPrevMonth(){this.currentMonth=this.getPrevMonth(this.currentMonth.year,this.currentMonth.month),this.renderCalendar()}async goToToday(){let t=new Date;this.currentMonth={year:t.getFullYear(),month:t.getMonth()},this.renderCalendar()}async renderCalendar(){let t=this.containerEl;t.empty(),this.addStyles();let i=t.createEl("div",{cls:"tracemind-calendar"});this.renderHeader(i),this.renderWeekdayHeader(i),await this.renderDays(i)}renderHeader(t){let i=t.createEl("div",{cls:"lifewiki-calendar-header"});i.createEl("button",{cls:"lifewiki-calendar-nav-btn",text:"\u2039"}).addEventListener("click",()=>this.goToPrevMonth()),i.createEl("span",{cls:"lifewiki-calendar-title",text:this.getMonthTitle()}).addEventListener("click",()=>this.goToToday()),i.createEl("button",{cls:"lifewiki-calendar-nav-btn",text:"\u203A"}).addEventListener("click",()=>this.goToNextMonth())}renderWeekdayHeader(t){let i=["\u65E5","\u4E00","\u4E8C","\u4E09","\u56DB","\u4E94","\u516D"],n=t.createEl("div",{cls:"lifewiki-calendar-weekdays"});for(let r of i)n.createEl("div",{cls:"lifewiki-calendar-weekday",text:r})}async renderDays(t){let i=this.getMonthDays(this.currentMonth.year,this.currentMonth.month),n=t.createEl("div",{cls:"lifewiki-calendar-grid"});for(let r of i){let s=await this.createDayElement(r,n);n.appendChild(s)}}async createDayElement(t,i){let n=i.createEl("div",{cls:"lifewiki-calendar-day"}),r=this.isCurrentMonth(t),s=this.isToday(t);r||n.addClass("lifewiki-calendar-day-other-month"),s&&n.addClass("lifewiki-calendar-day-today");let a=this.formatDate(t);await this.diaryExistsForDate(a)&&n.addClass("lifewiki-calendar-day-has-diary");let l=n.createEl("span",{cls:"lifewiki-calendar-day-num",text:String(t.getDate())});return n.addEventListener("click",()=>{this.handleDateClick(t)}),n}getMonthTitle(){let t=["\u4E00\u6708","\u4E8C\u6708","\u4E09\u6708","\u56DB\u6708","\u4E94\u6708","\u516D\u6708","\u4E03\u6708","\u516B\u6708","\u4E5D\u6708","\u5341\u6708","\u5341\u4E00\u6708","\u5341\u4E8C\u6708"];return`${this.currentMonth.year}\u5E74 ${t[this.currentMonth.month]}`}formatDate(t){let i=t.getFullYear(),n=String(t.getMonth()+1).padStart(2,"0"),r=String(t.getDate()).padStart(2,"0");return`${i}-${n}-${r}`}addStyles(){let t="lifewiki-calendar-styles";if(document.getElementById(t))return;let i=document.createElement("style");i.id=t,i.textContent=`
		...
		`,document.head.appendChild(i)}};async function X(e,t){e.vault.getAbstractFileByPath(t)||await e.vault.createFolder(t)}function $t(e){return typeof e>"u"||e===null}function tn(e){return typeof e=="object"&&e!==null}function nn(e){return Array.isArray(e)?e:$t(e)?[]:[e]}function rn(e,t){var i,n,r,s;if(t)for(s=Object.keys(t),i=0,n=s.length;i<n;i+=1)r=s[i],e[r]=t[r];return e}function an(e,t){var i="",n;for(n=0;n<t;n+=1)i+=e;return i}function sn(e){return e===0&&Number.NEGATIVE_INFINITY===1/e}var on=$t,ln=tn,cn=nn,un=an,dn=sn,pn=rn,E={isNothing:on,isObject:ln,toArray:cn,repeat:un,isNegativeZero:dn,extend:pn};function jt(e,t){var i="",n=e.reason||"(unknown reason)";return e.mark?(e.mark.name&&(i+='in "'+e.mark.name+'" '),i+="("+(e.mark.line+1)+":"+(e.mark.column+1)+")",!t&&e.mark.snippet&&(i+=`

`+e.mark.snippet),n+" "+i):n}function ee(e,t){Error.call(this),this.name="YAMLException",this.reason=e,this.mark=t,this.message=jt(this,!1),Error.captureStackTrace?Error.captureStackTrace(this,this.constructor):this.stack=new Error().stack||""}ee.prototype=Object.create(Error.prototype);ee.prototype.constructor=ee;ee.prototype.toString=function(t){return this.name+": "+jt(this,t)};var A=ee;function $e(e,t,i,n,r){var s="",a="",o=Math.floor(r/2)-1;return n-t>o&&(s=" ... ",t=n-o+s.length),i-n>o&&(a=" ...",i=n+o-a.length),{str:s+e.slice(t,i).replace(/\t/g,"\u2192")+a,pos:n-t+s.length}}function je(e,t){return E.repeat(" ",t-e.length)+e}function hn(e,t){if(t=Object.create(t||null),!e.buffer)return null;t.maxLength||(t.maxLength=79),typeof t.indent!="number"&&(t.indent=1),typeof t.linesBefore!="number"&&(t.linesBefore=3),typeof t.linesAfter!="number"&&(t.linesAfter=2);for(var i=/\r?\n|\r|\0/g,n=[0],r=[],s,a=-1;s=i.exec(e.buffer);)r.push(s.index),n.push(s.index+s[0].length),e.position<=s.index&&a<0&&(a=n.length-2);a<0&&(a=n.length-1);var o="",l,c,u=Math.min(e.line+t.linesAfter,r.length).toString().length,d=t.maxLength-(t.indent+u+3);for(l=1;l<=t.linesBefore&&!(a-l<0);l++)c=$e(e.buffer,n[a-l],r[a-l],e.position-(n[a]-n[a-l]),d),o=E.repeat(" ",t.indent)+je((e.line-l+1).toString(),u)+" | "+c.str+`
`+o;for(c=$e(e.buffer,n[a],r[a],e.position,d),o+=E.repeat(" ",t.indent)+je((e.line+1).toString(),u)+" | "+c.str+`
`,o+=E.repeat("-",t.indent+u+3+c.pos)+`^
`,l=1;l<=t.linesAfter&&!(a+l>=r.length);l++)c=$e(e.buffer,n[a+l],r[a+l],e.position-(n[a]-n[a+l]),d),o+=E.repeat(" ",t.indent)+je((e.line+l+1).toString(),u)+" | "+c.str+`
`;return o.replace(/\n$/,"")}var fn=hn,gn=["kind","multi","resolve","construct","instanceOf","predicate","represent","representName","defaultStyle","styleAliases"],mn=["scalar","sequence","mapping"];function yn(e){var t={};return e!==null&&Object.keys(e).forEach(function(i){e[i].forEach(function(n){t[String(n)]=i})}),t}function xn(e,t){if(t=t||{},Object.keys(t).forEach(function(i){if(gn.indexOf(i)===-1)throw new A('Unknown option "'+i+'" is met in definition of "'+e+'" YAML type.')}),this.options=t,this.tag=e,this.kind=t.kind||null,this.resolve=t.resolve||function(){return!0},this.construct=t.construct||function(i){return i},this.instanceOf=t.instanceOf||null,this.predicate=t.predicate||null,this.represent=t.represent||null,this.representName=t.representName||null,this.defaultStyle=t.defaultStyle||null,this.multi=t.multi||!1,this.styleAliases=yn(t.styleAliases||null),mn.indexOf(this.kind)===-1)throw new A('Unknown kind "'+this.kind+'" is specified for "'+e+'" YAML type.')}var C=xn;function Ct(e,t){var i=[];return e[t].forEach(function(n){var r=i.length;i.forEach(function(s,a){s.tag===n.tag&&s.kind===n.kind&&s.multi===n.multi&&(r=a)}),i[r]=n}),i}function vn(){var e={scalar:{},sequence:{},mapping:{},fallback:{},multi:{scalar:[],sequence:[],mapping:[],fallback:[]}},t,i;function n(r){r.multi?(e.multi[r.kind].push(r),e.multi.fallback.push(r)):e[r.kind][r.tag]=e.fallback[r.tag]=r}for(t=0,i=arguments.length;t<i;t+=1)arguments[t].forEach(n);return e}function Ue(e){return this.extend(e)}Ue.prototype.extend=function(t){var i=[],n=[];if(t instanceof C)n.push(t);else if(Array.isArray(t))n=n.concat(t);else if(t&&(Array.isArray(t.implicit)||Array.isArray(t.explicit)))t.implicit&&(i=i.concat(t.implicit)),t.explicit&&(n=n.concat(t.explicit));else throw new A("Schema.extend argument should be a Type, [ Type ], or a schema definition ({ implicit: [...], explicit: [...] })");i.forEach(function(s){if(!(s instanceof C))throw new A("Specified list of YAML types (or a single Type object) contains a non-Type object.");if(s.loadKind&&s.loadKind!=="scalar")throw new A("There is a non-scalar type in the implicit list of a schema. Implicit resolving of such types is not supported.");if(s.multi)throw new A("There is a multi type in the implicit list of a schema. Multi tags can only be listed as explicit.")}),n.forEach(function(s){if(!(s instanceof C))throw new A("Specified list of YAML types (or a single Type object) contains a non-Type object.")});var r=Object.create(Ue.prototype);return r.implicit=(this.implicit||[]).concat(i),r.explicit=(this.explicit||[]).concat(n),r.compiledImplicit=Ct(r,"implicit"),r.compiledExplicit=Ct(r,"explicit"),r.compiledTypeMap=vn(r.compiledImplicit,r.compiledExplicit),r};var Ht=Ue,Ut=new C("tag:yaml.org,2002:str",{kind:"scalar",construct:function(e){return e!==null?e:""}}),zt=new C("tag:yaml.org,2002:seq",{kind:"sequence",construct:function(e){return e!==null?e:[]}}),Vt=new C("tag:yaml.org,2002:map",{kind:"mapping",construct:function(e){return e!==null?e:{}}}),Yt=new Ht({explicit:[Ut,zt,Vt]});function bn(e){if(e===null)return!0;var t=e.length;return t===1&&e==="~"||t===4&&(e==="null"||e==="Null"||e==="NULL")}function wn(){return null}function kn(e){return e===null}var Kt=new C("tag:yaml.org,2002:null",{kind:"scalar",resolve:bn,construct:wn,predicate:kn,represent:{canonical:function(){return"~"},lowercase:function(){return"null"},uppercase:function(){return"NULL"},camelcase:function(){return"Null"},empty:function(){return""}},defaultStyle:"lowercase"});function En(e){if(e===null)return!1;var t=e.length;return t===4&&(e==="true"||e==="True"||e==="TRUE")||t===5&&(e==="false"||e==="False"||e==="FALSE")}function Cn(e){return e==="true"||e==="True"||e==="TRUE"}function Tn(e){return Object.prototype.toString.call(e)==="[object Boolean]"}var Wt=new C("tag:yaml.org,2002:bool",{kind:"scalar",resolve:En,construct:Cn,predicate:Tn,represent:{lowercase:function(e){return e?"true":"false"},uppercase:function(e){return e?"TRUE":"FALSE"},camelcase:function(e){return e?"True":"False"}},defaultStyle:"lowercase"});function An(e){return 48<=e&&e<=57||65<=e&&e<=70||97<=e&&e<=102}function Sn(e){return 48<=e&&e<=55}function In(e){return 48<=e&&e<=57}function Mn(e){if(e===null)return!1;var t=e.length,i=0,n=!1,r;if(!t)return!1;if(r=e[i],(r==="-"||r==="+")&&(r=e[++i]),r==="0"){if(i+1===t)return!0;if(r=e[++i],r==="b"){for(i++;i<t;i++)if(r=e[i],r!=="_"){if(r!=="0"&&r!=="1")return!1;n=!0}return n&&r!=="_"}if(r==="x"){for(i++;i<t;i++)if(r=e[i],r!=="_"){if(!An(e.charCodeAt(i)))return!1;n=!0}return n&&r!=="_"}if(r==="o"){for(i++;i<t;i++)if(r=e[i],r!=="_"){if(!Sn(e.charCodeAt(i)))return!1;n=!0}return n&&r!=="_"}}if(r==="_")return!1;for(;i<t;i++)if(r=e[i],r!=="_"){if(!In(e.charCodeAt(i)))return!1;n=!0}return!(!n||r==="_")}function Bn(e){var t=e,i=1,n;if(t.indexOf("_")!==-1&&(t=t.replace(/_/g,"")),n=t[0],(n==="-"||n==="+")&&(n==="-"&&(i=-1),t=t.slice(1),n=t[0]),t==="0")return 0;if(n==="0"){if(t[1]==="b")return i*parseInt(t.slice(2),2);if(t[1]==="x")return i*parseInt(t.slice(2),16);if(t[1]==="o")return i*parseInt(t.slice(2),8)}return i*parseInt(t,10)}function Fn(e){return Object.prototype.toString.call(e)==="[object Number]"&&e%1===0&&!E.isNegativeZero(e)}var Gt=new C("tag:yaml.org,2002:int",{kind:"scalar",resolve:Mn,construct:Bn,predicate:Fn,represent:{binary:function(e){return e>=0?"0b"+e.toString(2):"-0b"+e.toString(2).slice(1)},octal:function(e){return e>=0?"0o"+e.toString(8):"-0o"+e.toString(8).slice(1)},decimal:function(e){return e.toString(10)},hexadecimal:function(e){return e>=0?"0x"+e.toString(16).toUpperCase():"-0x"+e.toString(16).toUpperCase().slice(1)}},defaultStyle:"decimal",styleAliases:{binary:[2,"bin"],octal:[8,"oct"],decimal:[10,"dec"],hexadecimal:[16,"hex"]}}),Pn=new RegExp("^(?:[-+]?(?:[0-9][0-9_]*)(?:\\.[0-9_]*)?(?:[eE][-+]?[0-9]+)?|\\.[0-9_]+(?:[eE][-+]?[0-9]+)?|[-+]?\\.(?:inf|Inf|INF)|\\.(?:nan|NaN|NAN))$");function Dn(e){return!(e===null||!Pn.test(e)||e[e.length-1]==="_")}function Ln(e){var t,i;return t=e.replace(/_/g,"").toLowerCase(),i=t[0]==="-"?-1:1,"+-".indexOf(t[0])>=0&&(t=t.slice(1)),t===".inf"?i===1?Number.POSITIVE_INFINITY:Number.NEGATIVE_INFINITY:t===".nan"?NaN:i*parseFloat(t,10)}var _n=/^[-+]?[0-9]+e/;function Rn(e,t){var i;if(isNaN(e))switch(t){case"lowercase":return".nan";case"uppercase":return".NAN";case"camelcase":return".NaN"}else if(Number.POSITIVE_INFINITY===e)switch(t){case"lowercase":return".inf";case"uppercase":return".INF";case"camelcase":return".Inf"}else if(Number.NEGATIVE_INFINITY===e)switch(t){case"lowercase":return"-.inf";case"uppercase":return"-.INF";case"camelcase":return"-.Inf"}else if(E.isNegativeZero(e))return"-0.0";return i=e.toString(10),_n.test(i)?i.replace("e",".e"):i}function On(e){return Object.prototype.toString.call(e)==="[object Number]"&&(e%1!==0||E.isNegativeZero(e))}var qt=new C("tag:yaml.org,2002:float",{kind:"scalar",resolve:Dn,construct:Ln,predicate:On,represent:Rn,defaultStyle:"lowercase"}),Qt=Yt.extend({implicit:[Kt,Wt,Gt,qt]}),Jt=Qt,Xt=new RegExp("^([0-9][0-9][0-9][0-9])-([0-9][0-9])-([0-9][0-9])$"),Zt=new RegExp("^([0-9][0-9][0-9][0-9])-([0-9][0-9]?)-([0-9][0-9]?)(?:[Tt]|[ \\t]+)([0-9][0-9]?):([0-9][0-9]):([0-9][0-9])(?:\\.([0-9]*))?(?:[ \\t]*(Z|([-+])([0-9][0-9]?)(?::([0-9][0-9]))?))?$");function Nn(e){return e===null?!1:Xt.exec(e)!==null||Zt.exec(e)!==null}function $n(e){var t,i,n,r,s,a,o,l=0,c=null,u,d,p;if(t=Xt.exec(e),t===null&&(t=Zt.exec(e)),t===null)throw new Error("Date resolve error");if(i=+t[1],n=+t[2]-1,r=+t[3],!t[4])return new Date(Date.UTC(i,n,r));if(s=+t[4],a=+t[5],o=+t[6],t[7]){for(l=t[7].slice(0,3);l.length<3;)l+="0";l=+l}return t[9]&&(u=+t[10],d=+(t[11]||0),c=(u*60+d)*6e4,t[9]==="-"&&(c=-c)),p=new Date(Date.UTC(i,n,r,s,a,o,l)),c&&p.setTime(p.getTime()-c),p}function jn(e){return e.toISOString()}var ei=new C("tag:yaml.org,2002:timestamp",{kind:"scalar",resolve:Nn,construct:$n,instanceOf:Date,represent:jn});function Hn(e){return e==="<<"||e===null}var ti=new C("tag:yaml.org,2002:merge",{kind:"scalar",resolve:Hn}),We=`ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=
\r`;function Un(e){if(e===null)return!1;var t,i,n=0,r=e.length,s=We;for(i=0;i<r;i++)if(t=s.indexOf(e.charAt(i)),!(t>64)){if(t<0)return!1;n+=6}return n%8===0}function zn(e){var t,i,n=e.replace(/[\r\n=]/g,""),r=n.length,s=We,a=0,o=[];for(t=0;t<r;t++)t%4===0&&t&&(o.push(a>>16&255),o.push(a>>8&255),o.push(a&255)),a=a<<6|s.indexOf(n.charAt(t));return i=r%4*6,i===0?(o.push(a>>16&255),o.push(a>>8&255),o.push(a&255)):i===18?(o.push(a>>10&255),o.push(a>>2&255)):i===12&&o.push(a>>4&255),new Uint8Array(o)}function Vn(e){var t="",i=0,n,r,s=e.length,a=We;for(n=0;n<s;n++)n%3===0&&n&&(t+=a[i>>18&63],t+=a[i>>12&63],t+=a[i>>6&63],t+=a[i&63]),i=(i<<8)+e[n];return r=s%3,r===0?(t+=a[i>>18&63],t+=a[i>>12&63],t+=a[i>>6&63],t+=a[i&63]):r===2?(t+=a[i>>10&63],t+=a[i>>4&63],t+=a[i<<2&63],t+=a[64]):r===1&&(t+=a[i>>2&63],t+=a[i<<4&63],t+=a[64],t+=a[64]),t}function Yn(e){return Object.prototype.toString.call(e)==="[object Uint8Array]"}var ii=new C("tag:yaml.org,2002:binary",{kind:"scalar",resolve:Un,construct:zn,predicate:Yn,represent:Vn}),Kn=Object.prototype.hasOwnProperty,Wn=Object.prototype.toString;function Gn(e){if(e===null)return!0;var t=[],i,n,r,s,a,o=e;for(i=0,n=o.length;i<n;i+=1){if(r=o[i],a=!1,Wn.call(r)!=="[object Object]")return!1;for(s in r)if(Kn.call(r,s))if(!a)a=!0;else return!1;if(!a)return!1;if(t.indexOf(s)===-1)t.push(s);else return!1}return!0}function qn(e){return e!==null?e:[]}var ni=new C("tag:yaml.org,2002:omap",{kind:"sequence",resolve:Gn,construct:qn}),Qn=Object.prototype.toString;function Jn(e){if(e===null)return!0;var t,i,n,r,s,a=e;for(s=new Array(a.length),t=0,i=a.length;t<i;t+=1){if(n=a[t],Qn.call(n)!=="[object Object]"||(r=Object.keys(n),r.length!==1))return!1;s[t]=[r[0],n[r[0]]]}return!0}function Xn(e){if(e===null)return[];var t,i,n,r,s,a=e;for(s=new Array(a.length),t=0,i=a.length;t<i;t+=1)n=a[t],r=Object.keys(n),s[t]=[r[0],n[r[0]]];return s}var ri=new C("tag:yaml.org,2002:pairs",{kind:"sequence",resolve:Jn,construct:Xn}),Zn=Object.prototype.hasOwnProperty;function er(e){if(e===null)return!0;var t,i=e;for(t in i)if(Zn.call(i,t)&&i[t]!==null)return!1;return!0}function tr(e){return e!==null?e:{}}var ai=new C("tag:yaml.org,2002:set",{kind:"mapping",resolve:er,construct:tr}),Ge=Jt.extend({implicit:[ei,ti],explicit:[ii,ni,ri,ai]}),_=Object.prototype.hasOwnProperty,ge=1,si=2,oi=3,me=4,He=1,ir=2,Tt=3,nr=/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x84\x86-\x9F\uFFFE\uFFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]/,rr=/[\x85\u2028\u2029]/,ar=/[,\[\]\{\}]/,li=/^(?:!|!!|![a-z\-]+!)$/i,ci=/^(?:!|[^,\[\]\{\}])(?:%[0-9a-f]{2}|[0-9a-z\-#;\/\?:@&=\+\$,_\.!~\*'\(\)\[\]])*$/i;function At(e){return Object.prototype.toString.call(e)}function I(e){return e===10||e===13}function O(e){return e===9||e===32}function S(e){return e===9||e===32||e===10||e===13}function U(e){return e===44||e===91||e===93||e===123||e===125}function sr(e){var t;return 48<=e&&e<=57?e-48:(t=e|32,97<=t&&t<=102?t-97+10:-1)}function or(e){return e===120?2:e===117?4:e===85?8:0}function lr(e){return 48<=e&&e<=57?e-48:-1}function St(e){return e===48?"\0":e===97?"\x07":e===98?"\b":e===116||e===9?"	":e===110?`
`:e===118?"\v":e===102?"\f":e===114?"\r":e===101?"\x1B":e===32?" ":e===34?'"':e===47?"/":e===92?"\\":e===78?"\x85":e===95?"\xA0":e===76?"\u2028":e===80?"\u2029":""}function cr(e){return e<=65535?String.fromCharCode(e):String.fromCharCode((e-65536>>10)+55296,(e-65536&1023)+56320)}function ui(e,t,i){t==="__proto__"?Object.defineProperty(e,t,{configurable:!0,enumerable:!0,writable:!0,value:i}):e[t]=i}var di=new Array(256),pi=new Array(256);for(R=0;R<256;R++)di[R]=St(R)?1:0,pi[R]=St(R);var R;function ur(e,t){this.input=e,this.filename=t.filename||null,this.schema=t.schema||Ge,this.onWarning=t.onWarning||null,this.legacy=t.legacy||!1,this.json=t.json||!1,this.listener=t.listener||null,this.implicitTypes=this.schema.compiledImplicit,this.typeMap=this.schema.compiledTypeMap,this.length=e.length,this.position=0,this.line=0,this.lineStart=0,this.lineIndent=0,this.firstTabInLine=-1,this.documents=[]}function hi(e,t){var i={name:e.filename,buffer:e.input.slice(0,-1),position:e.position,line:e.line,column:e.position-e.lineStart};return i.snippet=fn(i),new A(t,i)}function m(e,t){throw hi(e,t)}function ye(e,t){e.onWarning&&e.onWarning.call(null,hi(e,t))}var It={YAML:function(t,i,n){var r,s,a;t.version!==null&&m(t,"duplication of %YAML directive"),n.length!==1&&m(t,"YAML directive accepts exactly one argument"),r=/^([0-9]+)\.([0-9]+)$/.exec(n[0]),r===null&&m(t,"ill-formed argument of the YAML directive"),s=parseInt(r[1],10),a=parseInt(r[2],10),s!==1&&m(t,"unacceptable YAML version of the document"),t.version=n[0],t.checkLineBreaks=a<2,a!==1&&a!==2&&ye(t,"unsupported YAML version of the document")},TAG:function(t,i,n){var r,s;n.length!==2&&m(t,"TAG directive accepts exactly two arguments"),r=n[0],s=n[1],li.test(r)||m(t,"ill-formed tag handle (first argument) of the TAG directive"),_.call(t.tagMap,r)&&m(t,'there is a previously declared suffix for "'+r+'" tag handle'),ci.test(s)||m(t,"ill-formed tag prefix (second argument) of the TAG directive");try{s=decodeURIComponent(s)}catch{m(t,"tag prefix is malformed: "+s)}t.tagMap[r]=s}};function L(e,t,i,n){var r,s,a,o;if(t<i){if(o=e.input.slice(t,i),n)for(r=0,s=o.length;r<s;r+=1)a=o.charCodeAt(r),a===9||32<=a&&a<=1114111||m(e,"expected valid JSON character");else nr.test(o)&&m(e,"the stream contains non-printable characters");e.result+=o}}function Mt(e,t,i,n){var r,s,a,o;for(E.isObject(i)||m(e,"cannot merge mappings; the provided source object is unacceptable"),r=Object.keys(i),a=0,o=r.length;a<o;a+=1)s=r[a],_.call(t,s)||(ui(t,s,i[s]),n[s]=!0)}function z(e,t,i,n,r,s,a,o,l){var c,u;if(Array.isArray(r))for(r=Array.prototype.slice.call(r),c=0,u=r.length;c<u;c+=1)Array.isArray(r[c])&&m(e,"nested arrays are not supported inside keys"),typeof r=="object"&&At(r[c])==="[object Object]"&&(r[c]="[object Object]");if(typeof r=="object"&&At(r)==="[object Object]"&&(r="[object Object]"),r=String(r),t===null&&(t={}),n==="tag:yaml.org,2002:merge")if(Array.isArray(s))for(c=0,u=s.length;c<u;c+=1)Mt(e,t,s[c],i);else Mt(e,t,s,i);else!e.json&&!_.call(i,r)&&_.call(t,r)&&(e.line=a||e.line,e.lineStart=o||e.lineStart,e.position=l||e.position,m(e,"duplicated mapping key")),ui(t,r,s),delete i[r];return t}function qe(e){var t;t=e.input.charCodeAt(e.position),t===10?e.position++:t===13?(e.position++,e.input.charCodeAt(e.position)===10&&e.position++):m(e,"a line break is expected"),e.line+=1,e.lineStart=e.position,e.firstTabInLine=-1}function w(e,t,i){for(var n=0,r=e.input.charCodeAt(e.position);r!==0;){for(;O(r);)r===9&&e.firstTabInLine===-1&&(e.firstTabInLine=e.position),r=e.input.charCodeAt(++e.position);if(t&&r===35)do r=e.input.charCodeAt(++e.position);while(r!==10&&r!==13&&r!==0);if(I(r))for(qe(e),r=e.input.charCodeAt(e.position),n++,e.lineIndent=0;r===32;)e.lineIndent++,r=e.input.charCodeAt(++e.position);else break}return i!==-1&&n!==0&&e.lineIndent<i&&ye(e,"deficient indentation"),n}function be(e){var t=e.position,i;return i=e.input.charCodeAt(t),!!((i===45||i===46)&&i===e.input.charCodeAt(t+1)&&i===e.input.charCodeAt(t+2)&&(t+=3,i=e.input.charCodeAt(t),i===0||S(i)))}function Qe(e,t){t===1?e.result+=" ":t>1&&(e.result+=E.repeat(`
`,t-1))}function dr(e,t,i){var n,r,s,a,o,l,c,u,d=e.kind,p=e.result,f;if(f=e.input.charCodeAt(e.position),S(f)||U(f)||f===35||f===38||f===42||f===33||f===124||f===62||f===39||f===34||f===37||f===64||f===96||(f===63||f===45)&&(r=e.input.charCodeAt(e.position+1),S(r)||i&&U(r)))return!1;for(e.kind="scalar",e.result="",s=a=e.position,o=!1;f!==0;){if(f===58){if(r=e.input.charCodeAt(e.position+1),S(r)||i&&U(r))break}else if(f===35){if(n=e.input.charCodeAt(e.position-1),S(n))break}else{if(e.position===e.lineStart&&be(e)||i&&U(f))break;if(I(f))if(l=e.line,c=e.lineStart,u=e.lineIndent,w(e,!1,-1),e.lineIndent>=t){o=!0,f=e.input.charCodeAt(e.position);continue}else{e.position=a,e.line=l,e.lineStart=c,e.lineIndent=u;break}}o&&(L(e,s,a,!1),Qe(e,e.line-l),s=a=e.position,o=!1),O(f)||(a=e.position+1),f=e.input.charCodeAt(++e.position)}return L(e,s,a,!1),e.result?!0:(e.kind=d,e.result=p,!1)}function pr(e,t){var i,n,r;if(i=e.input.charCodeAt(e.position),i!==39)return!1;for(e.kind="scalar",e.result="",e.position++,n=r=e.position;(i=e.input.charCodeAt(e.position))!==0;)if(i===39)if(L(e,n,e.position,!0),i=e.input.charCodeAt(++e.position),i===39)n=e.position,e.position++,r=e.position;else return!0;else I(i)?(L(e,n,r,!0),Qe(e,w(e,!1,t)),n=r=e.position):e.position===e.lineStart&&be(e)?m(e,"unexpected end of the document within a single quoted scalar"):(e.position++,r=e.position);m(e,"unexpected end of the stream within a single quoted scalar")}function hr(e,t){var i,n,r,s,a,o;if(o=e.input.charCodeAt(e.position),o!==34)return!1;for(e.kind="scalar",e.result="",e.position++,i=n=e.position;(o=e.input.charCodeAt(e.position))!==0;){if(o===34)return L(e,i,e.position,!0),e.position++,!0;if(o===92){if(L(e,i,e.position,!0),o=e.input.charCodeAt(++e.position),I(o))w(e,!1,t);else if(o<256&&di[o])e.result+=pi[o],e.position++;else if((a=or(o))>0){for(r=a,s=0;r>0;r--)o=e.input.charCodeAt(++e.position),(a=sr(o))>=0?s=(s<<4)+a:m(e,"expected hexadecimal character");e.result+=cr(s),e.position++}else m(e,"unknown escape sequence");i=n=e.position}else I(o)?(L(e,i,n,!0),Qe(e,w(e,!1,t)),i=n=e.position):e.position===e.lineStart&&be(e)?m(e,"unexpected end of the document within a double quoted scalar"):(e.position++,n=e.position)}m(e,"unexpected end of the stream within a double quoted scalar")}function fr(e,t){var i=!0,n,r,s,a=e.tag,o,l=e.anchor,c,u,d,p,f,g=Object.create(null),h,y,b,x;if(x=e.input.charCodeAt(e.position),x===91)u=93,f=!1,o=[];else if(x===123)u=125,f=!0,o={};else return!1;for(e.anchor!==null&&(e.anchorMap[e.anchor]=o),x=e.input.charCodeAt(++e.position);x!==0;){if(w(e,!0,t),x=e.input.charCodeAt(e.position),x===u)return e.position++,e.tag=a,e.anchor=l,e.kind=f?"mapping":"sequence",e.result=o,!0;i?x===44&&m(e,"expected the node content, but found ','"):m(e,"missed comma between flow collection entries"),y=h=b=null,d=p=!1,x===63&&(c=e.input.charCodeAt(e.position+1),S(c)&&(d=p=!0,e.position++,w(e,!0,t))),n=e.line,r=e.lineStart,s=e.position,V(e,t,ge,!1,!0),y=e.tag,h=e.result,w(e,!0,t),x=e.input.charCodeAt(e.position),(p||e.line===n)&&x===58&&(d=!0,x=e.input.charCodeAt(++e.position),w(e,!0,t),V(e,t,ge,!1,!0),b=e.result),f?z(e,o,g,y,h,b,n,r,s):d?o.push(z(e,null,g,y,h,b,n,r,s)):o.push(h),w(e,!0,t),x=e.input.charCodeAt(e.position),x===44?(i=!0,x=e.input.charCodeAt(++e.position)):i=!1}m(e,"unexpected end of the stream within a flow collection")}function gr(e,t){var i,n,r=He,s=!1,a=!1,o=t,l=0,c=!1,u,d;if(d=e.input.charCodeAt(e.position),d===124)n=!1;else if(d===62)n=!0;else return!1;for(e.kind="scalar",e.result="";d!==0;)if(d=e.input.charCodeAt(++e.position),d===43||d===45)He===r?r=d===43?Tt:ir:m(e,"repeat of a chomping mode identifier");else if((u=lr(d))>=0)u===0?m(e,"bad explicit indentation width of a block scalar; it cannot be less than one"):a?m(e,"repeat of an indentation width identifier"):(o=t+u-1,a=!0);else break;if(O(d)){do d=e.input.charCodeAt(++e.position);while(O(d));if(d===35)do d=e.input.charCodeAt(++e.position);while(!I(d)&&d!==0)}for(;d!==0;){for(qe(e),e.lineIndent=0,d=e.input.charCodeAt(e.position);(!a||e.lineIndent<o)&&d===32;)e.lineIndent++,d=e.input.charCodeAt(++e.position);if(!a&&e.lineIndent>o&&(o=e.lineIndent),I(d)){l++;continue}if(e.lineIndent<o){r===Tt?e.result+=E.repeat(`
`,s?1+l:l):r===He&&s&&(e.result+=`
`);break}for(n?O(d)?(c=!0,e.result+=E.repeat(`
`,s?1+l:l)):c?(c=!1,e.result+=E.repeat(`
`,l+1)):l===0?s&&(e.result+=" "):e.result+=E.repeat(`
`,l):e.result+=E.repeat(`
`,s?1+l:l),s=!0,a=!0,l=0,i=e.position;!I(d)&&d!==0;)d=e.input.charCodeAt(++e.position);L(e,i,e.position,!1)}return!0}function Bt(e,t){var i,n=e.tag,r=e.anchor,s=[],a,o=!1,l;if(e.firstTabInLine!==-1)return!1;for(e.anchor!==null&&(e.anchorMap[e.anchor]=s),l=e.input.charCodeAt(e.position);l!==0&&(e.firstTabInLine!==-1&&(e.position=e.firstTabInLine,m(e,"tab characters must not be used in indentation")),!(l!==45||(a=e.input.charCodeAt(e.position+1),!S(a))));){if(o=!0,e.position++,w(e,!0,-1)&&e.lineIndent<=t){s.push(null),l=e.input.charCodeAt(e.position);continue}if(i=e.line,V(e,t,oi,!1,!0),s.push(e.result),w(e,!0,-1),l=e.input.charCodeAt(e.position),(e.line===i||e.lineIndent>t)&&l!==0)m(e,"bad indentation of a sequence entry");else if(e.lineIndent<t)break}return o?(e.tag=n,e.anchor=r,e.kind="sequence",e.result=s,!0):!1}function mr(e,t,i){var n,r,s,a,o,l,c=e.tag,u=e.anchor,d={},p=Object.create(null),f=null,g=null,h=null,y=!1,b=!1,x;if(e.firstTabInLine!==-1)return!1;for(e.anchor!==null&&(e.anchorMap[e.anchor]=d),x=e.input.charCodeAt(e.position);x!==0;){if(!y&&e.firstTabInLine!==-1&&(e.position=e.firstTabInLine,m(e,"tab characters must not be used in indentation")),n=e.input.charCodeAt(e.position+1),s=e.line,(x===63||x===58)&&S(n))x===63?(y&&(z(e,d,p,f,g,null,a,o,l),f=g=h=null),b=!0,y=!0,r=!0):y?(y=!1,r=!0):m(e,"incomplete explicit mapping pair; a key node is missed; or followed by a non-tabulated empty line"),e.position+=1,x=n;else{if(a=e.line,o=e.lineStart,l=e.position,!V(e,i,si,!1,!0))break;if(e.line===s){for(x=e.input.charCodeAt(e.position);O(x);)x=e.input.charCodeAt(++e.position);if(x===58)x=e.input.charCodeAt(++e.position),S(x)||m(e,"a whitespace character is expected after the key-value separator within a block mapping"),y&&(z(e,d,p,f,g,null,a,o,l),f=g=h=null),b=!0,y=!1,r=!1,f=e.tag,g=e.result;else if(b)m(e,"can not read an implicit mapping pair; a colon is missed");else return e.tag=c,e.anchor=u,!0}else if(b)m(e,"can not read a block mapping entry; a multiline key may not be an implicit key");else return e.tag=c,e.anchor=u,!0}if((e.line===s||e.lineIndent>t)&&(y&&(a=e.line,o=e.lineStart,l=e.position),V(e,t,me,!0,r)&&(y?g=e.result:h=e.result),y||(z(e,d,p,f,g,h,a,o,l),f=g=h=null),w(e,!0,-1),x=e.input.charCodeAt(e.position)),(e.line===s||e.lineIndent>t)&&x!==0)m(e,"bad indentation of a mapping entry");else if(e.lineIndent<t)break}return y&&z(e,d,p,f,g,null,a,o,l),b&&(e.tag=c,e.anchor=u,e.kind="mapping",e.result=d),b}function yr(e){var t,i=!1,n=!1,r,s,a;if(a=e.input.charCodeAt(e.position),a!==33)return!1;if(e.tag!==null&&m(e,"duplication of a tag property"),a=e.input.charCodeAt(++e.position),a===60?(i=!0,a=e.input.charCodeAt(++e.position)):a===33?(n=!0,r="!!",a=e.input.charCodeAt(++e.position)):r="!",t=e.position,i){do a=e.input.charCodeAt(++e.position);while(a!==0&&a!==62);e.position<e.length?(s=e.input.slice(t,e.position),a=e.input.charCodeAt(++e.position)):m(e,"unexpected end of the stream within a verbatim tag")}else{for(;a!==0&&!S(a);)a===33&&(n?m(e,"tag suffix cannot contain exclamation marks"):(r=e.input.slice(t-1,e.position+1),li.test(r)||m(e,"named tag handle cannot contain such characters"),n=!0,t=e.position+1)),a=e.input.charCodeAt(++e.position);s=e.input.slice(t,e.position),ar.test(s)&&m(e,"tag suffix cannot contain flow indicator characters")}s&&!ci.test(s)&&m(e,"tag name cannot contain such characters: "+s);try{s=decodeURIComponent(s)}catch{m(e,"tag name is malformed: "+s)}return i?e.tag=s:_.call(e.tagMap,r)?e.tag=e.tagMap[r]+s:r==="!"?e.tag="!"+s:r==="!!"?e.tag="tag:yaml.org,2002:"+s:m(e,'undeclared tag handle "'+r+'"'),!0}function xr(e){var t,i;if(i=e.input.charCodeAt(e.position),i!==38)return!1;for(e.anchor!==null&&m(e,"duplication of an anchor property"),i=e.input.charCodeAt(++e.position),t=e.position;i!==0&&!S(i)&&!U(i);)i=e.input.charCodeAt(++e.position);return e.position===t&&m(e,"name of an anchor node must contain at least one character"),e.anchor=e.input.slice(t,e.position),!0}function vr(e){var t,i,n;if(n=e.input.charCodeAt(e.position),n!==42)return!1;for(n=e.input.charCodeAt(++e.position),t=e.position;n!==0&&!S(n)&&!U(n);)n=e.input.charCodeAt(++e.position);return e.position===t&&m(e,"name of an alias node must contain at least one character"),i=e.input.slice(t,e.position),_.call(e.anchorMap,i)||m(e,'unidentified alias "'+i+'"'),e.result=e.anchorMap[i],w(e,!0,-1),!0}function V(e,t,i,n,r){var s,a,o,l=1,c=!1,u=!1,d,p,f,g,h,y;if(e.listener!==null&&e.listener("open",e),e.tag=null,e.anchor=null,e.kind=null,e.result=null,s=a=o=me===i||oi===i,n&&w(e,!0,-1)&&(c=!0,e.lineIndent>t?l=1:e.lineIndent===t?l=0:e.lineIndent<t&&(l=-1)),l===1)for(;yr(e)||xr(e);)w(e,!0,-1)?(c=!0,o=s,e.lineIndent>t?l=1:e.lineIndent===t?l=0:e.lineIndent<t&&(l=-1)):o=!1;if(o&&(o=c||r),(l===1||me===i)&&(ge===i||si===i?h=t:h=t+1,y=e.position-e.lineStart,l===1?o&&(Bt(e,y)||mr(e,y,h))||fr(e,h)?u=!0:(a&&gr(e,h)||pr(e,h)||hr(e,h)?u=!0:vr(e)?(u=!0,(e.tag!==null||e.anchor!==null)&&m(e,"alias node should not have any properties")):dr(e,h,ge===i)&&(u=!0,e.tag===null&&(e.tag="?")),e.anchor!==null&&(e.anchorMap[e.anchor]=e.result)):l===0&&(u=o&&Bt(e,y))),e.tag===null)e.anchor!==null&&(e.anchorMap[e.anchor]=e.result);else if(e.tag==="?"){for(e.result!==null&&e.kind!=="scalar"&&m(e,'unacceptable node kind for !<?> tag; it should be "scalar", not "'+e.kind+'"'),d=0,p=e.implicitTypes.length;d<p;d+=1)if(g=e.implicitTypes[d],g.resolve(e.result)){e.result=g.construct(e.result),e.tag=g.tag,e.anchor!==null&&(e.anchorMap[e.anchor]=e.result);break}}else if(e.tag!=="!"){if(_.call(e.typeMap[e.kind||"fallback"],e.tag))g=e.typeMap[e.kind||"fallback"][e.tag];else for(g=null,f=e.typeMap.multi[e.kind||"fallback"],d=0,p=f.length;d<p;d+=1)if(e.tag.slice(0,f[d].tag.length)===f[d].tag){g=f[d];break}g||m(e,"unknown tag !<"+e.tag+">"),e.result!==null&&g.kind!==e.kind&&m(e,"unacceptable node kind for !<"+e.tag+'> tag; it should be "'+g.kind+'", not "'+e.kind+'"'),g.resolve(e.result,e.tag)?(e.result=g.construct(e.result,e.tag),e.anchor!==null&&(e.anchorMap[e.anchor]=e.result)):m(e,"cannot resolve a node with !<"+e.tag+"> explicit tag")}return e.listener!==null&&e.listener("close",e),e.tag!==null||e.anchor!==null||u}function br(e){var t=e.position,i,n,r,s=!1,a;for(e.version=null,e.checkLineBreaks=e.legacy,e.tagMap=Object.create(null),e.anchorMap=Object.create(null);(a=e.input.charCodeAt(e.position))!==0&&(w(e,!0,-1),a=e.input.charCodeAt(e.position),!(e.lineIndent>0||a!==37));){for(s=!0,a=e.input.charCodeAt(++e.position),i=e.position;a!==0&&!S(a);)a=e.input.charCodeAt(++e.position);for(n=e.input.slice(i,e.position),r=[],n.length<1&&m(e,"directive name must not be less than one character in length");a!==0;){for(;O(a);)a=e.input.charCodeAt(++e.position);if(a===35){do a=e.input.charCodeAt(++e.position);while(a!==0&&!I(a));break}if(I(a))break;for(i=e.position;a!==0&&!S(a);)a=e.input.charCodeAt(++e.position);r.push(e.input.slice(i,e.position))}a!==0&&qe(e),_.call(It,n)?It[n](e,n,r):ye(e,'unknown document directive "'+n+'"')}if(w(e,!0,-1),e.lineIndent===0&&e.input.charCodeAt(e.position)===45&&e.input.charCodeAt(e.position+1)===45&&e.input.charCodeAt(e.position+2)===45?(e.position+=3,w(e,!0,-1)):s&&m(e,"directives end mark is expected"),V(e,e.lineIndent-1,me,!1,!0),w(e,!0,-1),e.checkLineBreaks&&rr.test(e.input.slice(t,e.position))&&ye(e,"non-ASCII line breaks are interpreted as content"),e.documents.push(e.result),e.position===e.lineStart&&be(e)){e.input.charCodeAt(e.position)===46&&(e.position+=3,w(e,!0,-1));return}if(e.position<e.length-1)m(e,"end of the stream or a document separator is expected");else return}function fi(e,t){e=String(e),t=t||{},e.length!==0&&(e.charCodeAt(e.length-1)!==10&&e.charCodeAt(e.length-1)!==13&&(e+=`
`),e.charCodeAt(0)===65279&&(e=e.slice(1)));var i=new ur(e,t),n=e.indexOf("\0");for(n!==-1&&(i.position=n,m(i,"null byte is not allowed in input")),i.input+="\0";i.input.charCodeAt(i.position)===32;)i.lineIndent+=1,i.position+=1;for(;i.position<i.length-1;)br(i);return i.documents}function wr(e,t,i){t!==null&&typeof t=="object"&&typeof i>"u"&&(i=t,t=null);var n=fi(e,i);if(typeof t!="function")return n;for(var r=0,s=n.length;r<s;r+=1)t(n[r])}function kr(e,t){var i=fi(e,t);if(i.length!==0){if(i.length===1)return i[0];throw new A("expected a single document in the stream, but found more")}}var Er=wr,Cr=kr,gi={loadAll:Er,load:Cr},mi=Object.prototype.toString,yi=Object.prototype.hasOwnProperty,Je=65279,Tr=9,te=10,Ar=13,Sr=32,Ir=33,Mr=34,ze=35,Br=37,Fr=38,Pr=39,Dr=42,xi=44,Lr=45,xe=58,_r=61,Rr=62,Or=63,Nr=64,vi=91,bi=93,$r=96,wi=123,jr=124,ki=125,T={};T[0]="\\0";T[7]="\\a";T[8]="\\b";T[9]="\\t";T[10]="\\n";T[11]="\\v";T[12]="\\f";T[13]="\\r";T[27]="\\e";T[34]='\\"';T[92]="\\\\";T[133]="\\N";T[160]="\\_";T[8232]="\\L";T[8233]="\\P";var Hr=["y","Y","yes","Yes","YES","on","On","ON","n","N","no","No","NO","off","Off","OFF"],Ur=/^[-+]?[0-9_]+(?::[0-9_]+)+(?:\.[0-9_]*)?$/;function zr(e,t){var i,n,r,s,a,o,l;if(t===null)return{};for(i={},n=Object.keys(t),r=0,s=n.length;r<s;r+=1)a=n[r],o=String(t[a]),a.slice(0,2)==="!!"&&(a="tag:yaml.org,2002:"+a.slice(2)),l=e.compiledTypeMap.fallback[a],l&&yi.call(l.styleAliases,o)&&(o=l.styleAliases[o]),i[a]=o;return i}function Vr(e){var t,i,n;if(t=e.toString(16).toUpperCase(),e<=255)i="x",n=2;else if(e<=65535)i="u",n=4;else if(e<=4294967295)i="U",n=8;else throw new A("code point within a string may not be greater than 0xFFFFFFFF");return"\\"+i+E.repeat("0",n-t.length)+t}var Yr=1,ie=2;function Kr(e){this.schema=e.schema||Ge,this.indent=Math.max(1,e.indent||2),this.noArrayIndent=e.noArrayIndent||!1,this.skipInvalid=e.skipInvalid||!1,this.flowLevel=E.isNothing(e.flowLevel)?-1:e.flowLevel,this.styleMap=zr(this.schema,e.styles||null),this.sortKeys=e.sortKeys||!1,this.lineWidth=e.lineWidth||80,this.noRefs=e.noRefs||!1,this.noCompatMode=e.noCompatMode||!1,this.condenseFlow=e.condenseFlow||!1,this.quotingType=e.quotingType==='"'?ie:Yr,this.forceQuotes=e.forceQuotes||!1,this.replacer=typeof e.replacer=="function"?e.replacer:null,this.implicitTypes=this.schema.compiledImplicit,this.explicitTypes=this.schema.compiledExplicit,this.tag=null,this.result="",this.duplicates=[],this.usedDuplicates=null}function Ft(e,t){for(var i=E.repeat(" ",t),n=0,r=-1,s="",a,o=e.length;n<o;)r=e.indexOf(`
`,n),r===-1?(a=e.slice(n),n=o):(a=e.slice(n,r+1),n=r+1),a.length&&a!==`
`&&(s+=i),s+=a;return s}function Ve(e,t){return`
`+E.repeat(" ",e.indent*t)}function Wr(e,t){var i,n,r;for(i=0,n=e.implicitTypes.length;i<n;i+=1)if(r=e.implicitTypes[i],r.resolve(t))return!0;return!1}function ve(e){return e===Sr||e===Tr}function ne(e){return 32<=e&&e<=126||161<=e&&e<=55295&&e!==8232&&e!==8233||57344<=e&&e<=65533&&e!==Je||65536<=e&&e<=1114111}function Pt(e){return ne(e)&&e!==Je&&e!==Ar&&e!==te}function Dt(e,t,i){var n=Pt(e),r=n&&!ve(e);return(i?n:n&&e!==xi&&e!==vi&&e!==bi&&e!==wi&&e!==ki)&&e!==ze&&!(t===xe&&!r)||Pt(t)&&!ve(t)&&e===ze||t===xe&&r}function Gr(e){return ne(e)&&e!==Je&&!ve(e)&&e!==Lr&&e!==Or&&e!==xe&&e!==xi&&e!==vi&&e!==bi&&e!==wi&&e!==ki&&e!==ze&&e!==Fr&&e!==Dr&&e!==Ir&&e!==jr&&e!==_r&&e!==Rr&&e!==Pr&&e!==Mr&&e!==Br&&e!==Nr&&e!==$r}function qr(e){return!ve(e)&&e!==xe}function Z(e,t){var i=e.charCodeAt(t),n;return i>=55296&&i<=56319&&t+1<e.length&&(n=e.charCodeAt(t+1),n>=56320&&n<=57343)?(i-55296)*1024+n-56320+65536:i}function Ei(e){var t=/^\n* /;return t.test(e)}var Ci=1,Ye=2,Ti=3,Ai=4,H=5;function Qr(e,t,i,n,r,s,a,o){var l,c=0,u=null,d=!1,p=!1,f=n!==-1,g=-1,h=Gr(Z(e,0))&&qr(Z(e,e.length-1));if(t||a)for(l=0;l<e.length;c>=65536?l+=2:l++){if(c=Z(e,l),!ne(c))return H;h=h&&Dt(c,u,o),u=c}else{for(l=0;l<e.length;c>=65536?l+=2:l++){if(c=Z(e,l),c===te)d=!0,f&&(p=p||l-g-1>n&&e[g+1]!==" ",g=l);else if(!ne(c))return H;h=h&&Dt(c,u,o),u=c}p=p||f&&l-g-1>n&&e[g+1]!==" "}return!d&&!p?h&&!a&&!r(e)?Ci:s===ie?H:Ye:i>9&&Ei(e)?H:a?s===ie?H:Ye:p?Ai:Ti}function Jr(e,t,i,n,r){e.dump=(function(){if(t.length===0)return e.quotingType===ie?'""':"''";if(!e.noCompatMode&&(Hr.indexOf(t)!==-1||Ur.test(t)))return e.quotingType===ie?'"'+t+'"':"'"+t+"'";var s=e.indent*Math.max(1,i),a=e.lineWidth===-1?-1:Math.max(Math.min(e.lineWidth,40),e.lineWidth-s),o=n||e.flowLevel>-1&&i>=e.flowLevel;function l(c){return Wr(e,c)}switch(Qr(t,o,e.indent,a,l,e.quotingType,e.forceQuotes&&!n,r)){case Ci:return t;case Ye:return"'"+t.replace(/'/g,"''")+"'";case Ti:return"|"+Lt(t,e.indent)+_t(Ft(t,s));case Ai:return">"+Lt(t,e.indent)+_t(Ft(Xr(t,a),s));case H:return'"'+Zr(t)+'"';default:throw new A("impossible error: invalid scalar style")}})()}function Lt(e,t){var i=Ei(e)?String(t):"",n=e[e.length-1]===`
`,r=n&&(e[e.length-2]===`
`||e===`
`),s=r?"+":n?"":"-";return i+s+`
`}function _t(e){return e[e.length-1]===`
`?e.slice(0,-1):e}function Xr(e,t){for(var i=/(\n+)([^\n]*)/g,n=(function(){var c=e.indexOf(`
`);return c=c!==-1?c:e.length,i.lastIndex=c,Rt(e.slice(0,c),t)})(),r=e[0]===`
`||e[0]===" ",s,a;a=i.exec(e);){var o=a[1],l=a[2];s=l[0]===" ",n+=o+(!r&&!s&&l!==""?`
`:"")+Rt(l,t),r=s}return n}function Rt(e,t){if(e===""||e[0]===" ")return e;for(var i=/ [^ ]/g,n,r=0,s,a=0,o=0,l="";n=i.exec(e);)o=n.index,o-r>t&&(s=a>r?a:o,l+=`
`+e.slice(r,s),r=s+1),a=o;return l+=`
`,e.length-r>t&&a>r?l+=e.slice(r,a)+`
`+e.slice(a+1):l+=e.slice(r),l.slice(1)}function Zr(e){for(var t="",i=0,n,r=0;r<e.length;i>=65536?r+=2:r++)i=Z(e,r),n=T[i],!n&&ne(i)?(t+=e[r],i>=65536&&(t+=e[r+1])):t+=n||Vr(i);return t}function ea(e,t,i){var n="",r=e.tag,s,a,o;for(s=0,a=i.length;s<a;s+=1)o=i[s],e.replacer&&(o=e.replacer.call(i,String(s),o)),(F(e,t,o,!1,!1)||typeof o>"u"&&F(e,t,null,!1,!1))&&(n!==""&&(n+=","+(e.condenseFlow?"":" ")),n+=e.dump);e.tag=r,e.dump="["+n+"]"}function Ot(e,t,i,n){var r="",s=e.tag,a,o,l;for(a=0,o=i.length;a<o;a+=1)l=i[a],e.replacer&&(l=e.replacer.call(i,String(a),l)),(F(e,t+1,l,!0,!0,!1,!0)||typeof l>"u"&&F(e,t+1,null,!0,!0,!1,!0))&&((!n||r!=="")&&(r+=Ve(e,t)),e.dump&&te===e.dump.charCodeAt(0)?r+="-":r+="- ",r+=e.dump);e.tag=s,e.dump=r||"[]"}function ta(e,t,i){var n="",r=e.tag,s=Object.keys(i),a,o,l,c,u;for(a=0,o=s.length;a<o;a+=1)u="",n!==""&&(u+=", "),e.condenseFlow&&(u+='"'),l=s[a],c=i[l],e.replacer&&(c=e.replacer.call(i,l,c)),F(e,t,l,!1,!1)&&(e.dump.length>1024&&(u+="? "),u+=e.dump+(e.condenseFlow?'"':"")+":"+(e.condenseFlow?"":" "),F(e,t,c,!1,!1)&&(u+=e.dump,n+=u));e.tag=r,e.dump="{"+n+"}"}function ia(e,t,i,n){var r="",s=e.tag,a=Object.keys(i),o,l,c,u,d,p;if(e.sortKeys===!0)a.sort();else if(typeof e.sortKeys=="function")a.sort(e.sortKeys);else if(e.sortKeys)throw new A("sortKeys must be a boolean or a function");for(o=0,l=a.length;o<l;o+=1)p="",(!n||r!=="")&&(p+=Ve(e,t)),c=a[o],u=i[c],e.replacer&&(u=e.replacer.call(i,c,u)),F(e,t+1,c,!0,!0,!0)&&(d=e.tag!==null&&e.tag!=="?"||e.dump&&e.dump.length>1024,d&&(e.dump&&te===e.dump.charCodeAt(0)?p+="?":p+="? "),p+=e.dump,d&&(p+=Ve(e,t)),F(e,t+1,u,!0,d)&&(e.dump&&te===e.dump.charCodeAt(0)?p+=":":p+=": ",p+=e.dump,r+=p));e.tag=s,e.dump=r||"{}"}function Nt(e,t,i){var n,r,s,a,o,l;for(r=i?e.explicitTypes:e.implicitTypes,s=0,a=r.length;s<a;s+=1)if(o=r[s],(o.instanceOf||o.predicate)&&(!o.instanceOf||typeof t=="object"&&t instanceof o.instanceOf)&&(!o.predicate||o.predicate(t))){if(i?o.multi&&o.representName?e.tag=o.representName(t):e.tag=o.tag:e.tag="?",o.represent){if(l=e.styleMap[o.tag]||o.defaultStyle,mi.call(o.represent)==="[object Function]")n=o.represent(t,l);else if(yi.call(o.represent,l))n=o.represent[l](t,l);else throw new A("!<"+o.tag+'> tag resolver accepts not "'+l+'" style');e.dump=n}return!0}return!1}function F(e,t,i,n,r,s,a){e.tag=null,e.dump=i,Nt(e,i,!1)||Nt(e,i,!0);var o=mi.call(e.dump),l=n,c;n&&(n=e.flowLevel<0||e.flowLevel>t);var u=o==="[object Object]"||o==="[object Array]",d,p;if(u&&(d=e.duplicates.indexOf(i),p=d!==-1),(e.tag!==null&&e.tag!=="?"||p||e.indent!==2&&t>0)&&(r=!1),p&&e.usedDuplicates[d])e.dump="*ref_"+d;else{if(u&&p&&!e.usedDuplicates[d]&&(e.usedDuplicates[d]=!0),o==="[object Object]")n&&Object.keys(e.dump).length!==0?(ia(e,t,e.dump,r),p&&(e.dump="&ref_"+d+e.dump)):(ta(e,t,e.dump),p&&(e.dump="&ref_"+d+" "+e.dump));else if(o==="[object Array]")n&&e.dump.length!==0?(e.noArrayIndent&&!a&&t>0?Ot(e,t-1,e.dump,r):Ot(e,t,e.dump,r),p&&(e.dump="&ref_"+d+e.dump)):(ea(e,t,e.dump),p&&(e.dump="&ref_"+d+" "+e.dump));else if(o==="[object String]")e.tag!=="?"&&Jr(e,e.dump,t,s,l);else{if(o==="[object Undefined]")return!1;if(e.skipInvalid)return!1;throw new A("unacceptable kind of an object to dump "+o)}e.tag!==null&&e.tag!=="?"&&(c=encodeURI(e.tag[0]==="!"?e.tag.slice(1):e.tag).replace(/!/g,"%21"),e.tag[0]==="!"?c="!"+c:c.slice(0,18)==="tag:yaml.org,2002:"?c="!!"+c.slice(18):c="!<"+c+">",e.dump=c+" "+e.dump)}return!0}function na(e,t){var i=[],n=[],r,s;for(Ke(e,i,n),r=0,s=n.length;r<s;r+=1)t.duplicates.push(i[n[r]]);t.usedDuplicates=new Array(s)}function Ke(e,t,i){var n,r,s;if(e!==null&&typeof e=="object")if(r=t.indexOf(e),r!==-1)i.indexOf(r)===-1&&i.push(r);else if(t.push(e),Array.isArray(e))for(r=0,s=e.length;r<s;r+=1)Ke(e[r],t,i);else for(n=Object.keys(e),r=0,s=n.length;r<s;r+=1)Ke(e[n[r]],t,i)}function ra(e,t){t=t||{};var i=new Kr(t);i.noRefs||na(e,i);var n=e;return i.replacer&&(n=i.replacer.call({"":n},"",n)),F(i,0,n,!0,!0)?i.dump+`
`:""}var aa=ra,sa={dump:aa};function Xe(e,t){return function(){throw new Error("Function yaml."+e+" is removed in js-yaml 4. Use yaml."+t+" instead, which is now safe by default.")}}var oa=C,la=Ht,ca=Yt,ua=Qt,da=Jt,pa=Ge,ha=gi.load,fa=gi.loadAll,ga=sa.dump,ma=A,ya={binary:ii,float:qt,map:Vt,null:Kt,pairs:ri,set:ai,timestamp:ei,bool:Wt,int:Gt,merge:ti,omap:ni,seq:zt,str:Ut},xa=Xe("safeLoad","load"),va=Xe("safeLoadAll","loadAll"),ba=Xe("safeDump","dump"),N={Type:oa,Schema:la,FAILSAFE_SCHEMA:ca,JSON_SCHEMA:ua,CORE_SCHEMA:da,DEFAULT_SCHEMA:pa,load:ha,loadAll:fa,dump:ga,YAMLException:ma,types:ya,safeLoad:xa,safeLoadAll:va,safeDump:ba};var $={name:"",occupation:"",company:"",city:"",skills:[],roles:[],relationships:[],goals:[],focusAreas:[],lastUpdated:new Date().toISOString().split("T")[0]};function Si(e){let t={...$},i=e.match(/^---\n([\s\S]*?)\n---/);if(i)try{let n=N.load(i[1]);n&&(typeof n.name=="string"&&(t.name=n.name),typeof n.occupation=="string"&&(t.occupation=n.occupation),typeof n.company=="string"&&(t.company=n.company),typeof n.city=="string"&&(t.city=n.city),Array.isArray(n.skills)&&(t.skills=n.skills.map(String)),Array.isArray(n.roles)&&(t.roles=n.roles.map(String)),Array.isArray(n.relationships)&&(t.relationships=n.relationships.map(String)),Array.isArray(n.goals)&&(t.goals=n.goals.map(String)),Array.isArray(n.focusAreas)&&(t.focusAreas=n.focusAreas.map(String)))}catch{}return!t.name&&!t.company?wa(e):t}function wa(e){let t={...$},i=e.split(`
`),n="";for(let r of i){let s=r.trim();if(s.startsWith("## ")){n=s.slice(3);continue}if(n==="\u57FA\u672C\u4FE1\u606F"&&s.startsWith("- ")){let a=s.slice(2),[o,...l]=a.split("\uFF1A"),c=l.join("\uFF1A");o==="\u59D3\u540D"&&(t.name=c),(o==="\u804C\u4E1A"||o==="\u804C\u4F4D")&&(t.occupation=c),(o==="\u516C\u53F8/\u7EC4\u7EC7"||o==="\u516C\u53F8")&&(t.company=c),o==="\u57CE\u5E02"&&(t.city=c)}if(s.startsWith("- ")&&!s.includes("\uFF1A")){let a=s.slice(2);if(a==="_\u6682\u65E0_")continue;n==="\u6280\u80FD\u4E0E\u4E13\u4E1A"&&t.skills.push(a),n==="\u89D2\u8272\u4E0E\u5173\u7CFB"&&t.roles.push(a),n==="\u76EE\u6807\u4E0E\u8BA1\u5212"&&t.goals.push(a),n==="\u5173\u6CE8\u9886\u57DF"&&t.focusAreas.push(a)}if(s.startsWith("- \u5173\u7CFB\uFF1A")&&n==="\u89D2\u8272\u4E0E\u5173\u7CFB"){let a=s.slice(5);a!=="_\u6682\u65E0_"&&t.relationships.push(a)}}return t}var ka="TraceMind/PROFILE.md";async function Ii(e){let t=e.vault.getFileByPath(ka);if(!t)return{...$};try{let i=await e.vault.read(t);return Si(i)}catch{return{...$}}}function re(e){let t=0;for(let i=0;i<e.length;i++){let n=e.charCodeAt(i);t=(t<<5)-t+n,t=t&t}return Math.abs(t).toString(16).padStart(8,"0").slice(0,8)}var Ea=["company","role","relationship_to_user"],Ca=["responsibility","communicationStyle"],Ta=["personality","preferences","skills"],Aa=["subtype","status"],Sa=["deadline","description"],Ia=["priority","goals"],Ma=["subtype"],Ba=["occurrenceCount","context"],Fa=["goals","emotions"],Pa={project:"P0",task:"P0",product:"P1",technology:"P1",document:"P2",location:"P2",other:"P2"},Os=Object.keys(Pa);var Ze={P0:1.5,P1:1,P2:.5},we={person:{p0:Ea,p1:Ca,p2:Ta},object:{p0:Aa,p1:Sa,p2:Ia},theme:{p0:Ma,p1:Ba,p2:Fa}};function et(e,t){let i=we[e],n=i.p0.length===0||i.p0.every(a=>t[a]!=null),r=i.p1.some(a=>t[a]!=null),s=i.p2.some(a=>t[a]!=null);return n&&r&&s?"L3":n&&r?"L2":n?"L1":"L0"}function Mi(e,t,i,n=1){let r=we[e];if(r.p0.length+r.p1.length+r.p2.length===0)return 0;let s=0,a=r.p0.filter(c=>t[c]!=null).length,o=a/Math.max(r.p0.length,1),l=a===0&&r.p0.length>0?r.p0.length/3*.5:o;if(s+=l*Ze.P0,r.p1.length>0){let c=r.p1.filter(u=>t[u]!=null).length;s+=c/r.p1.length*Ze.P1}if(r.p2.length>0){let c=r.p2.filter(u=>t[u]!=null).length;s+=c/r.p2.length*Ze.P2}return s*n*(1+Math.log1p(i))}var tt={create(e){let t=new Date().toISOString();return{id:re(e.name),userId:e.userId||"",cardType:e.cardType,name:e.name,aliases:e.aliases||[],attributes:e.attributes||{},relatedPeople:[],relatedObjects:[],relatedThemes:[],evidenceEntryIds:[],confidence:.5,maturity:et(e.cardType,e.attributes||{}),status:"needs_confirmation",lifecycle:"candidate",importance:0,createdAt:t,lastUpdated:t}}};var Da=N.dump,La=N.load,ke="---";function Y(e){let t={id:e.id,name:e.name,type:e.cardType,maturity:e.maturity,confidence:e.confidence,status:e.status,aliases:e.aliases,createdAt:e.createdAt,lastUpdated:e.lastUpdated};for(let[a,o]of Object.entries(e.attributes))o!=null&&(t[a]=o);let i=[`# ${e.name}`,""],n=Ra(e);n.length>0&&(i.push("## \u57FA\u672C\u4FE1\u606F"),i.push(...n),i.push(""));let r=e.attributes.interactions||[];if(r.length>0){i.push("## \u4E92\u52A8\u8BB0\u5F55");for(let a of r.slice(-5)){let o=a.timestamp?new Date(a.timestamp).toISOString().split("T")[0]:"";i.push("- "+o+" "+a.content)}i.push("")}else e.attributes.interactions;let s=Da(t).trim();return`${ke}
${s}
${ke}

${i.join(`
`)}`}function ae(e){let t=_a(e),i=t.frontmatter,n=t.body,r=i.type,s=Array.isArray(i.aliases)?i.aliases:[],a=new Set(["id","name","type","maturity","confidence","status","aliases","createdAt","lastUpdated","lifecycle","importance","userId","relatedPeople","relatedObjects","relatedThemes","evidenceEntryIds"]),o={};for(let[l,c]of Object.entries(i))!a.has(l)&&c!=null&&(o[l]=c);return{id:i.id||re(i.name),userId:"",cardType:r,name:i.name,aliases:s,attributes:o,relatedPeople:[],relatedObjects:[],relatedThemes:[],evidenceEntryIds:[],confidence:i.confidence??.5,maturity:i.maturity??"L0",status:i.status??"needs_confirmation",lifecycle:"candidate",importance:0,createdAt:i.createdAt||new Date().toISOString(),lastUpdated:i.lastUpdated||new Date().toISOString()}}function _a(e){let t=e.trim();if(!t.startsWith(ke))return{frontmatter:{},body:t};let i=t.indexOf(ke,3);if(i===-1)return{frontmatter:{},body:t};let n=t.slice(3,i).trim(),r=t.slice(i+3).trim();return{frontmatter:La(n)||{},body:r}}function Ra(e){let t=[];return e.cardType==="person"&&(e.attributes.company&&t.push("- \u516C\u53F8\uFF1A"+e.attributes.company),e.attributes.role&&t.push("- \u804C\u4F4D\uFF1A"+e.attributes.role),e.attributes.relationship_to_user&&t.push("- \u5173\u7CFB\uFF1A"+e.attributes.relationship_to_user),e.attributes.responsibility&&t.push("- \u804C\u8D23\uFF1A"+e.attributes.responsibility)),e.cardType==="object"&&(e.attributes.subtype&&t.push("- \u7C7B\u578B\uFF1A"+e.attributes.subtype),e.attributes.status&&t.push("- \u72B6\u6001\uFF1A"+e.attributes.status),e.attributes.deadline&&t.push("- \u622A\u6B62\u65E5\u671F\uFF1A"+e.attributes.deadline)),e.cardType==="theme"&&(e.attributes.subtype&&t.push("- \u7C7B\u578B\uFF1A"+e.attributes.subtype),e.attributes.occurrenceCount&&t.push("- \u51FA\u73B0\u6B21\u6570\uFF1A"+e.attributes.occurrenceCount)),t}function Oa(e,t){let i=`\u4F60\u662F\u4E00\u4E2A\u7CBE\u51C6\u7684\u5B9E\u4F53\u63D0\u53D6\u548C\u65E5\u8BB0\u5206\u7C7B\u4E13\u5BB6\u3002\u8BF7\u5BF9\u4EE5\u4E0B\u65E5\u8BB0\u6587\u672C\u8FDB\u884C\u5206\u6790\u3002
...
## \u5173\u6CE8\u9886\u57DF
- _\u6682\u65E0_
`),n.setButtonText("\u5DF2\u5B8C\u6210"),n.setDisabled(!0),new P.Notice("\u6863\u6848\u521B\u5EFA\u5B8C\u6210")})}),t.createEl("h3",{text:"\u6B65\u9AA4 3\uFF1A\u5F00\u59CB\u4F7F\u7528"}),new P.Setting(t).setName("\u5B8C\u6210\u8BBE\u7F6E").setDesc("\u8BBE\u7F6E\u5B8C\u6210\u540E\uFF0C\u4F60\u53EF\u4EE5\u5F00\u59CB\u4F7F\u7528 TraceMind \u4E86").addButton(n=>{n.setButtonText("\u5B8C\u6210"),n.setCta(),n.onClick(async()=>{this.close(),await this.onComplete()})})}onClose(){let{contentEl:t}=this;t.empty()}};var Ja=/^---\n([\s\S]*?)\n---\n?/;function $i(e){let t=e.match(Ja);if(!t)return null;let i=t[1],n=e.slice(t[0].length),r={};for(let c of i.split(`
`)){let u=c.indexOf(":");if(u===-1)continue;let d=c.slice(0,u).trim(),p=c.slice(u+1).trim();d&&p&&(r[d]=p)}let s=r.date,a=r.contentHash,o=r.generatedAt,l=parseInt(r.blockCount||"0",10);return!s||!a||!o||!n.trim()?null:{date:s,content:n.trim(),contentHash:a,generatedAt:o,blockCount:l}}function ji(e){return`${["---",`date: ${e.date}`,`generatedAt: ${e.generatedAt}`,`contentHash: ${e.contentHash}`,`blockCount: ${e.blockCount}`,"---"].join(`
`)}

${e.content}
`}function ot(e){return`TraceMind/insights/${e}.md`}pe();W();var Hi=/^###\s+(\d{2}:\d{2})\s+(.+)$/m,Xa=/<!--\s*TM:([a-z0-9]+)\s*-->/;function Za(){return Math.random().toString(16).slice(2,10).padStart(8,"0")}function lt(e){let t=[],i=e.split(`
`),n=0;for(;n<i.length;){let s=i[n].match(Hi);if(s){let a=s[1],l=s[2].trim().split(/\s+/).filter(d=>d.startsWith("#")).map(d=>d.slice(1));n++;let c=[],u=[];for(;n<i.length;){let d=i[n];if(d.match(Hi))break;if(!d.trim()){n++;continue}let p=d.match(Xa);if(p){let f=p[1];t.push({timestamp:a,content:c.join(`
