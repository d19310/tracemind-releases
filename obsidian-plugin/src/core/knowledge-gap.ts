...
import { CardType, MaturityLevel } from './context-card';
import { getAttributesByPriority, hasAttribute, normalizeEntityAttributes } from './entity-schema';
...
export function detectKnowledgeGaps(
  ...
  // Read subtype for subtype-aware priority lists
  ...
  for (const attr of priority.p0) {
    if (!hasAttribute(normalized, attr)) {
      gaps.push({
