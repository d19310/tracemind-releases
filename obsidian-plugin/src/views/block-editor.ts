...
const DEFAULT_SLOGAN = '记录，是AI时代的人生复利。';
...
export class BlockEditorView extends ItemView {
	...
	private async renderView(): Promise<void> {
		...
		// Tagline on right — read from diary file's [!NOTE], fall back to default
		const slogan = await this.loadSlogan();
	...
	/**
	 * Read the slogan from the diary file's > [!NOTE] line.
	 * Falls back to DEFAULT_SLOGAN if file not found or no NOTE line.
	 */
	private async loadSlogan(): Promise<string> {
		const dailyPath = `Daily/${this.currentDate}.md`;
		let file = this.app.vault.getAbstractFileByPath(dailyPath);
