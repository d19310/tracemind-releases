...
import { buildClarificationAttributeGuide, buildVaultSchemaGuide, getSubtypeLabel } from '../ai/entity-type-config';
...
export class AIAnalysisPanelView extends ItemView {
	...
	private updateSendBtnState() {
		...
		// Loading state: disable button and change icon
		if (this.isLoading) {
			this.sendBtnEl.setAttr('disabled', 'true');
			this.sendBtnEl.setAttr('title', '处理中...');
			setIcon(this.sendBtnEl, 'loader-2');
		} else {
			this.sendBtnEl.removeAttribute('disabled');
			this.sendBtnEl.setAttr('title', '发送');
			setIcon(this.sendBtnEl, 'arrow-up');
		}
	}
...
@media (max-width: 400px) {
	...
	private async loadOrGenerateInsight() {
		...
		const callbacks: InsightStreamCallbacks = {
			...
			onDone: (_fullText: string) => { void _fullText;
	...
	private async parseClarificationResponse(
		...
		const subtypeLabel = entity.subtype
			? getSubtypeLabel(entity.type, entity.subtype) || entity.subtype
			: '';
		const typeDesc = subtypeLabel
			? `${entity.type} / ${entity.subtype}（${subtypeLabel}）`
			: entity.type;
		const contextParts = [
			'用户回答了关于「' + entity.name + '」（类型：' + typeDesc + '）的澄清问题。',
		];
		if (profileContext) {
			contextParts.push('');
		...
		const parsePrompt = contextParts.concat([
			'\u8BF7\u4ECE\u7528\u6237\u56DE\u7B54\u4E2D\u63D0\u53D6\u5173\u952E\u5C5E\u6027\u4FE1\u606F\u3002',
			'',
			'当前实体类型是 ' + typeDesc + '，可用的属性名为：',
			buildClarificationAttributeGuide(entity.type, entity.subtype),
			'',
	 ...
	 */
	private normalizeAttributes(raw: Record<string, string>, entityType: EntityType): Record<string, string> { void entityType;
	...
	private buildChatSystemPrompt(): string {
		...
		parts.push('');
		parts.push(buildVaultSchemaGuide());
	...
	) { void systemMessage; void messages;
		const sessionManager = this.plugin.getSessionManager();
