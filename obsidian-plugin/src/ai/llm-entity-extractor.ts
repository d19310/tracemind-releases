...
import { buildRequest, parseResponse, createProviderHttpError, validateConfig } from './provider-config';
import type { ProviderType } from './provider-config';
...
export async function extractEntitiesWithLLM(
  ...
  const req = buildRequest(config, [{ role: 'user', content: prompt }]);
  console.log('[TraceMind] LLM URL:', req.url);
  ...
  const msg = parseResponse(provider, body);
  console.log('[TraceMind] LLM raw response:', msg.content);

  const output = parseLLMResponse(msg.content);
